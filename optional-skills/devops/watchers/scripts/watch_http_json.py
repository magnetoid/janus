...
Usage (via cron with --no-agent):
    ...
    janus cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$JANUS_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Janus-Watcher/1.0"})
