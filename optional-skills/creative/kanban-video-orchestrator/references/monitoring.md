...
janus kanban watch --tenant <project-slug>

# Snapshot of the board
janus kanban list --tenant <project-slug>
janus kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
janus kanban stats --tenant <project-slug>

# Visual dashboard (browser)
janus dashboard

# Inspect a specific task (includes comments + events)
janus kanban show <task-id>

# Follow a single task's event stream
janus kanban tail <task-id>
```

Verify available subcommands with `janus kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `janus kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `janus kanban block <id>` then `janus kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `janus kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `janus kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `janus profile list` (profile exists?), `janus status` (gateway/dispatcher up?), and verify tenant. |
...
janus kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
janus kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(janus kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
janus kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
janus kanban block <task-id>
janus kanban archive <task-id>

# Diagnose what happened
janus kanban show <task-id>      # task body, comments, recent events
janus kanban tail <task-id>      # follow the live event stream
janus kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `janus kanban create` for the director
...
while true; do
    clear
    janus kanban list --tenant <slug>
    echo "---"
    janus kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `janus kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `janus profile list` and start
