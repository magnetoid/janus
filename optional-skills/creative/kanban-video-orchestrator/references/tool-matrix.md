...
Maps each role archetype to the Janus skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public janus-agent
...
## Janus skills relevant to video production

### Visual / rendering skills (`janus-agent/skills/creative/`)
...
### Design / pre-production skills (`janus-agent/skills/creative/`)
...
### Audio / media skills (`janus-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`janus-agent/skills/devops/`)
...
These are **not** Janus skills but external CLIs / APIs that profiles invoke.
...
## Built-in Janus tools for media review

These are native Janus tools — not invoked via terminal but through their own
...
key is present in `~/.janus/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Janus profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.janus/.env` → environment variable.
...
When a new Janus-public video skill ships:
