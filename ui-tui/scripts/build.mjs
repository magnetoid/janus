...
await build({
  ...
  // Skip the prebuilt @janus/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@janus/ink': resolve(root, 'packages/janus-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The janus_cli launcher
