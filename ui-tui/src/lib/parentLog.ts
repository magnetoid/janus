...
const logDir = join(process.env.JANUS_HOME?.trim() || join(homedir(), '.janus'), 'logs')
...
// a real ~/.janus (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('janus-tui: parent lifecycle log unavailable\n')
    }
  }
}
