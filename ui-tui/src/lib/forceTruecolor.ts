...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.JANUS_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Janus on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via JANUS_TUI_TRUECOLOR=1.
