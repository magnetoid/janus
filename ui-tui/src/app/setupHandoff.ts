import type { RunExternalProcess } from '@janus/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`janus ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching janus: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`janus ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
