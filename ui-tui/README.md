...
React + Ink terminal UI for Janus. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `JANUS_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.janus/.janus_history` or under `JANUS_HOME`.
...
```text
ui-tui/
  packages/janus-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      janus-ink.d.ts    type declarations for @janus/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent JanusCLI subprocess for slash commands
```
