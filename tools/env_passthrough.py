...
def _is_janus_provider_credential(name: str) -> bool:
    """True if ``name`` is a Janus-managed provider credential (API key,
    token, or similar) per ``_JANUS_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Janus API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    try:
        ...
        return False
    return name in _JANUS_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Janus-managed provider credentials (from
    ``_JANUS_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Janus-managed
    ...
    Non-Janus third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_janus_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Janus provider "
                "credential %r (blocked by _JANUS_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Janus-managed provider credentials must not be passed
                ...
                if _is_janus_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Janus "
                        "provider credential %r from config.yaml (blocked "
                        "by _JANUS_PROVIDER_ENV_BLOCKLIST). Operator "
