...
~/.janus/skills/. Existing skills (bundled, hub-installed, or user-created)
...
# All skills live in ~/.janus/skills/ (single source of truth)
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`janus curator unpin {name}` if they want to delete it. "
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.janus/skills/) first, then any
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER Janus profiles.
    ...
    # Default profile (~/.janus/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.janus/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`janus -p "
            ...
            base += (
                ...
                f"Switch profiles (`janus -p <name>`) to edit there, or "
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_janus_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        ...
        "will refuse with a message pointing the user to `janus curator unpin <name>`. "
