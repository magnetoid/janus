...
This module provides the glue:
    - ``JanusTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Janus Agent"
"""
...
from janus_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__janus_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses JANUS_HOME so each profile gets its own OAuth tokens.
    Layout: ``JANUS_HOME/mcp-tokens/``
    """
    try:
        from janus_constants import get_janus_home
        base = Path(get_janus_home())
    except ImportError:
        base = Path(os.environ.get("JANUS_HOME", str(Path.home() / ".janus")))
...
# ---------------------------------------------------------------------------
# JanusTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class JanusTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        JANUS_HOME/mcp-tokens/<server_name>.json         -- tokens
        JANUS_HOME/mcp-tokens/<server_name>.client.json   -- client info
        JANUS_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Janus records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Janus.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://github.com/magnetoid/janus",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `janus mcp login <server>` later to "
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Janus Agent")
    scope = cfg.get("scope")
