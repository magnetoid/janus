...
One ``CDPSupervisor`` runs per Janus ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "janus-dialog-bridge.invalid"
...
(() => {
  if (window.__janusDialogBridgeInstalled) return;
  window.__janusDialogBridgeInstalled = true;
  const ENDPOINT = "http://janus-dialog-bridge.invalid/";
