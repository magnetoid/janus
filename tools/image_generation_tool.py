...
selectable via ``janus tools`` → Image Generation; the active model is
...
FAL_MODELS: Dict[str, Dict[str, Any]] = {
    ...
    # existing FAL key / Cloud Industry Portal subscription rather than register
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                ...
                f"the Cloud Industry Portal's FAL proxy. Either:\n"
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        if remote_home:
            return f"{str(remote_home).rstrip('/')}/.janus"
    ...
    # Janus cache roots can be translated without side effects. SSH can still
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed_nous_tools_enabled():
        lines.append(
            ...
            "gateway enabled (`janus setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `janus tools` "
        "→ Image Generation (run `janus plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
...
def check_image_generation_requirements() -> bool:
    ...
    the historical behavior: shipping janus with a FAL key configured
