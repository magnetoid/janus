...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("JANUS_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("JANUS_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("JANUS_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set JANUS_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind JANUS_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds JANUS_SESSION_PLATFORM via
    ...
    if env_var_enabled("JANUS_CRON_SESSION"):
        return False
    if env_var_enabled("JANUS_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $JANUS_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_JANUS_ENV_PATH = (
    r'(?:~\/\.janus/|'
    r'(?:\$home|\$\{home\})/\.janus/|'
    r'(?:\$janus_home|\$\{janus_home\})/)'
    r'\.env\b'
)
# ~/.janus/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _JANUS_ENV_PATH; matches the JANUS_HOME override form as
# well as ~/.janus/.
_JANUS_CONFIG_PATH = (
    r'(?:~\/\.janus/|'
    r'(?:\$home|\$\{home\})/\.janus/|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bjanus\s+gateway\s+(stop|restart)\b', "stop/restart janus gateway (kills running agents)"),
    (r'\bjanus\s+update\b', "janus update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `janus gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart janus-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart janus-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(janus|gateway|cli\.py)\b', "kill janus/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill janus` but not
    # `kill -9 $(pgrep -f janus)` because the substitution is opaque
    ...
    # In-place edit of a Janus-managed security file (~/.janus/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_JANUS_CONFIG_PATH}|{_JANUS_ENV_PATH})', "in-place edit of Janus config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_JANUS_CONFIG_PATH}|{_JANUS_ENV_PATH})', "in-place edit of Janus config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_JANUS_CONFIG_PATH}|{_JANUS_ENV_PATH})', "in-place edit of Janus config/env (perl/ruby)"),
