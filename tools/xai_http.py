...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``janus tools`` repaint, tool-registration scans,
    ...
    2. ``~/.janus/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_janus_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.janus/.env`` first, then ``os.environ``.

    Wraps :func:`janus_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from janus_cli.config import get_env_value as _janus_get_env_value

        value = _janus_get_env_value(name)
...
def janus_xai_user_agent() -> str:
    """Return a stable Janus-specific User-Agent for xAI HTTP calls."""
    try:
        from janus_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Janus-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Janus-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``janus_cli.config.get_env_value`` so keys
    stored in ``~/.janus/.env`` (the standard Janus location) are honored —
