"""CLI entry point for the janus-agent ACP adapter.

Loads environment variables from ``~/.janus/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from JANUS_HOME (default ``~/.janus``)."""
    from janus_cli.env_loader import load_janus_dotenv

    janus_home = get_janus_home()
    loaded = load_janus_dotenv(janus_home=janus_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", janus_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="janus-acp",
        description="Run Janus Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Janus version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _run_check() -> None:
    ...
    from acp_adapter.server import JanusACPAgent  # noqa: F401

    print("Janus ACP check OK")


def _run_setup() -> None:
    from janus_cli.main import main as janus_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "janus", "model"]
        janus_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``janus postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting janus-agent ACP adapter")
    ...
    import acp
    from .server import JanusACPAgent
