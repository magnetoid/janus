"""ACP auth helpers — detect and advertise Janus authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "janus-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Janus runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Janus can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Janus.
    ...
    not have Janus provider credentials configured yet, so Janus always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Janus using the currently configured "
