# Janus Observer Hooks

Janus observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Janus' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Janus catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Janus only builds
