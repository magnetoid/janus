...
Janus middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Janus'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Janus calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Janus retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Janus guardrails,
...
- `telemetry_schema_version`: currently `janus.observer.v1`
- `middleware_schema_version`: currently `janus.middleware.v1`
...
Janus stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Janus runs
them as a nested chain in registration order. Middleware failures are fail-open:
Janus logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Janus applies middleware in this order:
...
For each tool call, Janus applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Janus pre-execution path against the effective arguments:
...
```bash
janus plugins enable <plugin-name>
```

For isolated local testing, use one `JANUS_HOME` for plugin enablement and the
agent run:

```bash
export JANUS_HOME=/tmp/janus-middleware-test
mkdir -p "$JANUS_HOME"
janus plugins enable <plugin-name>
janus chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run janus plugins enable <plugin-name>
uv run janus chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["janus_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/janus-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Janus expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Janus treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Janus preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Janus does not convert downstream
