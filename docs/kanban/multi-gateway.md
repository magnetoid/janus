...
Janus supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.janus/config.yaml`:
...
Or set the env var: `JANUS_KANBAN_DISPATCH_IN_GATEWAY=false`
