...
When running Janus inside Docker, the default `network_mode: host` gives the
...
The Janus [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   JANUS_UID=$(id -u) JANUS_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=janus,janus-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://janus-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Janus trust model and vulnerability reporting
