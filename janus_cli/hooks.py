"""janus hooks — inspect and manage shell-script hooks.

Usage::

    janus hooks list
    janus hooks test <event> [--for-tool X] [--payload-file F]
    janus hooks revoke <command>
    janus hooks doctor

Consent records live under ``~/.janus/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.janus/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``janus hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: janus hooks {list|test|revoke|doctor}")
        print("Run 'janus hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.janus/config.yaml.")
        print("See `janus hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `janus hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `janus hooks test` and `janus hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Janus wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `janus hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `janus hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `janus hooks doctor`.")
