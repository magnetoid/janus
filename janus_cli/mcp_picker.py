"""MCP picker — interactive `janus mcp picker` (also the default `janus mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``janus mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `janus plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from janus_cli.cli_output import prompt_yes_no
from janus_cli.curses_ui import curses_single_select
from janus_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Janus session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from janus_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `janus mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: janus mcp install <name>    Picker: janus mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Janus is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Janus — run `janus update` "
...
def show_catalog() -> None:
    """`janus mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`janus mcp picker` (and default `janus mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`janus mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `janus mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
