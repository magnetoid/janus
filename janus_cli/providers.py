"""
Single source of truth for provider identity in Janus Agent.
...
2. **Janus overlays** — transport type, auth patterns, aggregator flags,
...
# -- Janus overlay ----------------------------------------------------------
# Janus-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class JanusOverlay:
    """Janus-specific provider metadata layered on top of models.dev."""
...
JANUS_OVERLAYS: Dict[str, JanusOverlay] = {
    "openrouter": JanusOverlay(
    ...
    "nous": JanusOverlay(
        ...
        base_url_override="https://inference-api.cloud-industry.com/v1",
    ),
    ...
    "openai-api": JanusOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": JanusOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": JanusOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="JANUS_QWEN_BASE_URL",
    ),
    "google-gemini-cli": JanusOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": JanusOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": JanusOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": JanusOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": JanusOverlay(
    ...
    "kimi-for-coding": JanusOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": JanusOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": JanusOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": JanusOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": JanusOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": JanusOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": JanusOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": JanusOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "opencode": JanusOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": JanusOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "azure-foundry": JanusOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": JanusOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "janus", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Janus overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Janus overlay
    ...
    overlay = JANUS_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + janus extra
    ...
    if overlay is not None:
        # Janus-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in JANUS_OVERLAYS
    if provider == "bedrock":
