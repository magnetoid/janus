...
Profile-aware location: ``$JANUS_HOME/logs/dashboard-auth.log``.
...
from ``janus_constants`` or other janus_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$JANUS_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``janus_constants.get_janus_home`` semantics: env var wins,
    else ``~/.janus``. A local copy avoids an import cycle with the
    middleware which lives below ``janus_cli``.
    """
    home = os.environ.get("JANUS_HOME") or str(Path.home() / ".janus")
