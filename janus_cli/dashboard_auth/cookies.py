...
Three cookies in play:
  - janus_session_at:   the OAuth access token
                         (HttpOnly, lifetime = token TTL, ~15 min)
  - janus_session_rt:   the OAuth refresh token
                         (HttpOnly, lifetime = 24h, ROTATING + reuse-detected)
                         Cloud Industry Portal issues a rotating refresh token for the
                         dashboard auth-code grant (Portal NAS #293 / janus
  ...
  - janus_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/janus) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/janus`` covers
...
setter wrote ``__Secure-janus_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "janus_session_at"
SESSION_RT_COOKIE = "janus_session_rt"
PKCE_COOKIE = "janus_session_pkce"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /janus`` we want ``Path=/janus`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    ``refresh_token`` is written as the RT cookie when non-empty. Cloud Industry Portal
    issues a 24h rotating refresh token (janus #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/janus``)
