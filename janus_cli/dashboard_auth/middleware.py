...
from janus_cli.dashboard_auth.base import ProviderError, RefreshExpiredError
from janus_cli.dashboard_auth.cookies import read_session_cookies
from janus_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /janus``, the
    ``login_url`` is prefixed (``/janus/login?next=...``) so the
    ...
    """
    from janus_cli.dashboard_auth.prefix import prefix_from_request
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["janus.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # sends only ``janus_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if at:
        ...
        # tries to reach Cloud Industry Portal's JWKS; if that's unreachable it raises,
    ...
    if session is None:
        ...
        from janus_cli.dashboard_auth.cookies import clear_session_cookies
        from janus_cli.dashboard_auth.prefix import prefix_from_request
