...
- ``janus_cli.providers``        -- canonical provider identity + overlays
- ``janus_cli.model_normalize``  -- per-provider name formatting
...
def is_nous_janus_non_agentic(model_name: str) -> bool:
    ...
    if not model_name:
        return False
    return bool(_NOUS_JANUS_NON_AGENTIC_RE.search(model_name))


def _check_janus_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Hermes 3/4 chat model."""
    if is_nous_janus_non_agentic(model_name):
        return _JANUS_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``janus config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from janus_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'janus model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'janus doctor' — config issues detected:"
        ...
        from janus_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from janus_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from janus_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in janus_cli.runtime_provider.resolve_runtime_provider;
    ...
    janus_warn = _check_janus_model_warning(new_model)
    if janus_warn:
        warnings.append(janus_warn)
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from janus_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from janus_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from janus_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by janus provider ID
    ...
    # --- 1. Check Janus-mapped providers ---
    from janus_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from janus_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for janus_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(janus_id)
        if (
            _alias_target
            and _alias_target != janus_id
        ...
        pconfig = PROVIDER_REGISTRY.get(janus_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (JANUS_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from janus_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(janus_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `janus model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(janus_id)
        if not model_ids:
            model_ids = curated.get(janus_id, [])
            if janus_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(janus_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Janus-only providers (nous, openai-codex, copilot, opencode-go) ---
    from janus_cli.providers import JANUS_OVERLAYS
    from janus_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Janus provider ID.
    # JANUS_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Janus IDs ("copilot").
    _mdev_to_janus = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in JANUS_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Janus slug — e.g. "github-copilot" → "copilot"
        janus_slug = _mdev_to_janus.get(pid, pid)
        if janus_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(janus_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, janus_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or janus_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(janus_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", janus_slug, exc)
        ...
        # configured.
        if not has_creds and janus_slug == "anthropic":
            try:
                ...
                if (janus_creds and janus_creds.get("accessToken")) or \
        ...
        if janus_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(janus_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(janus_slug)
                model_ids = _ids if _ids else (curated.get(janus_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(janus_slug, []) or curated.get(pid, [])
        elif janus_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `janus model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from janus_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(janus_slug, []) or curated.get(pid, [])
                if janus_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(janus_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": janus_slug,
            "name": get_label(janus_slug),
            "is_current": janus_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "janus",
        ...
        seen_slugs.add(janus_slug.lower())
        _record_builtin_endpoint(janus_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or JANUS_OVERLAYS (keeps /model in sync
    # with `janus model`).
    try:
        from janus_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Janus's canonical write key (matches
            ...
            # Janus writes ``models:`` as a dict keyed by model id
            # (see janus_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Janus's own writer uses; a hyphen variant is accepted
            ...
            # active model. Janus's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, janus_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`janus_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from janus_cli.models import fetch_openrouter_models
