"""
Dump command for janus CLI.

Outputs a compact, plain-text summary of the user's Janus setup
...
from janus_cli.config import get_janus_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.janus_build_sha`` by the Dockerfile's
    ``JANUS_GIT_SHA`` build-arg (see ``janus_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``janus dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from janus_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(janus_home: Path) -> int:
...
def _cron_summary(janus_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = janus_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"janus_home:      {display_janus_home()}")
    ...
    toolsets = config.get("toolsets", ["janus-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(janus_home)}")
    lines.append(f"  skills:             {_count_skills(janus_home)}")
