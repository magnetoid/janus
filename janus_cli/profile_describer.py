...
Used by ``janus profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``janus_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Janus Agent kanban board.
...
Rules:
  ...
  - Never write "Janus Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_janus_home())
        else:
