"""
Janus Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``janus desktop`` / ``janus gui``)
     Built inside the agent checkout under ``$JANUS_HOME/janus-agent/``:
       ...
       - ``$JANUS_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Janus.app`` or ``~/Applications/Janus.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Janus`` (NSIS per-user)
...
the app name ("Janus"), separate from ``$JANUS_HOME``:
  - macOS:   ``~/Library/Application Support/Janus``
  - Windows: ``%APPDATA%\\Janus``
  - Linux:   ``$XDG_CONFIG_HOME/Janus`` (default ``~/.config/Janus``)
...
``janus uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(janus_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return janus_home / "janus-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Janus"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Janus"
...
def source_built_gui_artifacts(janus_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``janus desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``janus-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(janus_home)
    ...
    return [
        ...
        janus_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Janus" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Janus.app"),
            home / "Applications" / "Janus.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Janus).
            local_base / "Programs" / "Janus",
            # Older / alternate layout some builds used.
            local_base / "janus-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Janus")
    else:
        ...
        paths += [
            data_base / "applications" / "janus.desktop",
            data_base / "applications" / "Janus.desktop",
        ]
    return paths


def agent_is_installed(janus_home: Path) -> bool:
    """Return True when a usable Python agent install exists under JANUS_HOME.
    ...
    if (agent_root / "janus_cli").is_dir():
        return True
...
def gui_is_installed(janus_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(janus_home):
        if p.exists():
            return True
...
def gui_install_summary(janus_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = janus_home if janus_home is not None else get_janus_home()
...
def uninstall_gui(janus_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``janus-agent/janus_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$JANUS_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove janus' or "
            "'sudo dnf remove janus'). AppImage builds are a single file you "
