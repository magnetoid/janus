"""``janus logs`` — view and filter Janus log files.
...
Usage examples::
    ...
    janus logs                    # last 50 lines of agent.log
    janus logs -f                 # follow agent.log in real time
    janus logs errors             # last 50 lines of errors.log
    janus logs gateway -n 100    # last 100 lines of gateway.log
    janus logs gui -f            # follow gui.log (dashboard/pty/ws)
    janus logs desktop -f        # follow desktop.log (Electron app boot/backend)
    janus logs --level WARNING    # only WARNING+ lines
    janus logs --session abc123   # filter by session ID substring
    janus logs --component tools  # only tool-related lines
...
def tail_log(
    ...
    *,
    ...
    log_path = get_janus_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Janus runs — try 'janus chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_janus_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_janus_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_janus_home()}/logs/")
        return

    print(f"Log files in {display_janus_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'janus chat' to generate logs)")
