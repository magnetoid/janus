...
Pure display functions with no JanusCLI state dependency.
"""
...
from janus_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE

JANUS_LOGO = """[bold #FFD700]     ██╗ █████╗ ███╗   ██╗██╗   ██╗███████╗[/]
[bold #FFD700]     ██║██╔══██╗████╗  ██║██║   ██║██╔════╝[/]
[#FFBF00]     ██║███████║██╔██╗ ██║██║   ██║███████╗[/]
[#FFBF00]██   ██║██╔══██║██║╚██╗██║██║   ██║╚════██║[/]
[#CD7F32]╚█████╔╝██║  ██║██║ ╚████║╚██████╔╝███████║[/]
...
# Janus emblem — a gateway with an hourglass: the god of doorways &
# thresholds, two faces to past and future. Block art so it always aligns.
JANUS_EMBLEM = """[#CD7F32]   ▗▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▖   [/]
[#CD7F32]   ▐███████████████▌   [/]
[#FFBF00]   ▐███▘       ▝███▌   [/]
[#FFBF00]   ▐██   ╲▄▄▄╱   ██▌   [/]
...
# (e.g. nix-built janus — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/magnetoid/janus.git"
...
def _fetch_pypi_latest(package: str = "janus-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Janus update is available.

    Two paths: if ``JANUS_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = janus_home / ".update_check"
    embedded_rev = os.environ.get("JANUS_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # JANUS_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `janus update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/janus/update/check`
    ...
    if embedded_rev:
        ...
        # $JANUS_HOME/janus-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = janus_home / "janus-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Janus git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$JANUS_HOME/janus-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``janus_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from janus_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/magnetoid/janus/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Janus checkout. Cached per-process. Release URL always points at the
    canonical magnetoid/janus repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Janus Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else JANUS_EMBLEM
    except Exception:
        _bskin = None
        _hero = JANUS_EMBLEM
    left_lines = ["", _hero, ""]
    ...
    left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Cloud Industry[/]")
    ...
    try:
        from janus_cli.codex_runtime_switch import get_current_runtime
        from janus_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from janus_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built janus; we know an update
    ...
    # Pip-install warning — `pip install janus-agent` is not the supported
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else JANUS_LOGO
        console.print(_logo)
        console.print()
