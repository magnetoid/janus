...
# xAI's public API is OpenAI-compatible for the endpoints Janus commonly
# uses. The Responses endpoint is included because Janus' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Janus-managed OAuth credentials."""

    auth_hint = "janus auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`janus auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`janus auth reset xai-oauth` or re-authenticate with "
                    "`janus auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `janus auth add xai-oauth --type oauth`."
            )
