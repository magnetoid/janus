"""janus webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.janus/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _janus_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: janus gateway run
"""
...
def webhook_command(args):
    """Entry point for 'janus webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: janus webhook {subscribe|list|remove|test}")
        print("Run 'janus webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (janus gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: janus webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from janus webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (janus gateway run)")
