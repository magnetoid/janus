"""`janus checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.janus/checkpoints/``.  Actions:

    janus checkpoints               # same as `status`
    janus checkpoints status        # total size, project count, breakdown
    janus checkpoints list          # per-project checkpoint counts + workdir
    janus checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    janus checkpoints clear [-f]    # nuke the entire base (asks first)
    janus checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    janus checkpoints
    janus checkpoints prune --retention-days 3 --max-size-mb 200
    janus checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: janus checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``janus checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `janus checkpoints` → status
