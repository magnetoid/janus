"""
Janus Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.janus/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Janus PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing janus-agent or janus PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Janus Agent" comment and following line
                if '# Janus Agent' in line or '# janus-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('janus' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing janus
                if 'janus' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the janus wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "janus",
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains janus_cli reference)
                content = wrapper.read_text()
                if 'janus_cli' in content or 'janus-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(janus_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``janus`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Janus home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (janus_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``janus gateway run`` processes
    ...
    try:
        from janus_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from janus_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``JANUS_HOME`` and ``JANUS_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\janus\git\cmd``, ``%LOCALAPPDATA%\janus\git\bin``,
#      ``%LOCALAPPDATA%\janus\git\usr\bin``, ``%LOCALAPPDATA%\janus\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\janus\git\`` and Node to
#      ``%LOCALAPPDATA%\janus\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``janus dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\janus\gateway-service\`` and sometimes
...
def _janus_path_markers(janus_home: Path) -> list[str]:
    """Path-entry substrings that identify Janus-owned User-PATH entries."""
    root = str(janus_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare janus-agent install dir.
    markers = [root + "\\janus-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if JANUS_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "janus".  We don't want to catch
    # unrelated entries like "cjanus-foo" or "ephermeral", so we look for
    # backslash-janus as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(janus_home: Path) -> list[str]:
    """Strip Janus-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _janus_path_markers(janus_home)
            kept: list[str] = []
...
def remove_janus_env_vars_windows() -> list[str]:
    """Delete JANUS_HOME and JANUS_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("JANUS_HOME", "JANUS_GIT_BASH_PATH"):
                try:
...
def _is_default_janus_home(janus_home: Path) -> bool:
    """Return True when ``janus_home`` points at the default (non-profile) root."""
    try:
        from janus_constants import get_default_janus_root
        return janus_home.resolve() == get_default_janus_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its JANUS_HOME directory.

    We shell out to ``janus -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current JANUS_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m janus_cli.main` so we don't depend on a `janus`
    #    wrapper that may be half-removed mid-uninstall.
    janus_invocation = [_sys.executable, "-m", "janus_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                janus_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's JANUS_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``janus uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$JANUS_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from janus_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(janus_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Janus Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Janus Chat GUI installation was found.")
        print(f"  Checked: {janus_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Janus agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(janus_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Janus agent at {janus_home / 'janus-agent'}")
        print(f"  • Your config, sessions, and secrets under {janus_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Janus agent is still installed. Run 'janus' to use the CLI,")
    print("or 'janus uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.janus/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.janus/ for future reinstall
    """
    project_root = get_project_root()
    janus_home = get_janus_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie JANUS_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_janus_home(janus_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.janus data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Janus Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {janus_home / 'config.yaml'}")
    print(f"  Secrets: {janus_home / '.env'}")
    print(f"  Data:    {janus_home / 'cron/'}, {janus_home / 'sessions/'}, {janus_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their JANUS_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Janus data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Janus code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``janus`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$JANUS_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in janus_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\janus\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(janus_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Janus-owned PATH entries in User environment")

        log_info("Removing JANUS_HOME / JANUS_GIT_BASH_PATH User env vars...")
        removed_env = remove_janus_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Janus-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing janus command...")
    ...
    #     (only when they still point into this Janus home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Janus-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(janus_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Janus-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(janus_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE JANUS_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from janus_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(janus_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.janus/, just remove the janus-agent subdir
            if janus_home in project_root.parents or project_root.parent == janus_home:
    ...
    #     under JANUS_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(janus_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(janus_home)
    ...
    # 5. Optionally remove ~/.janus/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile JANUS_HOME dirs live under
        ...
        try:
            if janus_home.exists():
                shutil.rmtree(janus_home)
                log_success(f"Removed {janus_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {janus_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {janus_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://github.com/magnetoid/janus)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://github.com/magnetoid/janus | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Janus Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m janus_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import janus_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``janus_constants`` + ``janus_cli.colors``
    (and lazily ``janus_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m janus_cli.uninstall")
    parser.add_argument(
        "--mode",
