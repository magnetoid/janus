...
def format_nous_portal_entitlement_message(
    ...
    *,
    ...
    if account_info is None:
        return (
            f"Janus could not verify your Nous Portal entitlement, so {capability} "
    ...
    if not account_info.logged_in:
        ...
        return (
            f"Log in to Nous Portal to use {capability}: run `janus model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"Janus could not verify your Nous Portal paid access, so {capability} "
            f"is unavailable."
        )
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `janus model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"Janus could not find a Nous Portal account or organisation for this "
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `janus model` to refresh Janus."
        return message
...
def _fresh_account_info(
    *,
    ...
    try:
        from janus_cli.auth import get_provider_auth_state, resolve_nous_access_token
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from janus_cli.auth import _decode_jwt_claims
    except Exception:
        return None
