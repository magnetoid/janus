...
on Japanese locales, etc.).  Janus's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``janus_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Janus is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``JANUS_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("JANUS_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Janus-managed Git install directories so
    ...
    # already-running shells don't see the change — which means janus
    ...
    # install without Janus-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Janus's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Janus's
    ``janus config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Janus (User env var in Windows Settings, or
    export in a PowerShell profile) and Janus picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Janus-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\janus\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``janus`` from right after install) retain their old PATH.

    Any subprocess Janus spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Janus-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "janus", "git", "cmd"),
        os.path.join(local_appdata, "janus", "git", "bin"),
        os.path.join(local_appdata, "janus", "git", "usr", "bin"),
        # Janus venv Scripts directory — host of the janus.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes janus, but harmless to include.
        os.path.join(local_appdata, "janus", "janus-agent", "venv", "Scripts"),
