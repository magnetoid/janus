...
Add, remove, or reorder entries here — both `janus setup` and
`janus` provider-selection will pick up the change automatically.
"""
...
from janus_cli import __version__ as _JANUS_VERSION

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_JANUS_USER_AGENT = f"janus-cli/{_JANUS_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `janus model`
    flow without maintaining a separate static list.
    """
    from janus_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $JANUS_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $JANUS_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Janus refreshes the cache from
...
# ---------------------------------------------------------------------------
# Cloud Industry Portal free-model helper
# ---------------------------------------------------------------------------
# The Cloud Industry Portal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# Cloud Industry Portal account tier detection
# ---------------------------------------------------------------------------
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Janus that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Janus that doesn't ship
...
def check_nous_free_tier(*, force_fresh: bool = False) -> bool:
    """Check if the current Cloud Industry Portal user is on a free (unpaid) tier.
    ...
    try:
        from janus_cli.nous_account import get_nous_portal_account_info
...
# ---------------------------------------------------------------------------
# Cloud Industry Portal recommended models
#
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    """Fetch the Cloud Industry Portal's curated recommended-models payload.
...
def _resolve_nous_portal_url() -> str:
    ...
    try:
        ...
        return "https://portal.cloud-industry.com"
...
# this list:  janus model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `janus model` interactive picker
# ---------------------------------------------------------------------------
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("nous",           "Cloud Industry Portal",              "Cloud Industry Portal (Everything your agent needs, 300+ models with bundled tool use)"),
...
#
# Some vendors expose several Janus provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `janus model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``janus model``, the
...
# sensible default, but Cloud Industry Portal is a per-token *metered aggregator* whose
...
# path. The *interactive* default (GUI onboarding / ``janus model``) uses the
# richer free/paid-tier-aware resolver — see ``get_recommended_default_model``
# in janus_cli/web_server.py and ``partition_nous_models_by_tier`` — which can
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``janus auth add openai-codex`` without
    ``janus model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``janus model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    janus-agent is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (Cloud Industry Portal, private mirrors, older catalog snapshots) don't populate
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from janus_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — janus-agent
...
def get_curated_nous_model_ids() -> list[str]:
    """Return the curated Cloud Industry Portal model-id list.
    ...
    """
    try:
        from janus_cli.model_catalog import get_curated_nous_models
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, Cloud Industry Portal).
    """
...
_DEFAULT_NOUS_INFERENCE_BASE = "https://inference-api.cloud-industry.com"


def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for Cloud Industry Portal pricing.
    ...
    """
    try:
        from janus_cli.auth import resolve_nous_runtime_credentials
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    if normalized == "nous":
        ...
        if base_url:
            # Nous base_url typically looks like https://inference-api.cloud-industry.com/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``janus model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from janus_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from janus_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Janus' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``janus_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Janus should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``janus auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.janus/.env``.
    ...
    try:
        ...
        from janus_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Janus release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Janus release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        # Try live Cloud Industry Portal /models endpoint
        try:
        ...
        # added Portal models still surface without a Janus release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``janus model`` shows.
...
#
# Cache strategy:
#   - One JSON file at $JANUS_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    ...
    return get_janus_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$JANUS_HOME/auth.json`` and external
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from janus_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=True)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "JanusAgent/1.0",
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _JANUS_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Janus' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _JANUS_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from janus_constants import get_janus_home
    return get_janus_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Janus will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Janus cannot verify the model name."
