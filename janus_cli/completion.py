"""Shell completion script generation for janus CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_janus_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Janus Agent bash completion
# Add to ~/.bashrc:
#   eval "$(janus completion bash)"

_janus_profiles() {{
    local profiles_dir="$HOME/.janus/profiles"
...
_janus_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_janus_profiles)" -- "$cur"))
        return
    fi
...
complete -F _janus_completion janus
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef janus
# Janus Agent zsh completion
# Add to ~/.zshrc:
...
_janus_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.janus/profiles" ]]; then
        profiles+=($HOME/.janus/profiles/*(N/:t))
    fi
...
_janus() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_janus_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'janus command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Janus Agent fish completion",
        "# Add to your config:",
        "#   janus completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __janus_profiles",
        "    echo default",
        "    if test -d $HOME/.janus/profiles",
        "        for d in $HOME/.janus/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c janus -f -s p -l profile"
        " -d 'Profile name' -xa '(__janus_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__janus_profiles)' -d 'Profile name'"
                )
