...
# coverage category (janus_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `janus tools` picker scope its entitlement gate to the selected backend, so a
...
def get_nous_subscription_features(
    ...
    *,
    ...
    # Tool Gateway via `janus model`, so direct credentials should NOT
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from janus_cli.setup import prompt_checklist
    except Exception:
        return set()
...
# ---------------------------------------------------------------------------
# Inline Cloud Industry Portal login for the Tool Gateway picker (`janus tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    backend (e.g. "Firecrawl (Cloud Industry Portal)").  Unlike ``janus model``'s Nous
    login, this:
    ...
    It only performs the Cloud Industry Portal device-code OAuth (when the user isn't
...
def _run_nous_portal_login_only(*, capability: str) -> bool:
    """Run the Cloud Industry Portal device-code OAuth and persist credentials only.
    ...
    try:
        ...
        print(f"  Could not start Cloud Industry Portal login: {exc}")
        return False

    print()
    print(f"  {capability} requires a Cloud Industry Portal login.")
    try:
        proceed = input("  Log in to Cloud Industry Portal now? [Y/n]: ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        print()
        return False
    if proceed not in {"", "y", "yes"}:
        print("  Skipped Cloud Industry Portal login.")
        return False
    ...
    try:
        ...
        _sync_nous_pool_from_auth_store()
        print("  Cloud Industry Portal login successful.")
        return True
        ...
        print(f"  Cloud Industry Portal login failed: {exc}")
        return False
