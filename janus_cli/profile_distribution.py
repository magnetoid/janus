"""Profile distributions — shareable, packaged Janus profiles via git.

A distribution is a Janus profile published as a git repository (or
...
* ``janus profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``janus skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``janus profile``, not a parallel tree):

    janus profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "janus-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            janus_requires=str(data.get("janus_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.janus_requires:
            out["janus_requires"] = self.janus_requires
        if self.author:
...
def check_janus_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Janus {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``janus profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Janus profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Janus distribution."
        )

    # Version check up-front so we fail fast
    check_janus_requires(manifest.janus_requires, janus_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.janus).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="janus_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `janus profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`janus profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="janus_dist_update_") as tmp:
        plan = plan_install(
