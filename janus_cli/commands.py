"""Slash command definitions and autocomplete for the Janus CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Janus config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("plan", "Plan the next task before executing (propose a plan + wait for approval)",
               "Session", args_hint="[task]"),
    CommandDef("goal", "Set a standing goal Janus works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Janus is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.janus/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Janus Agent to the latest version", "Info"),
    CommandDef("version", "Show Janus Agent version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`janus_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/janus`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full Janus
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``janus skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``janus skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``janus skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/janus <verb>`` subcommand.
    ...
    can still reach them via ``/janus <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/janus`` is always reserved as the first entry so the
    legacy ``/janus <subcommand>`` form keeps working for anything that
    ...
    # Reserve /janus as the catch-all top-level command.
    entries.append(("janus", "Talk to Janus or run a subcommand", "[subcommand] [args]"))
    seen.add("janus")
...
def slack_app_manifest(request_url: str = "https://janus-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /janus handler.

    Maps both canonical names and aliases so /janus bg do stuff works
    the same as /janus background do stuff.

    Plugin-registered slash commands are included so ``/janus <plugin-cmd>``
