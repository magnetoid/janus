"""Janus CLI skin/theme engine.

A data-driven skin system that lets users customize the CLI's visual appearance.
Skins are defined as YAML files in ~/.janus/skins/ or as built-in presets.
...
.. code-block:: yaml
    ...
    branding:
      agent_name: "Janus Agent"          # Banner title, status display
      welcome: "Welcome message"          # Shown at CLI startup
      goodbye: "Goodbye! ⚕"              # Shown on exit
      response_label: " ⚕ Janus "       # Response box header label
...
.. code-block:: python
    ...
    from janus_cli.skin_engine import get_active_skin, list_skins, set_active_skin

    skin = get_active_skin()
    print(skin.colors["banner_title"])    # "#FFD700"
    print(skin.get_branding("agent_name"))  # "Janus Agent"

    set_active_skin("ares")               # Switch to built-in ares skin
    set_active_skin("mytheme")            # Switch to user skin from ~/.janus/skins/
...
Drop a YAML file in ``~/.janus/skins/<name>.yaml`` following the schema above.
...
_BUILTIN_SKINS: Dict[str, Dict[str, Any]] = {
    "default": {
        "name": "default",
        "description": "Classic Janus — gold and kawaii",
        "colors": {
...
def get_active_goodbye(fallback: str = "Until next time. ⟡") -> str:
