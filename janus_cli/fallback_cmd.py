"""
janus fallback — manage the fallback provider chain.
...
Subcommands:
  janus fallback [list]   Show the current fallback chain (default when no subcommand)
  janus fallback add      Pick provider + model via the same picker as `janus model`,
                           then append the selection to the chain
  janus fallback remove   Pick an entry to delete from the chain
  janus fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.janus/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from janus_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `janus model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  janus fallback add")
        print()
        return
    ...
    print("  Docs: https://github.com/magnetoid/janus")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `janus model`, then append the selection to the chain."""
    from janus_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `janus model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `janus fallback list` to view, or `janus fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``janus fallback [subcommand]``."""
