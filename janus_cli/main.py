#!/usr/bin/env python3
"""
Janus CLI - Main entry point.

Usage:
    janus                     # Interactive chat (default)
    janus chat                # Interactive chat
    janus gateway             # Run gateway in foreground
    janus gateway start       # Start gateway as service
    janus gateway stop        # Stop gateway service
    janus gateway status      # Show gateway status
    janus gateway install     # Install gateway service
    janus gateway uninstall   # Uninstall gateway service
    janus setup               # Interactive setup wizard
    janus logout              # Clear stored authentication
    janus status              # Show status of all components
    janus cron                # Manage cron jobs
    janus cron list           # List cron jobs
    janus cron status         # Check if cron scheduler is running
    janus doctor              # Check configuration and dependencies
    janus honcho setup                    # Configure Honcho AI memory integration
    janus honcho status                   # Show Honcho config and connection status
    janus honcho sessions                 # List directory → session name mappings
    janus honcho map <name>               # Map current directory to a session name
    janus honcho peer                     # Show peer names and dialectic settings
    janus honcho peer --user NAME         # Set user peer name
    janus honcho peer --ai NAME           # Set AI peer name
    janus honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    janus honcho mode                     # Show current memory mode
    janus honcho mode [hybrid|honcho|local]  # Set memory mode
    janus honcho tokens                   # Show token budget settings
    janus honcho tokens --context N       # Set session.context() token cap
    janus honcho tokens --dialectic N     # Set dialectic result char cap
    janus honcho identity                 # Show AI peer identity representation
    janus honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    janus honcho migrate                  # Step-by-step migration guide: OpenClaw native → Janus + Honcho
    janus version             Show version
    janus update              Update to latest version
    janus uninstall           Uninstall Janus Agent
    janus acp                 Run as an ACP server for editor integration
    janus sessions browse     Interactive session picker with search

    janus claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: janus_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``janus_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``janus update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``janus_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, janus crashes on import and the user can't run
# ``janus update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
def _set_process_title() -> None:
    """Set the process title to 'janus' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``janus tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``janus.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("janus")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"janus", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"janus")
        # Windows: the .exe name is already ``janus.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* janus_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("JANUS_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".janus", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``JANUS_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("JANUS_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``JANUS_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("JANUS_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`janus --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from janus_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Janus Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``janus --version`` before config/logging imports on Termux."""
    if os.environ.get("JANUS_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to JANUS_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (janus tools, janus setup, janus model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'janus {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any janus module import.
#
# Many modules cache JANUS_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("JANUS_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.janus/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set JANUS_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors janus_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If JANUS_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.janus/profiles/coder or
    # /opt/data/profiles/coder).  If JANUS_HOME points to the janus root
    # instead (e.g. systemd hardcodes JANUS_HOME=/root/.janus), we must
    # still read active_profile — the user may have switched profiles via
    # `janus profile use` and the gateway should honour that choice.
    # See issue #22502.
    janus_home_env = os.environ.get("JANUS_HOME", "")
    if profile_name is None and janus_home_env:
    ...
    if profile_name is None:
        try:
            ...
            active_path = get_default_janus_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set JANUS_HOME
    if profile_name is not None:
        try:
            from janus_cli.profiles import resolve_profile_env

            janus_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent janus from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["JANUS_HOME"] = janus_home
...
load_janus_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → JANUS_REDACT_SECRETS env
# var BEFORE janus_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_janus_home() / "config.yaml"
    if _cfg_path.exists():
        with open(_cfg_path, encoding="utf-8") as _f:
            _early_cfg_raw = _yaml_early.safe_load(_f) or {}
        if "JANUS_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["JANUS_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `janus` subcommands
...
try:
    from janus_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from janus_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if janus_constants not importable yet
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_janus_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("JANUS_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("JANUS_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from janus_cli.config import get_env_path, get_janus_home, load_config
    from janus_cli.auth import get_auth_status

    # Determine whether Janus itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_janus_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    # Only count these if Janus has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Janus to use their tokens.
    if _has_janus_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, janus_bin
        cli_args: the original CLI arguments (everything after 'janus')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    janus_bin = container_info["janus_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo janus {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo janus {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  janus --tui --resume {target}")
    if title:
        print(f'  janus --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @janus/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@janus" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/janus-ink/package.json",
    "packages/janus-ink/index.js",
    "packages/janus-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``JANUS_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("JANUS_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``JANUS_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("JANUS_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    janus_home = os.environ.get("JANUS_HOME") or str(Path.home() / ".janus")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "JANUS_HOME": janus_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(janus_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(janus_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if janus_cli_dir is None:
        janus_cli_dir = Path(__file__).parent
    bundled = janus_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (JANUS_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("JANUS_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from janus_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("JANUS_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with JANUS_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset JANUS_TUI_DIR (e.g. `unset JANUS_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if (
        ...
        npm = _node_bin("npm")
        if not os.environ.get("JANUS_QUIET"):
    ...
    if tui_dev:
        # Keep the local @janus/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @janus/ink resolves through
        # packages/janus-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "janus-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from janus_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="janus-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["JANUS_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["JANUS_PYTHON_SRC_ROOT"] = os.environ.get(
        "JANUS_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("JANUS_PYTHON", sys.executable)
    env.setdefault("JANUS_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["JANUS_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["JANUS_MODEL"] = model
        env["JANUS_INFERENCE_MODEL"] = model
    if provider:
        env["JANUS_TUI_PROVIDER"] = provider
        env["JANUS_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["JANUS_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["JANUS_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["JANUS_TUI_SKILLS"] = value
    if query:
        env["JANUS_TUI_QUERY"] = query
    if image:
        env["JANUS_TUI_IMAGE"] = image
    if checkpoints:
        env["JANUS_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["JANUS_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["JANUS_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["JANUS_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["JANUS_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["JANUS_ACCEPT_HOOKS"] = "1"
    ...
    # JANUS_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `janus --tui` try to
    ...
    # still set JANUS_TUI_RESUME themselves.
    env.pop("JANUS_TUI_RESUME", None)
    if resume_session_id:
        env["JANUS_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `janus update` so
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``JANUS_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``janus kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``janus kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("JANUS_KANBAN_BOARD"):
        return
    try:
        from janus_cli.kanban_db import get_current_board

        os.environ["JANUS_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.janus/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Janus — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Janus still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``JANUS_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("JANUS_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'janus sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from janus_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from janus_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'janus doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Janus isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  janus setup")
        print()
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'janus setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from janus_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["JANUS_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.janus/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["JANUS_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["JANUS_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["JANUS_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from janus_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from janus_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Janus?")
        print()
    ...
    # has no creds.json.  Every subsequent `janus gateway` then paid a 30s
    ...
    session_dir = get_janus_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: janus gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `janus gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  janus gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Janus Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  janus gateway")
        ...
        print()
        print("  Or install as a service: janus gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'janus whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from janus_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from janus_cli.config import stamp_install_method
    from janus_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Janus post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``janus model``) and the setup wizard
    ...
    from janus_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("JANUS_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'janus model' for "
                "available providers, or run 'janus doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in janus_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.janus/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.janus/.env if the active provider is not 'custom'.
    ...
    """
    from janus_cli.config import get_env_value, save_env_value, load_config
...
#
# Janus uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `janus model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `janus model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`janus_cli.plugins.PluginContext.register_auxiliary_task`) appear
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``janus model`` flow, then route aux tasks to them here.
    """
    from janus_cli.config import load_config
    from janus_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from janus_cli.auth import _prompt_model_selection
    from janus_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from janus_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from janus_constants import OPENROUTER_BASE_URL
    from janus_cli.auth import (
        ProviderConfig,
    ...
    # in-flow (K/R/C) instead of having to edit ~/.janus/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `janus setup` from scratch. OpenRouter
    ...
    from janus_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from janus_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier:
        try:
            from janus_cli.nous_account import (
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `janus auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from janus_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from janus_cli.models import _PROVIDER_MODELS
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            print()
            try:
                # Forward CLI flags from ``janus model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``janus model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``janus auth add xai-oauth``).  Fall
...
def _model_flow_custom(config):
    ...
    from janus_cli.config import get_env_value, load_config, save_config
    from janus_cli.secret_prompt import masked_secret_prompt
    ...
    from janus_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `janus model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Janus URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from janus_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from janus_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from janus_cli import azure_detect
    ...
    print("Anthropic-style API endpoints.  Janus will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Janus will install it now (the preflight below "
...
def _remove_custom_provider(config):
    ...
    try:
        from janus_cli.curses_ui import curses_radiolist
...
# downstream call sites read `janus_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `janus` invocation — including
# fast paths like `janus --version` and slash-command dispatch that never
...
def _model_flow_copilot(config, current_model=""):
    ...
    from janus_cli.config import save_env_value, load_config, save_config
    from janus_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from janus_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from janus_cli.copilot_auth import validate_copilot_token
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from janus_cli.env_loader import format_secret_source_suffix
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Janus turns to `copilot --acp`.")
    print("  Janus currently starts its own ACP subprocess for each request.")
    print("  Janus uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set JANUS_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``janus setup`` / ``janus model``.
    ...
    recover from a malformed paste without editing ``~/.janus/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from janus_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `janus setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from janus_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so janus knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Janus typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Janus, enable billing on your "
    ...
    if provider_id == "lmstudio":
        ...
        from janus_cli.models import fetch_lmstudio_models
        ...
        from janus_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from janus_constants import display_janus_home as _dhh_fn

            print(
                f"    Janus will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'janus model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Janus CLI with a provider."""
    from janus_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from janus_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from janus_cli.auth_commands import auth_command
...
def cmd_cron(args):
    """Cron job management."""
    from janus_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from janus_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``janus slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: janus slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `janus slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from janus_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from janus_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from janus_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from janus_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `janus security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from janus_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from janus_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from janus_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from janus_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Janus home directory to a zip file."""
    if getattr(args, "quick", False):
        from janus_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from janus_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Janus backup from a zip file."""
    from janus_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        ...
        from janus_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Janus Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from janus_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from janus_cli.uninstall import run_gui_uninstall
    ...
    from janus_cli.uninstall import run_uninstall
...
# Critical files that every ``janus`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``janus update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "janus_cli/main.py",
    "janus_cli/config.py",
    "janus_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``janus`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="janus-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``janus update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``janus_cli/web_dist/`` (per vite.config.ts
    outDir: "../janus_cli/web_dist"), NOT to ``web/dist/``, so Python
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``janus`` launcher present but ``janus_cli`` not importable.
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``janus update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``janus web``).
    ...
    # build path usable on a stock `py -m janus_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (janus web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``janus update`` can unconditionally call ``janus desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``janus desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $JANUS_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $JANUS_HOME."""
    from janus_constants import get_janus_home
    return get_janus_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``janus update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Janus.app/Contents/MacOS/Janus"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Janus.exe",
            release_dir / "win-ia32-unpacked" / "Janus.exe",
            release_dir / "win-arm64-unpacked" / "Janus.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "janus",
            release_dir / "linux-unpacked" / "Janus",
            release_dir / "linux-arm64-unpacked" / "janus",
            release_dir / "linux-arm64-unpacked" / "Janus",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Janus'`` desktop build failure: a corrupt zip in the
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Janus is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Janus.app/Contents/MacOS/Janus  ->  app bundle = .../Janus.app
    app = exe.parents[2]
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Janus Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Janus Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from janus_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "fake_boot", False):
        env["JANUS_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["JANUS_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "janus_root", None):
        env["JANUS_DESKTOP_JANUS_ROOT"] = str(Path(args.janus_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["JANUS_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  janus gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if build_result.returncode != 0 and not source_mode:
                ...
                # on the final `electron` -> `Janus` rename: unpack-electron
            ...
            if not source_mode:
                ...
                # an in-place self-update (otherwise macOS reports "Janus is
    ...
    if source_mode:
        print("→ Launching Janus Desktop from source build...")
    ...
    print(f"→ Launching packaged Janus Desktop: {packaged_executable}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``janus dashboard`` processes other than ourselves.

    ``janus dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``janus update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Janus Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``janus dashboard`` as
    ...
    environment variable ``JANUS_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "janus dashboard",
        "janus_cli.main dashboard",
        "janus_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "janus.*dashboard"`) keeps us consistent
            # with `janus_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `janus update`.
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``janus update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``janus update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: janus curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``janus dashboard`` processes.

    Called at the end of ``janus update`` (default ``reason``) and also
    from ``janus dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Janus Desktop Electron app spawns this dashboard as a
    # backend child, it sets JANUS_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("JANUS_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    if killed:
        ...
        print("    janus dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Janus Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `janus update "
            f"--branch {branch}`, or update against main with `janus update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/magnetoid/janus/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="janus-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"janus-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for janus-agent itself
        ...
        # GitHub ZIPs extract to janus-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"janus-agent-{branch}")
    ...
    from janus_cli.managed_uv import ensure_uv, update_managed_uv
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "janus-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Janus behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes janus completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Janus couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Janus couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Janus behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Janus couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Janus couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `janus update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/magnetoid/janus.git",
    "git@github.com:magnetoid/janus.git",
    "https://github.com/magnetoid/janus",
    "git@github.com:magnetoid/janus",
}
OFFICIAL_REPO_URL = "https://github.com/magnetoid/janus.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_janus_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_janus_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Janus repository.")
        print("  This means you may miss updates from magnetoid/janus.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/magnetoid/janus.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/magnetoid/janus.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``janus update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from janus_constants import get_default_janus_root

    default_home = get_default_janus_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``janus update`` is still progressing even if pip/uv itself is silent.
    """
...
def _janus_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "janus.exe",
        scripts_dir / "janus-gateway.exe",
    ]


def _detect_concurrent_janus_instances(
    ...
    The Janus Desktop Electron app spawns ``janus.EXE`` as a backend child,
    so during ``janus update`` the user-invoked process and the desktop's
    ...
    shims (``janus.exe`` / ``janus-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``janus update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``janus.exe``) is a separate process from the Python
    ...
    for shim in _janus_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated janus.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``janus update``
    ...
    #      janus.exe sitting *under* a non-Janus parent (e.g. a Janus
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "janus.exe"
    lines = ["✗ Another janus.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Janus Desktop, exit any open `janus` REPLs, and")
    lines.append("  stop the gateway (`janus gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `janus update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_janus_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``janus.exe``.
    ...
    when ``janus update`` runs, ``janus.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``janus.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next janus invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Janus Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Janus Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _janus_exe_shims(scripts_dir):
        ...
        print(
            "    Close Janus Desktop, exit other `janus` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `janus update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_janus_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``janus.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``janus.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``janus`` drops off PATH. This wraps
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_janus_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``janus.exe.old.*`` left by prior updates.

    Called early on every janus invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``janus update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `janus update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``janus.exe`` / ``janus-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_janus_exe`` for the rationale.
    """
    ...
    # hours later inside ``janus update``'s desktop-rebuild or skill-sync
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``janus update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``janus.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``janus`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `janus update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `janus update --force` after closing other janus processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from janus_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$JANUS_HOME/bin/uv``, but on Termux the official
    installer may not work (glibc vs bionic).  Fall back to ``pip install uv``
    which gets a Termux-compatible binary.
    """
    from janus_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `janus update`, so we install root-only first
...
class _UpdateOutputStream:
    """Stream wrapper used during ``janus update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.janus/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``janus update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``janus update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.janus/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``janus update --gateway``) the update is already
    ...
    try:
        ...
        # janus_cli.config.get_janus_home to simulate setup failure.
        from janus_cli.config import get_janus_home as _get_janus_home

        logs_dir = _get_janus_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``janus update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``janus update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from janus_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from janus_cli.config import recommended_update_command
        from janus_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``janus update`` without requiring a reinstall.  The
    ...
    ``janus`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/janus.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v janus'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/janus, code at /usr/local/lib/janus-agent).
    fhs_link = Path("/usr/local/bin/janus")
    ...
    path_comment = (
        "# Janus Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of JANUS_HOME before running the update.
    ...
    JANUS_HOME directories.  The ``--backup`` flag on ``janus update``
    ...
    try:
        from janus_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_janus_home so the user sees ~/.janus/...
    try:
        from janus_constants import get_janus_home, display_janus_home

        home = get_janus_home()
        try:
            display_path = f"{display_janus_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  janus import {out_path}")
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``janus update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Janus Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Janus Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `janus update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Janus via pip (for PyPI installs)."""
    from janus_cli import __version__
    from janus_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "janus-agent"]
        ...
        cmd = [pipx, "upgrade", "janus-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "janus-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "janus-agent"]
    ...
    print("✓ Update complete! Restart janus to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Janus Agent...")
    print()

    # On Windows, abort early if another janus.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_janus_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # though `git pull` can't touch $JANUS_HOME, this is cheap
        ...
        try:
            from janus_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in janus_cli/config.py bricked
        # every user who ran ``janus update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``janus
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``janus update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_janus_home added to janus_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # build.  ``janus desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``janus desktop`` shouldn't be forced into a full
        # Electron build by ``janus update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        if (desktop_dir / "package.json").exists() and shutil.which("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "janus_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                build_result = subprocess.run(_desktop_build_cmd, cwd=PROJECT_ROOT, check=False)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `janus desktop` to retry)")

        print()
        print("✓ Code updated!")

        # After git pull, source files on disk are newer than cached Python
        # modules in this process.  Reload janus_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_janus_home() added since the last release.
        try:
            import importlib
            import janus_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit JANUS_HOME so
        # it is not affected by sync_skills()'s module-level JANUS_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's JANUS_HOME env var points at the default or a named profile.
        try:
            from janus_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'janus config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: janus config migrate")
            else:
                print()
                print("Skipped. Run 'janus config migrate' later to configure.")
        else:
        ...
        try:
            from janus_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `janus curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``janus update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from janus_cli.tools_config import install_cua_driver
        ...
        # When running as ``janus update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_janus_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all janus-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. janus-gateway.service or janus-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: janus gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: janus -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Janus gateway unit files are still installed.
        # When both janus.service (from a pre-rename install) and the
        # current janus-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `janus update` surfaces the issue until the user migrates.
        try:
            from janus_cli.gateway import (
                has_legacy_janus_units,
                _find_legacy_janus_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_janus_units():
                print()
                print("⚠ Legacy Janus gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_janus_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (janus.service) fight the current")
                print("  janus-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    janus gateway migrate-legacy")
                print()
        ...
        print("  janus model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``janus -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → janus -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.janus)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  janus profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     janus -p {name} chat")
                else:
        ...
        from janus_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from janus_constants import get_janus_home as _hh
        ...
        from janus_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `janus profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → janus -p {name}  ({wrapper})")
        print()
        ...
        from janus_cli.profiles import profile_exists
        ...
        from janus_cli.profiles import rename_profile
        ...
        from janus_cli.profiles import export_profile
        ...
        from janus_cli.profiles import import_profile
        ...
        import tempfile
        from janus_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="janus_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  janus -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      janus -p {plan.manifest.name} chat")
        ...
        from janus_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `janus profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"janus -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from janus_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("janus_requires"):
            print(f"Requires:     Janus {data['janus_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from janus_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.janus_requires:
        print(f"  Requires: Janus {mf.janus_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``janus dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``janus dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No janus dashboard processes running.")
        return 0

    print(f"{len(pids)} janus dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No janus dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `janus update`.
    ...
    # the same logs directory as every other Janus surface.
    try:
        from janus_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if "JANUS_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["JANUS_WEB_DIST"])
            if "JANUS_WEB_DIST" in os.environ
            else PROJECT_ROOT / "janus_cli" / "web_dist"
        )
    ...
    from janus_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from janus_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from janus_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Janus log files."""
    from janus_cli.logs import tail_log, list_logs
...
# so that in ``janus -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``janus_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``janus -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``janus`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("JANUS_TUI") == "1")
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from janus_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from janus_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["JANUS_DEFER_AGENT_STARTUP"] = "1"
            os.environ["JANUS_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["JANUS_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `janus --tui` is the hot path on phones. The full parser setup imports
...
def main():
    """Main entry point for janus CLI."""
    # Cosmetic: make the process show up as 'janus' instead of 'python3.11'
    ...
    # Sweep stale ``janus.exe.old.*`` quarantine files left by previous
    # ``janus update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_janus_exe``.
    try:
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: janus-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from janus_cli.fallback_cmd import cmd_fallback
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `janus model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    from janus_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from janus_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    migrate_hermes.add_argument(
        "--overwrite", action="store_true", help="Overwrite items that already exist in Janus"
    )
    migrate_hermes.add_argument(
        "--yes", "-y", action="store_true", help="Skip confirmation prompts"
    )
    migrate_hermes.set_defaults(func=cmd_migrate_hermes)
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if JANUS_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy janus.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Janus gateway unit files "
            "(e.g. janus.service) left over from older installs. Profile "
            "units (janus-gateway-<profile>.service) and unrelated "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `janus proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Janus Agent with an interactive wizard. "
        "Run a specific section: janus setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'janus setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Janus.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $JANUS_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Janus")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from janus_cli.send_cmd import register_send_subparser
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Janus CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: janus-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Janus with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set JANUS_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Janus Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.janus/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        "--profile",
        help="Janus profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.janus/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="Janus profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    # =========================================================================
    from janus_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.janus/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.janus/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Janus Agent setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `janus "
    ...
    security_parser = subparsers.add_parser(
        ...
        description=(
            "On-demand vulnerability scan against OSV.dev. Covers the Janus "
            "venv (installed PyPI dists), Python deps declared by plugins under "
            "~/.janus/plugins/, and pinned npx/uvx MCP servers in config.yaml. "
    ...
    audit_parser.add_argument(
        "--skip-venv",
        action="store_true",
        help="Skip scanning the Janus Python venv",
    )
    audit_parser.add_argument(
        "--skip-plugins",
        action="store_true",
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Janus setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Janus Agent. Use 'janus debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    janus debug share              Upload debug report and print URL
    janus debug share --lines 500  Include more log lines
    janus debug share --expire 30  Keep paste for 30 days
    janus debug share --local      Print report locally (no upload)
    janus debug share --no-redact  Disable upload-time secret redaction
    janus debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'janus debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Janus home directory to a zip file",
        description="Create a zip archive of your entire Janus configuration, "
        "skills, sessions, and data (excludes the janus-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/janus-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.janus/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo janus uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from janus_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Janus backup from a zip file",
        description="Extract a previously created Janus backup into your "
        "Janus home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Janus Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from janus_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.janus/skills/.bundled_manifest) "
            "so future 'janus update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    skills_opt_out = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "`janus update`, and any direct sync stop seeding bundled skills "
    ...
    skills_opt_in = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "again on the next `janus update`. Pass --sync to re-seed now."
        ),
    )
    ...
    from janus_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/janus-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `janus plugins enable <name>`",
    )
    ...
    # subcommand — ``janus --help``, ``janus version``, ``janus logs``,
    ...
    try:
        from janus_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_janus_home()}/memories/\n"
                )
                return
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'janus tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # janus tools disable <name...> [--platform cli]
    ...
    # janus tools enable <name...> [--platform cli]
    ...
    tools_postsetup_p = tools_sub.add_parser(
        ...
        description=(
            ...
            "same step `janus tools` runs after you pick a provider that\n"
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            ...
            from janus_cli.tools_config import run_post_setup_command

            sys.exit(run_post_setup_command(args))
        else:
            _require_tty("tools")
            from janus_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `janus computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `janus tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: janus computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: janus computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Janus as an MCP server",
        description=(
            "Manage MCP server connections and run Janus as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'janus mcp add' to connect to a new server, or\n"
            "'janus mcp serve' to expose Janus conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Janus as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `janus mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `janus mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `janus mcp install n8n`)",
    )
    ...
    def cmd_mcp(args):
        from janus_cli.mcp_config import mcp_command
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch janus --resume <id> by replacing the current process
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Janus",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Janus",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.janus/ (by default a "
        "single restore-point archive is written to ~/.janus/backups/ "
        "before apply; restorable with 'janus import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from janus_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Janus Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'janus config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--branch",
        default=None,
        ...
        help=(
            ...
            "If the local checkout is on a different branch, janus will "
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Windows: proceed with the update even when another janus.exe is detected. The concurrent process will likely cause WinError 32 warnings and may leave a reboot-deferred .exe replacement.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Janus Agent",
        description="Remove Janus Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Janus Agent as an ACP (Agent Client Protocol) server",
        description="Start Janus Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print Janus ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Janus provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.janus/node/ "
    ...
    def cmd_acp(args):
        """Launch Janus Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Janus instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `janus update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `janus profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Janus profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Janus Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    # for `janus dashboard` cmdlines and SIGTERM them directly — the same
    # path `janus update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running janus dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running janus dashboard processes and exit",
    )
    # Backward-compat shim: older Janus desktop app shells (<= 0.15.x) spawn the
    # backend as `janus dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Janus couldn't start" with no actionable cause. Accept and silently
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "JANUS_DASHBOARD_OAUTH_CLIENT_ID into ~/.janus/.env, and prints "
            "how to engage the login gate. Requires being logged in (janus setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://janus.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
    ...
    # for one release. The Janus-Setup.exe success screen tells users to
    # run `janus desktop` from a terminal, so the canonical name needs
    ...
    gui_parser = subparsers.add_parser(
        ...
        description=(
            "Launch the Janus Electron desktop app. By default this installs "
    ...
    gui_parser.add_argument(
        "--ignore-existing",
        action="store_true",
        help="Force Desktop to ignore any janus CLI already on PATH during backend resolution",
    )
    gui_parser.add_argument(
        "--janus-root",
        help="Override the Janus source root used by Desktop (sets JANUS_DESKTOP_JANUS_ROOT)",
    )
    gui_parser.add_argument(
        "--cwd",
        help="Initial project directory for Desktop chat sessions (sets JANUS_DESKTOP_CWD)",
    )
    gui_parser.add_argument(
        "--skip-build",
        action="store_true",
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Janus log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log / gui.log / desktop.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    janus logs                    Show last 50 lines of agent.log
    janus logs -f                 Follow agent.log in real time
    janus logs errors             Show last 50 lines of errors.log
    janus logs gateway -n 100     Show last 100 lines of gateway.log
    janus logs gui -f             Follow gui.log in real time
    janus logs desktop -f         Follow desktop.log (Electron app boot/backend)
    janus logs --level WARNING    Only show WARNING and above
    janus logs --session abc123   Filter by session ID
    janus logs --component tools  Only show tool-related lines
    janus logs --since 1h         Lines from the last hour
    janus logs --since 30m -f     Follow, starting from 30 min ago
    janus logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``janus -c Pokemon Agent Dev`` → ``janus -c 'Pokemon Agent Dev'``
    ...
    from janus_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'janus -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (janus hooks list, cron
