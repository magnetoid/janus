...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"(janus\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*janus)"
    r"|(systemctl\s+(restart|stop|start)\s+.*janus)"
    r"|(p?kill\s+.*janus.*gateway)"
)
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'janus cron create ...' or the /cron command in chat.", Colors.DIM))
        return
    ...
    if not find_gateway_pids():
        ...
        print(color("     Start it with: janus gateway install", Colors.DIM))
        print(color("                    sudo janus gateway install --system  # Linux servers", Colors.DIM))
        print()
...
def cron_status():
    ...
    if pids:
        ...
        print("    janus gateway install    # Install as a user service")
        print("    sudo janus gateway install --system  # Linux servers: boot-time system service")
        print("    janus gateway            # Or run in foreground")
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `janus gateway restart` from a shell outside the gateway.",
            Colors.RED,
        ))
        return 1
...
def cron_command(args):
    ...
    print("Usage: janus cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
