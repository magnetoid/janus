...
Pure logic: walks a Janus config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `janus doctor` and a future `janus migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Janus config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Janus config and return retirement issues.
