"""
Backup and import commands for janus CLI.

`janus backup` creates a zip archive of the entire ~/.janus/ directory
(excluding the janus-agent repo and transient files).

`janus import` restores from a backup zip, overlaying onto the current
JANUS_HOME root.
"""
...
from janus_constants import get_default_janus_root, get_janus_home, display_janus_home
...
_EXCLUDED_DIRS = {
    "janus-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to janus root) should be skipped."""
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside JANUS_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Janus home directory."""
    janus_root = get_default_janus_root()

    if not janus_root.is_dir():
        print(f"Error: Janus home directory not found at {janus_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"janus-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"janus-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_janus_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(janus_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(janus_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(janus_root)
    ...
    print(f"\nRestore with: janus import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Janus backup.
    ...
    # Look for telltale files that a janus home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Janus backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.janus/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a janus dir name
        if prefix in {".janus", "janus"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_janus_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (janus_root / "config.yaml").exists()
        has_env = (janus_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Janus configuration.")
        ...
        for member in members:
            ...
            target = janus_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(janus_root.resolve())
            except ValueError:
        ...
        profiles_dir = janus_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # janus_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: janus profile list  (after installing janus)")

        # Guidance
        print()
        if not (janus_root / "janus-agent").is_dir():
            print("Note: The janus-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: janus update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  janus -p {pname} gateway install")

        print("Done. Your Janus configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and janus backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to JANUS_HOME).
...
# — `janus update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(janus_home: Optional[Path] = None) -> Path:
    home = janus_home or get_janus_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    janus_home: Optional[Path] = None,
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(janus_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside JANUS_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``janus update``.
    ...
    Args:
        ...
        janus_home: Override for the Janus home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(janus_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for janus backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_janus_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, janus_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``janus_root`` to ``out_path``.
...
def _pre_update_backup_dir(janus_home: Optional[Path] = None) -> Path:
    home = janus_home or get_janus_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of JANUS_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<JANUS_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``janus update``) should continue even if the backup fails.
    """
    janus_root = janus_home or get_default_janus_root()
    if not janus_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(janus_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, janus_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `janus claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of JANUS_HOME under ``backups/`` before a
    ``janus claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``janus import <archive>``.  Writes to
    ``<JANUS_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `janus import` and the
