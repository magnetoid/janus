"""
Profile management for multiple isolated Janus instances.

Each profile is a fully independent JANUS_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.janus/profiles/<name>/`` by default.

The "default" profile is ``~/.janus`` itself — backward compatible,
zero migration needed.

Usage::

    janus profile create coder          # fresh profile + bundled skills
    janus profile create coder --clone  # also copy config, .env, SOUL.md, skills
    janus profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    janus -p coder chat                 # or via flag
    janus profile use coder             # set as sticky default
    janus profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See janus_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Marker file written by `janus profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `janus
...
# `janus skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Janus infrastructure directories that only the default profile
         (``~/.janus``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_janus_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.janus) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "janus-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "janus", "default", "test", "tmp", "root", "sudo",
})

# Janus subcommands that cannot be used as profile names/aliases
_JANUS_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the janus root, NOT to the current JANUS_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where JANUS_HOME points outside
    ``~/.janus``, profiles live under ``JANUS_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_janus_home() / "profiles"


def _get_default_janus_home() -> Path:
    """Return the default (pre-profile) JANUS_HOME path.

    In standard deployments this is ``~/.janus``.
    In Docker/custom deployments where JANUS_HOME is outside ``~/.janus``
    (e.g. ``/opt/data``), returns JANUS_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_janus_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Janus installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its JANUS_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_janus_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, janus subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _JANUS_SUBCOMMANDS:
        return f"'{canon}' conflicts with a janus subcommand"
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``janus -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``janus profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``janus profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Janus config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_janus_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``janus update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.janus)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.janus/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `janus update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`janus profile create --no-skills`).\n"
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `janus profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `janus -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches JANUS_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``janus profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "JANUS_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.janus).\n"
            "To remove everything, use: janus uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from janus_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set JANUS_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("JANUS_HOME")
    try:
        os.environ["JANUS_HOME"] = str(profile_dir)
        from janus_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["JANUS_HOME"] = old_home
        elif "JANUS_HOME" in os.environ:
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.janus/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: janus profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from JANUS_HOME.

    Returns ``"default"`` if JANUS_HOME is not set or points to ``~/.janus``.
    Returns the profile name if JANUS_HOME points into ``~/.janus/profiles/<name>``.
    Returns ``"custom"`` if JANUS_HOME is set to an unrecognized path.
    """
    from janus_constants import get_janus_home
    janus_home = get_janus_home()
    resolved = janus_home.resolve()

    default_resolved = _get_default_janus_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.janus itself — its parent is ~/ and its
        # directory name is ".janus", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: janus profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.janus itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.janus). "
            "Specify a different name: janus profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="janus_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"janus_{old_name}"
    legacy_old_host = f"janus.{old_name}"
    new_host = f"janus_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_janus_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("janus_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a JANUS_HOME path string.

    Called early in the CLI entry point, before any janus modules
    are imported, to set the JANUS_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
