"""
Interactive setup wizard for Janus Agent.
...
_DOCS_BASE = "https://github.com/magnetoid/janus"
...
# display_janus_home imported lazily at call sites (stale-module safety during janus update)
...
from janus_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from janus_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Janus Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Janus using environment variables or config commands:")
    print_info("  janus config set model.provider custom")
    print_info("  janus config set model.base_url http://localhost:8080/v1")
    print_info("  janus config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'janus setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from janus_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'janus setup')")


def _print_setup_summary(config: dict, janus_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'janus setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from janus_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `janus tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from janus_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'janus setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'janus setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'janus setup terminal'"))
    ...
    # Spotify (OAuth via janus auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'janus setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {janus_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('janus setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('janus setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('janus setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('janus setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('janus setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('janus config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('janus config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('janus config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('janus', Colors.GREEN)}              Start chatting")
    print(f"   {color('janus gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('janus doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `janus tools` and `janus setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``janus model``)
    ...
    provider added to ``janus model`` is automatically available here.
    ...
    # Delegate to the shared janus model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from janus_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: janus model")
    ...
    # on demand via `janus auth add`, `janus setup` vision, and
    # `janus setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``janus model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from janus_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.janus/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Janus auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via janus setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'janus setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Janus runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `janus setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `janus setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into JANUS_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("JANUS_MAX_ITERATIONS")
    ...
    print_info("  Run `janus setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # JANUS_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from janus_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
    ...
    try:
        ...
        if "/profiles/" in janus_home:
            profile_name = janus_home.rstrip("/").rsplit("/", 1)[-1]
    except Exception:
        pass
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Janus delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `janus update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://github.com/magnetoid/janus")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under JANUS_HOME, and print
    ...
    """
    try:
        from janus_cli.slack_cli import _build_full_manifest
        from janus_constants import get_janus_home

        manifest = _build_full_manifest(
            bot_name="Janus",
            bot_description="Your Janus agent on Slack",
        )
        target = Path(get_janus_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `janus slack manifest --write` anytime to refresh after "
            "Janus adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Janus delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Janus to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from janus_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print_info("   https://github.com/magnetoid/janus")
    print()
    print_info("   Open config in your editor:  janus config edit")
    print_info("   Open config in your editor:  janus config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from janus_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Janus from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'janus setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     janus config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_janus_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo janus gateway install --system")
                print_info("  Or run in foreground:  janus gateway")
        else:
            from janus_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   janus gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   janus gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `janus setup tools` and `janus tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``janus_cli.auth`` — lists every supported
    ...
    """
    try:
        from janus_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from janus_cli.config
    ...
    if section_key == "model":
        ...
        from janus_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_janus.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_janus migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_janus", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Janus.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Janus to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Janus at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Janus at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Janus at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Janus at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Janus equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Janus config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Janus.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Janus's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(janus_home: Path) -> bool:
    ...
    print_info("Janus can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: janus claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=janus_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: janus claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Janus configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Janus config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Janus (use janus claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot Cloud Industry Portal setup — OAuth + model pick + provider + Tool Gateway.

    Wired into ``janus setup --portal`` and ``janus portal``. This is the
    ...
    Janus session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``janus setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``janus model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``janus portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Janus Setup — Cloud Industry Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    print_info("    — all routed through your Cloud Industry Portal sub.")
    print()
    print_info("  Sign up: https://portal.cloud-industry.com/manage-subscription")
    print()
    ...
    # setup calls, so `janus portal` == quick setup's Nous step.
    try:
        from janus_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `janus portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `janus portal`: %s", exc)
        print()
        print_error(f"  Cloud Industry Portal setup encountered an error: {exc}")
        print_info("  You can retry later with `janus portal`.")
        return
    ...
    print_info("  Run `janus portal info` to inspect routing.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      janus setup           — full or quick (auto-detected)
      janus setup model     — just model/provider
      janus setup tts       — just text-to-speech
      janus setup terminal  — just terminal backend
      janus setup gateway   — just messaging platforms
      janus setup tools     — just tool configuration
      janus setup agent     — just agent settings
    """
    from janus_cli.config import is_managed, managed_error
    if is_managed():
    ...
    # --portal: one-shot Cloud Industry Portal setup. Skips the rest of the wizard.
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Janus Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Janus Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Janus Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, janus_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Janus configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'janus setup model|terminal|")
        ...
        setup_mode = prompt_choice(
            "How would you like to set up Janus?",
            [
                "Quick Setup (Cloud Industry Portal) — free OAuth login, no API keys, model + tools (recommended)",
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, janus_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {janus_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'janus config edit'")
    ...
    # Tune later with `janus setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, janus_home)


def _run_first_time_quick_setup(config: dict, janus_home, is_existing: bool):
    """Streamlined first-time setup via Cloud Industry Portal: OAuth, model, terminal & messaging.

    Routes straight to the Cloud Industry Portal provider — runs the device-code OAuth
    ...
    settings, tools); the user can customize later via ``janus setup <section>``
    or switch providers with ``janus model``.
    """
    from janus_cli.config import load_config

    # Step 1: Cloud Industry Portal — OAuth login + model selection.
    # _model_flow_nous() handles both the logged-out path (device-code OAuth,
    # which selects a model internally) and the already-logged-in path (curated
    # Nous model picker). Provider is set to "nous" by the login/model save.
    print()
    print_header("Cloud Industry Portal")
    print_info("One subscription, 300+ models, plus the Tool Gateway:")
    print_info("  web search, image generation, TTS, browser automation.")
    print_info("Sign up: https://portal.cloud-industry.com/manage-subscription")
    print()
    try:
        ...
        print()
        print_info("Cloud Industry Portal setup cancelled.")
    except Exception as exc:
        logger.debug("_model_flow_nous error during quick setup: %s", exc)
        print_warning(f"Cloud Industry Portal setup encountered an error: {exc}")
        print_info("You can try again later with: janus model")
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'janus setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    janus setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  janus setup gateway")
    print()

    _print_setup_summary(config, janus_home)


def _run_quick_setup(config: dict, janus_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'janus setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Janus to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'janus setup gateway'.")
