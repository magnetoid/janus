"""
Status command for janus CLI.

Shows the status of all Janus Agent components.
"""
...
from janus_cli.auth import AuthError, resolve_provider
from janus_cli.colors import Colors, color
from janus_cli.config import get_env_path, get_env_value, get_janus_home, load_config
from janus_cli.models import provider_label
from janus_cli.nous_account import (
    format_nous_portal_entitlement_message,
    get_nous_portal_account_info,
)
from janus_cli.nous_subscription import get_nous_subscription_features
from janus_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``janus config``'s
...
def show_status(args):
    """Show status of all Janus Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Janus Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: janus model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: janus auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: janus auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: janus model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from janus_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from janus_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   janus gateway")
    ...
    jobs_file = get_janus_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_janus_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'janus doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'janus setup' to configure", Colors.DIM))
    print()
