...
Automates what a user otherwise does by hand: open the Cloud Industry Portal
...
This command:
  1. Resolves a fresh Cloud Industry Portal access token from the existing login
  ...
  3. Writes ``JANUS_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``JANUS_DASHBOARD_PORTAL_URL`` into ``~/.janus/.env`` idempotently.
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    Precedence:
      1. ``override`` — explicit ``--portal-url`` flag or
         ``JANUS_DASHBOARD_PORTAL_URL`` env (used for testing against a
    ...
    try:
        from janus_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
...
def _register_self_hosted_client(
    *,
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "Cloud Industry Portal rejected the access token (401). "
        ...
        raise RuntimeError(
            f"Could not reach Cloud Industry Portal at {portal_base_url}: {exc.reason}"
        ) from exc
...
def _print_post_register_hint(
    *,
    ...
    from janus_cli.config import get_env_path

    env_path = get_env_path()
    print()
    print(f"  Wrote to {env_path}:")
    print(f"    JANUS_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
    if wrote_portal_url:
        print(f"    JANUS_DASHBOARD_PORTAL_URL={portal_base_url}")
    print()
    print(
        "  Heads up — Nous login only *engages* on a non-loopback bind. A plain\n"
        "  `janus dashboard` (localhost) leaves the gate off and serves locally\n"
    ...
    if custom_redirect_uri:
        ...
        print("    janus dashboard --host 0.0.0.0")
...
def cmd_dashboard_register(args) -> None:
    ...
    # stamped in by the orchestrator (NAS sets JANUS_DASHBOARD_OAUTH_CLIENT_ID
    ...
    if is_managed():
        print(
            "✗ `janus dashboard register` is not available in a managed/hosted "
            "install.\n"
    ...
    try:
        ...
        if getattr(exc, "relogin_required", False):
            print("✗ You're not logged into Cloud Industry Portal.")
            print("  Run `janus setup` (or `janus auth login nous`) first, then retry.")
        else:
            print(f"✗ Could not resolve a Cloud Industry Portal access token: {exc}")
        sys.exit(1)
    ...
    # JANUS_DASHBOARD_PORTAL_URL env var, else the stored login's portal.
    ...
    try:
        save_env_value("JANUS_DASHBOARD_OAUTH_CLIENT_ID", client_id)
    except Exception as exc:
        print(f"✗ Failed to write JANUS_DASHBOARD_OAUTH_CLIENT_ID to .env: {exc}")
        print(f"  Set it manually:  JANUS_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
        sys.exit(1)

    wrote_portal_url = False
    default_portal = "https://portal.cloud-industry.com"
    ...
    if not existing_portal and portal_base_url.rstrip("/") != default_portal:
        try:
            save_env_value("JANUS_DASHBOARD_PORTAL_URL", portal_base_url)
