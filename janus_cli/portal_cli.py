"""``janus portal`` — the human-readable entry point for Cloud Industry Portal.
...
``janus auth add nous --type oauth`` (which still works), is identical to
``janus setup --portal``, and runs the same Nous flow as the first-time quick
setup.

Subcommands:
  (none)   Log in to Cloud Industry Portal + set it up (one-shot onboarding).
...
already in ``janus auth`` or ``janus tools``. It's the onboarding + discovery
...
DEFAULT_PORTAL_URL = "https://portal.cloud-industry.com"
...
def _cmd_status(args) -> int:
    ...
    print()
    print(color("  Cloud Industry Portal", Colors.MAGENTA))
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `janus model`)")
    ...
    for feat in features.items():
        if feat.managed_by_nous:
            state = color("via Cloud Industry Portal", Colors.GREEN)
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Cloud Industry Portal — sign in with `janus portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    """Run the one-shot Cloud Industry Portal onboarding (login + model + provider + tools).
    ...
    """
    from janus_cli.setup import _run_portal_one_shot
...
def portal_command(args) -> int:
    """Top-level dispatch for `janus portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `janus portal` is the
        # human-readable alias for `janus auth add nous --type oauth` /
        # `janus setup --portal`.
    ...
    print("Run `janus portal -h` for usage.", file=sys.stderr)
    return 1
...
def add_parser(subparsers) -> None:
    ...
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Cloud Industry Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `janus portal` with no subcommand to log in to Cloud Industry Portal "
    ...
    portal_sub.add_parser(
        "login",
        help="Log in to Cloud Industry Portal + set it up (default; one-shot onboarding)",
    )
    portal_sub.add_parser(
        "info",
