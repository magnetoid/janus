#!/bin/sh
# shellcheck shell=sh
# /opt/janus/bin/janus — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `janus` user (UID 10000). When an operator runs `docker exec <c> janus ...`
# the default UID is root (0), and any file the command writes under
# $JANUS_HOME — auth.json, .env, config.yaml — ends up root-owned and
# unreadable to the supervised gateway. The most common manifestation: the
# user runs `docker exec <c> janus login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: Janus is not logged into Nous
# Portal" on every incoming message — even though `docker exec <c> janus
...
# Fix
# ---
# This shim sits at /opt/janus/bin/janus and is placed earliest on PATH.
# When invoked as root, it drops to the janus user (via s6-setuidgid)
# before exec'ing the real venv binary, so anything that writes under
# $JANUS_HOME is uid-aligned with the supervised processes. When invoked
# as any non-root UID — including the supervised processes themselves,
# `docker exec --user janus`, kanban subagents, etc. — it short-circuits
...
# (/opt/janus/.venv/bin/janus), so the second hop cannot re-enter this
# shim regardless of PATH state. No sentinel env var needed.
#
# Opt-out: set JANUS_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
# state via the janus CLI. Default is to drop.

set -e

REAL=/opt/janus/.venv/bin/janus

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "janus-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user janus`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi

# Root, with opt-out set? Honor it.
case "${JANUS_DOCKER_EXEC_AS_ROOT:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        exec "$REAL" "$@"
        ;;
esac

# Root, no opt-out. Drop to the janus user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "janus-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "janus-shim: re-run with --user janus or set JANUS_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the janus user's home before dropping privileges. Without
