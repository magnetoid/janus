# Janus Agent Has Had "Routines" Since March
...
Janus equivalent — works today:
```bash
janus webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Janus implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Janus Agent |
|---|---|---|
...
## Things Janus Does That Routines Can't
...
Janus has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Janus Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
janus gateway setup    # enable webhooks
janus webhook subscribe pr-review \
  --events "pull_request" \
...
*Janus Agent is built by [Cloud Industry](https://cloud-industry.com). Open source, model-agnostic, runs on your infrastructure.*
