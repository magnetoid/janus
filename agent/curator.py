...
def _state_file() -> Path:
    return get_janus_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.janus/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `janus update` re-seeds.
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``janus update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``janus curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `janus curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Janus' background skill CURATOR. This is an "
    ...
    "into ~/.janus/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: janus-config-*, janus-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.janus/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.janus/logs/curator/``)
    ...
    authored skill data in ``~/.janus/skills/``.

    ``ensure_janus_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_janus_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every janus install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: janus curator status
        keep an umbrella stable: janus curator pin document-tools
    ...
    lines.append("full report: janus curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: janus curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.janus/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.janus/skills/.archive/` for "
            "safety and can be restored via `janus curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.janus/skills/.archive/`. "
            "Restore any via `janus curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `janus curator restore <name>`")
    lines.append("- All archives live under `~/.janus/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `janus curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `janus update` only if "
        ...
        # recorded in state so `janus curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``janus model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `janus model` → auxiliary
