...
Plugins built on Janus Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Janus
...
:class:`~janus_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Janus' main config
...
handles every provider, fallback chain, and per-task override Janus
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`janus_cli.plugins.PluginContext`
