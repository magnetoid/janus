...
``auth.json``.  They intentionally have no dependency on ``janus_cli.auth`` so
...
# Sources Janus owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Janus-owned OAuth/device-code state)
