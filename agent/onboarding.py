...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Janus starts and finds ``~/.openclaw/``.

    Points users at ``janus claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``janus claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Janus, run "
        "`janus claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `janus claw cleanup` (renames it to ~/.openclaw.pre-migration — "
