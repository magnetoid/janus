...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Janus with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.janus/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Janus
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``janus tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``janus setup`` for
