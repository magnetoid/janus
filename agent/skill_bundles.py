...
Storage
-------
Bundles live in ``~/.janus/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``JANUS_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``janus bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under JANUS_HOME.

    Honors ``JANUS_BUNDLES_DIR`` for tests; falls back to
    ``<JANUS_HOME>/skill-bundles``.
    """
    override = os.environ.get("JANUS_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_janus_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `janus bundles` CLI subcommand.
# ---------------------------------------------------------------------------
