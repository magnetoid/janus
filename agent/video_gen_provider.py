...
as ``kind: backend``) or ``~/.janus/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``janus tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$JANUS_HOME/cache/videos/``, creating parents as needed."""
    from janus_constants import get_janus_home

    path = get_janus_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$JANUS_HOME/cache/videos/``.
