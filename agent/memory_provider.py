...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - janus_home (str): The active JANUS_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.janus``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "janus").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'janus memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], janus_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'janus memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``janus_home`` is the active JANUS_HOME directory path.
