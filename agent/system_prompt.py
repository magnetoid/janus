...
upstream prefix cache warm.  See ``janus-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Janus never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want JANUS_HOME persona while keeping
    ...
    # Pointer to the janus-agent skill + docs for user questions about Janus itself.
    stable_parts.append(JANUS_AGENT_HELP_GUIDANCE)
    ...
    # JANUS_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Active-profile hint — names the Janus profile the agent is running
    # under so it doesn't conflate ~/.janus/skills/ (default profile) with
    # ~/.janus/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        ...
        stable_parts.append(
            f"Active Janus profile: {active_profile}. This session reads "
            f"and writes ~/.janus/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.janus/skills/, ~/.janus/plugins/, "
            f"~/.janus/cron/, ~/.janus/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Janus never rebuilds or reinjects parts of it
