...
Owns one Codex thread per Janus session. Drives `turn/start`, consumes
...
# Janus' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_JANUS_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Janus/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Janus session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _JANUS_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("JANUS_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="janus",
            client_title="Janus Agent",
            client_version=_get_janus_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Janus' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Janus' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own janus-tools callback we
            # auto-accept — the user already approved Janus' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Janus approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Janus returns 'once', 'session', 'always', or 'deny'.
...
def _get_janus_version() -> str:
    """Best-effort Janus version string for codex's userAgent line."""
    try:
