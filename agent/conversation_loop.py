...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Janus tool use: "
    ...
    return (
        ...
        f"context, but Janus needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Janus config, set `model.ollama_num_ctx: 65536` "
...
def _is_nous_inference_route(provider: str, base_url: str) -> bool:
    ...
    return (
        base_url_host_matches(base, "inference-api.cloud-industry.com")
        or base_url_host_matches(base, "inference.cloud-industry.com")
    )
...
def run_conversation(
    ...
    # ``janus logs --session <id>`` can filter a single conversation.
    ...
    # are reused.  The system prompt is Janus's territory; plugins
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Janus internals.
        #
        # Janus invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Janus tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            ...
            if agent.provider == "nous":
                try:
                    ...
                    if _nous_remaining is not None and _nous_remaining > 0:
                        _nous_msg = (
                            f"Cloud Industry Portal rate limit active — "
            ...
            try:
                ...
                try:
                    from janus_cli.middleware import apply_llm_request_middleware
                ...
                if env_var_enabled("JANUS_DUMP_REQUESTS"):
                ...
                from janus_cli.middleware import run_llm_execution_middleware
                ...
                if (
                    ...
                    print(f"{agent.log_prefix}     • Re-authenticate: janus auth add nous")
                    print(f"{agent.log_prefix}     • Check credits / billing: https://portal.cloud-industry.com")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `janus doctor` for credential-chain diagnostics, or")
                    ...
                    from janus_constants import display_janus_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Janus-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: janus config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: janus config set ANTHROPIC_API_KEY \"\"")
                ...
                # ── Cloud Industry Portal: record rate limit & skip retries ─────
                ...
                #
                # IMPORTANT: Cloud Industry Portal multiplexes multiple upstream
                # providers (DeepSeek, Kimi, MiMo, Janus).  A 429 can
                ...
                # which Janus' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Janus' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`janus",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `janus auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `janus model`.", force=True)
                            else:  # nous
                                agent._vprint(f"{agent.log_prefix}   💡 Cloud Industry Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: janus portal", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Check your portal account: https://portal.cloud-industry.com", force=True)
                                # ``:free`` is OpenRouter slug syntax; Cloud Industry Portal will reject
                                ...
                                if isinstance(_model, str) and _model.endswith(":free"):
                                    ...
                                    agent._vprint(f"{agent.log_prefix}         Cloud Industry Portal won't recognize that model name. Either switch to a", force=True)
                            ...
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: janus setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        janus fallback add   (interactive picker — same as `janus model`)",
                            force=True,
                        )
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        _policy_response = (
                            ...
                            f"(not a Janus/gateway failure).\n\n"
                            f"Provider message: {_summary}\n\n"
                            f"Try rephrasing the request, narrowing the context, or "
                            f"adding a fallback provider with `janus fallback add`."
                        )
                        return {
    ...
    if final_response is None and (
        ...
        _kanban_task = os.environ.get("JANUS_KANBAN_TASK")
        if _kanban_task:
            try:
