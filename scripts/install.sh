...
# Python-driven tool session (e.g. Janus terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:magnetoid/janus.git"
REPO_URL_HTTPS="https://github.com/magnetoid/janus.git"
JANUS_HOME="${JANUS_HOME:-$HOME/.janus}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${JANUS_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$JANUS_INSTALL_DIR"
...
#   code at /usr/local/lib/janus-agent, command at /usr/local/bin/janus,
#   data still at /root/.janus (JANUS_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$JANUS_HOME/.no-bundled-skills so future"
            echo "                   'janus update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Janus.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.janus/janus-agent"
            echo "                   default (root, Linux): /usr/local/lib/janus-agent"
            echo "  --janus-home PATH  Data directory (default: ~/.janus, or \$JANUS_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Janus installs the code under"
            echo "  /usr/local/lib/janus-agent and links the command into"
            echo "  /usr/local/bin/janus (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$JANUS_HOME"
            echo "  (default /root/.janus).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$JANUS_HOME/janus-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs janus setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# intentional, but they leave the checkout dirty — which forces `janus update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Janus-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Janus Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install janus command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `janus` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $JANUS_HOME/janus-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $JANUS_HOME/janus-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/janus-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $JANUS_HOME/janus-agent — then preserve it)
#
# Always no-op when the user set --dir or $JANUS_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in JANUS_HOME.
    if is_termux; then
        INSTALL_DIR="$JANUS_HOME/janus-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$JANUS_HOME/janus-agent/.git" ]; then
            INSTALL_DIR="$JANUS_HOME/janus-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/janus-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/janus-agent"
        ...
        # /usr/local/bin/janus wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/janus"
        log_info "  Data:    $JANUS_HOME (unchanged)"
...
get_janus_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/janus" ]; then
        echo "$link_dir/janus"
    else
        echo "janus"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://github.com/magnetoid/janus)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Janus owns its own uv at $JANUS_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (janus_cli/managed_uv.py) looks in the same
    # place, so install.sh and `janus update` stay in sync.
    local _managed_uv="$JANUS_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $JANUS_HOME/bin ..."
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/janus-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/janus-uv-installer.$$.sh")"
    ...
    # directly into $JANUS_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$JANUS_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the Janus-
...
check_node() {
    ...
    # Prefer a Janus-managed Node from a previous run over a too-old system one.
    if [ -x "$JANUS_HOME/node/bin/node" ] && node_satisfies_build "$("$JANUS_HOME/node/bin/node" --version)"; then
        export PATH="$JANUS_HOME/node/bin:$PATH"
        log_success "Node.js $("$JANUS_HOME/node/bin/node" --version) found (Janus-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Janus-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.janus/node/..."
    ...
    # Place into ~/.janus/node/ and symlink binaries into the same bin dir
    # the janus command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$JANUS_HOME/node"
    mkdir -p "$JANUS_HOME"
    mv "$extracted_dir" "$JANUS_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$JANUS_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$JANUS_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$JANUS_HOME/node/bin/npx"  "$node_link_dir/npx"

    export PATH="$JANUS_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$JANUS_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.janus/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Janus install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Janus Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="janus-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Janus behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up janus command..."

    if [ "$USE_VENV" = true ]; then
        JANUS_BIN="$INSTALL_DIR/venv/bin/janus"
    else
        JANUS_BIN="$(which hermes 2>/dev/null || echo "")"
        if [ -z "$JANUS_BIN" ]; then
            log_warn "janus not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$JANUS_BIN" ]; then
        log_warn "janus entry point not found at $JANUS_BIN"
    ...
    # Create a user-facing shim for the janus command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $JANUS_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$JANUS_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/janus"
    cat > "$command_link_dir/janus" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$JANUS_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/janus"
    log_success "Installed janus launcher → $command_link_display_dir/janus"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "janus command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v janus' \
                >/dev/null 2>&1; then
        ...
        log_info "janus not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Janus Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Janus Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Janus Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so janus works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.janus directory structure (config at top level, code in subdir)
    mkdir -p "$JANUS_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.janus/.env (top level, easy to find)
    if [ ! -f "$JANUS_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$JANUS_HOME/.env"
            log_success "Created ~/.janus/.env from template"
        else
            touch "$JANUS_HOME/.env"
            log_success "Created ~/.janus/.env"
        fi
    else
        log_info "~/.janus/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$JANUS_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.janus/config.yaml (top level, easy to find)
    if [ ! -f "$JANUS_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$JANUS_HOME/config.yaml"
            log_success "Created ~/.janus/config.yaml from template"
        fi
    else
        log_info "~/.janus/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$JANUS_HOME/SOUL.md" ]; then
        cat > "$JANUS_HOME/SOUL.md" << 'SOUL_EOF'
# Janus Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Janus communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.janus/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.janus/"

    # Seed bundled skills into ~/.janus/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `janus update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'janus update'." \
            > "$JANUS_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $JANUS_HOME/.no-bundled-skills"
        log_info "  Future 'janus update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.janus/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.janus/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$JANUS_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$JANUS_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.janus/skills/"
            fi
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Janus Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Janus will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (janus --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `janus update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'janus setup' after install."
        return 0
    fi
    ...
    # Run janus setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m janus_cli.main setup < /dev/tty
    else
        python -m janus_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$JANUS_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Janus to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$JANUS_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'janus whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                JANUS_CMD="$(get_janus_command_path)"
                $JANUS_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'janus whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'janus gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $JANUS_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $JANUS_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: janus gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: janus gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $JANUS_CMD gateway > "$JANUS_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.janus/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: janus gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: janus gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $JANUS_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $JANUS_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $JANUS_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}janus${NC}              Start chatting"
    echo -e "   ${GREEN}janus setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}janus config${NC}       View/edit configuration"
    echo -e "   ${GREEN}janus config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}janus gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}janus update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'janus' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'janus' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'janus' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$JANUS_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$JANUS_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$JANUS_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$JANUS_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$JANUS_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Janus for pip install"
    ...
    JANUS_CMD="$(command -v hermes 2>/dev/null || echo "")"
    if [ -n "$JANUS_CMD" ]; then
        log_info "Running janus setup..."
        "$JANUS_CMD" setup
    else
        log_warn "janus command not found on PATH"
        log_info "Try: python -m janus_cli.main setup"
    fi
}
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Janus-managed Node to PATH, so
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Janus" ]; then
            app="$desktop_dir/release/linux-unpacked/Janus"
        elif [ -x "$desktop_dir/release/linux-unpacked/janus" ]; then
            app="$desktop_dir/release/linux-unpacked/janus"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Janus.app" \
            "$desktop_dir/release/mac/Janus.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Janus is damaged and can't be opened").
    ...
    # checkout stays clean for the next `janus update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Janus-managed Node
            # provisioned during prerequisites/node-deps (at $JANUS_HOME/node/bin)
        ...
        complete)
            ...
            resolve_install_layout
            print_success
            echo "git" > "$JANUS_HOME/.install_method"
            ;;
        *)
