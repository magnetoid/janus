...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$JanusHome = $(if ($env:JANUS_HOME) { $env:JANUS_HOME } else { "$env:LOCALAPPDATA\janus" }),
    [string]$InstallDir = $(if ($env:JANUS_HOME) { "$env:JANUS_HOME\janus-agent" } else { "$env:LOCALAPPDATA\janus\janus-agent" }),
    ...
    # builds apps/desktop into a launchable Janus.exe.
    #
    # Why opt-in:
    #   * Janus-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Janus.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Janus.exe
    ...
    #     `janus desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:magnetoid/janus.git"
$RepoUrlHttps = "https://github.com/magnetoid/janus.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * Janus Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Cloud Industry.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $JanusHome)) {
        New-Item -ItemType Directory -Force -Path $JanusHome | Out-Null
    }
    $envFile = Join-Path $JanusHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $JanusHome "node"
...
function Install-Uv {
    # Janus owns its own uv at $JanusHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (janus_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `janus update` stay in sync.
    $managedUv = Join-Path $JanusHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $JanusHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $JanusHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $JanusHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $JanusHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and JANUS_HOME wasn't propagated).
...
function Install-Git {
    ...
    which Janus uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\janus\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Janus needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Janus copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\janus\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``JANUS_GIT_BASH_PATH`` (User scope) so Janus can find it in a fresh
    ...
    # Download PortableGit into $JanusHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $JanusHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Janus features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Janus needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``JANUS_GIT_BASH_PATH`` (User env scope) so Janus can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $JanusHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $JanusHome\git\usr\bin\bash.exe
    $candidates += "$JanusHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$JanusHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("JANUS_GIT_BASH_PATH", $candidate, "User")
            $env:JANUS_GIT_BASH_PATH = $candidate
            Write-Info "Set JANUS_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Janus may not find Git Bash."
    Write-Info "If needed, set JANUS_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$JanusHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$JanusHome\node;$env:Path"
        Write-Success "Node.js $version found (Janus-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Janus-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $JanusHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $JanusHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\janus-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$JanusHome\node") { Remove-Item -Recurse -Force "$JanusHome\node" }
                Move-Item $extractedDir.FullName "$JanusHome\node"
                ...
                $version = & "$JanusHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $JanusHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\janus-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    $stashName = "janus-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Janus behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running janus processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/magnetoid/janus/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/magnetoid/janus/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/magnetoid/janus/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\janus-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\janus-agent-extract"
    ...
    # next `janus update` checkout aborts on a "dirty" tree the user never
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `janus.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install janus-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and janus.exe broken with
    ...
    # here, not 30 seconds later when the user runs `janus`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `janus dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `janus dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `janus dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up janus command..."
    
    if ($NoVenv) {
        $janusBin = "$InstallDir"
    } else {
        $janusBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so janus is globally available
    # On Windows, the janus.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$janusBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$janusBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $janusBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set JANUS_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\janus instead
    # of the Unix default ~/.janus
    $currentJanusHome = [Environment]::GetEnvironmentVariable("JANUS_HOME", "User")
    if (-not $currentJanusHome -or $currentJanusHome -ne $JanusHome) {
        [Environment]::SetEnvironmentVariable("JANUS_HOME", $JanusHome, "User")
        Write-Success "Set JANUS_HOME=$JanusHome"
    }
    $env:JANUS_HOME = $JanusHome
    
    # Update current session
    $env:Path = "$janusBin;$env:Path"
    
    Write-Success "janus command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.janus-bootstrap-complete which tells the Janus
    ...
    $markerPath = Join-Path $InstallDir ".janus-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create ~/.janus directory structure
    New-Item -ItemType Directory -Force -Path "$JanusHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$JanusHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$JanusHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$JanusHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$JanusHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$JanusHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$JanusHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$JanusHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$JanusHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$JanusHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.janus/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.janus/.env"
        }
    } else {
        Write-Info "~/.janus/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$JanusHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.janus/config.yaml from template"
        }
    } else {
        Write-Info "~/.janus/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and Janus's prompt-injection scanner
...
"@
        ...
        Write-Success "Created ~/.janus/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.janus/"
    
    # Seed bundled skills into ~/.janus/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.janus/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.janus/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$JanusHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.janus/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Janus-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'janus setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\janus-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\janus-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\janus-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Janus` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Janus.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_janus_desktop command
    # resolves apps/desktop/release/win-unpacked/Janus.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Janus-managed
    ...
    # The --dir mode produces an unpacked Janus.exe in
    ...
    # are NOT trying to work around). The Janus icon + product name are
    # stamped onto Janus.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\janus-desktop-build-$(Get-Random).log"
    ...
    try {
        ...
        if ($code -ne 0) {
            ...
            # ENOENT on the final `electron` -> `Janus` rename: app-builder's
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Janus.exe",
        "$desktopDir\release\win-arm64-unpacked\Janus.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Janus.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Janus icon + identity are stamped onto Janus.exe by the
    ...
    #    Janus.exe. We deliberately do NOT point them at `janus desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Janus.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Janus.lnk')
        )
        ...
        # with the Janus icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.janus/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'janus setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m janus_cli.main setup
    } else {
        python -m janus_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $janusCmd = "$InstallDir\venv\Scripts\janus.exe"
    if (-not (Test-Path $janusCmd)) {
        $janusCmd = "janus"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$JanusHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'janus whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: janus gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$JanusHome\logs\gateway.log"
            Start-Process -FilePath $janusCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$JanusHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: janus gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: janus gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$JanusHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$JanusHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$JanusHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$JanusHome\janus-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   janus              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   janus setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   janus config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   janus config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   janus gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   janus update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Janus repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Janus-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Janus to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://github.com/magnetoid/janus' -OutFile install.ps1" -ForegroundColor Yellow
