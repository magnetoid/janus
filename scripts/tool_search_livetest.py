#!/usr/bin/env python3
"""Live test harness for Janus Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any janus imports.
ORIGINAL_HOME = os.environ.get("JANUS_HOME")
ORIGINAL_AUTH = Path.home() / ".janus" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.janus/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.janus/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="janus_ts_live_"))
    janus_home = home_dir / ".janus"
    janus_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, janus_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".janus" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, janus_home / ".env")
        ...
        load_janus_dotenv(janus_home=str(Path.home() / ".janus"))
    ...
    (janus_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return janus_home
...
def reset_module_state():
    """Drop cached modules so the new JANUS_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "janus_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["JANUS_HOME"] = str(home)
...
def main():
    ...
    # Restore original JANUS_HOME
    if ORIGINAL_HOME is not None:
        os.environ["JANUS_HOME"] = ORIGINAL_HOME
    else:
