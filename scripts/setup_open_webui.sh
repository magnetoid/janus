...
# Bootstrap Open WebUI against Janus Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.janus/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-janus.sh
...
#   JANUS_API_PORT=8642
#   JANUS_API_HOST=127.0.0.1
#   JANUS_API_MODEL_NAME='Janus Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Janus Agent WebUI}"
...
JANUS_ENV_FILE="${JANUS_ENV_FILE:-$HOME/.janus/.env}"
JANUS_API_PORT="${JANUS_API_PORT:-8642}"
JANUS_API_HOST="${JANUS_API_HOST:-127.0.0.1}"
JANUS_API_CONNECT_HOST="${JANUS_API_CONNECT_HOST:-127.0.0.1}"
JANUS_API_MODEL_NAME="${JANUS_API_MODEL_NAME:-Janus Agent}"
JANUS_API_BASE_URL="http://${JANUS_API_CONNECT_HOST}:${JANUS_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-janus.sh"
LOG_DIR="$HOME/.janus/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$JANUS_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$JANUS_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.janus.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.janus</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.janus"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.janus"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-janus.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Janus Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-janus.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.janus/logs/openwebui.log
StandardError=append:%h/.janus/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-janus.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$JANUS_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Janus API server is configured...'
  upsert_env API_SERVER_ENABLED true "$JANUS_ENV_FILE"
  upsert_env API_SERVER_HOST "$JANUS_API_HOST" "$JANUS_ENV_FILE"
  upsert_env API_SERVER_PORT "$JANUS_API_PORT" "$JANUS_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$JANUS_API_MODEL_NAME" "$JANUS_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$JANUS_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Janus gateway so API server settings take effect...'
  janus gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${JANUS_API_CONNECT_HOST}:${JANUS_API_PORT}/health" >/dev/null; then
    log 'Janus API server did not answer on the first check. Trying to start gateway in the background...'
    nohup janus gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${JANUS_API_CONNECT_HOST}:${JANUS_API_PORT}/health" >/dev/null
  ...
  log "Janus API endpoint: ${JANUS_API_BASE_URL}"
