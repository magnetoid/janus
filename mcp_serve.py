"""
Janus MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Janus-specific extra)

Usage:
    janus mcp serve
    janus mcp serve --verbose
...
logger = logging.getLogger("janus.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using JANUS_HOME."""
    try:
        from janus_constants import get_janus_home
        return get_janus_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("JANUS_HOME", Path.home() / ".janus")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from janus_constants import get_janus_home
        directory_file = get_janus_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("JANUS_HOME", Path.home() / ".janus")
...
class EventBridge:
    ...
    This is the Janus equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from janus_constants import get_janus_home
            db_file = get_janus_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("JANUS_HOME", Path.home() / ".janus")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Janus MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "janus",
        instructions=(
            "Janus Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Janus MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
