...
This is **Janus Agent** by Cloud Industry — a self-improving AI agent with a learning loop. The directory name (`Janus`) is incidental; the package is `janus-agent`. Note: **this checkout is not a git repository.**
...
- **Slash commands:** add a `CommandDef` to `COMMAND_REGISTRY` in `janus_cli/commands.py` (everything downstream — CLI/gateway/Telegram/Slack/autocomplete — derives from it), then handlers in `cli.py` and `gateway/run.py`.
