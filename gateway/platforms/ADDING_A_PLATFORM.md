...
There are two ways to add a platform to the Janus gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.janus/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Janus code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `janus gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `janus_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Janus core.
...
And add it to the `janus-gateway` composite:

```python
"janus-gateway": {
    "includes": [..., "janus-your-platform"]
}
```
...
## 12. Status Display (`janus_cli/status.py`)
...
## 13. Gateway Setup Wizard (`janus_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ janus_cli/ toolsets.py \
  --include="*.py" -l | sort -u
