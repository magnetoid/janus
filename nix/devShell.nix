...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      janusNpmLib = self'.packages.default.passthru.janusNpmLib;
      ...
      # Non-npm packages may have their own devShellHook (e.g. janus-agent
    ...
    in
    {
      devShells.default = pkgs.mkShell {
        ...
        shellHook = ''
          echo "Janus Agent dev shell"
          ${combinedNonNpm}
          ${janusNpmLib.mkNpmDevShellHook npmPackageJsonPaths fixLockfilesExe}
          echo "Ready. Run 'janus' to start."
        '';
      };
    };
}
