...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      janus-agent = self'.packages.default;
      janusVenv = janus-agent.janusVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "janus-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${janusVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "janus-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "janus-build-package" { } ''
          echo "PASS: package built at ${janus-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "janus-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "janus-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${janus-agent}/bin/janus || (echo "FAIL: janus binary missing"; exit 1)
          test -x ${janus-agent}/bin/janus-agent || (echo "FAIL: janus-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${janus-agent}/bin/janus version 2>&1 | grep -qi "janus" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "janus-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in janus janus-agent janus-acp; do
            test -x ${janus-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "janus-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking janus --help ==="
          ${janus-agent}/bin/janus --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${janus-agent}/bin/janus --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "janus-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${janus-agent}/share/janus-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${janus-agent}/share/janus-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "JANUS_BUNDLED_SKILLS" ${janus-agent}/bin/janus || \
            (echo "FAIL: JANUS_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: JANUS_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "janus-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${janus-agent}/share/janus-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${janus-agent}/share/janus-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "JANUS_BUNDLED_PLUGINS" ${janus-agent}/bin/janus || \
            (echo "FAIL: JANUS_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: JANUS_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "janus-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${janus-agent}/share/janus-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${janus-agent}/share/janus-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${janus-agent}/share/janus-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "JANUS_BUNDLED_LOCALES" ${janus-agent}/bin/janus || \
            (echo "FAIL: JANUS_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: JANUS_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (JANUS_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && JANUS_BUNDLED_LOCALES=${janus-agent}/share/janus-agent/locales \
            ${janusVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with JANUS_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${janusVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${janusVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without JANUS_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "janus-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${janus-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${janus-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "JANUS_TUI_DIR" ${janus-agent}/bin/janus || \
            (echo "FAIL: JANUS_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: JANUS_TUI_DIR set in wrapper"
        ...
        # Verify JANUS_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        janus-node = pkgs.runCommand "janus-node-version" { } ''
          set -e
          echo "=== Checking JANUS_NODE in wrapper ==="
          grep -q "JANUS_NODE" ${janus-agent}/bin/janus || \
            (echo "FAIL: JANUS_NODE not set in wrapper"; exit 1)
          echo "PASS: JANUS_NODE present in wrapper"

          JANUS_NODE=$(sed -n "s/^export JANUS_NODE='\(.*\)'/\1/p" ${janus-agent}/bin/janus)
          test -x "$JANUS_NODE" || (echo "FAIL: JANUS_NODE=$JANUS_NODE not executable"; exit 1)
          echo "PASS: JANUS_NODE executable at $JANUS_NODE"

          NODE_MAJOR=$("$JANUS_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All JANUS_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify JANUS_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "janus-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(JANUS_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking JANUS_MANAGED guards ==="
          check_blocked "config set" ${janus-agent}/bin/janus config set model foo
          check_blocked "config edit" ${janus-agent}/bin/janus config edit
        ...
        extra-python-packages = let
          ...
          janusWithExtra = janus-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "janus-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${janusWithExtra}/bin/janus || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${janusWithExtra}/bin/janus || \
          ...
          if grep -q "PYTHONPATH" ${janus-agent}/bin/janus; then
        ...
        extra-dependency-groups = let
          janusWithGroups = janus-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "janus-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${janusWithGroups}"
          echo "venv: ${janusWithGroups.janusVenv}"
        ...
        messaging-variant = pkgs.runCommand "janus-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.janusVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "janus-config-roundtrip" {
          ...
          merge_and_load() {
            ...
            ${configMergeScript} ${nixSettings} "$janus_home/config.yaml"
            ${janusVenv}/bin/python3 -c '
import json, sys
