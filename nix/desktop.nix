# nix/desktop.nix — Janus Desktop (Electron) app build + wrapper
#
# `janusAgent` is the fully-built `.#default` package — it ships the
# `janus` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `JANUS_DESKTOP_JANUS` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Janus CLI").
# No reimplementation of the agent resolution in this wrapper.
{ pkgs, lib, stdenv, makeWrapper, janusNpmLib, electron, janusAgent, ... }:
let
  npm = janusNpmLib.mkNpmPassthru { folder = "apps/desktop"; attr = "desktop"; pname = "janus-desktop"; };
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/janus-desktop $out/bin
    cp -r ${renderer}/* $out/share/janus-desktop/

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # JANUS_DESKTOP_JANUS to the absolute path of the nix-built `janus`
    # binary so the desktop's resolver step 4 ("existing Janus CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/janus-desktop \
      --add-flags "$out/share/janus-desktop" \
      --set JANUS_DESKTOP_JANUS "${lib.getExe janusAgent}" \
