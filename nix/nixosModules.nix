# nix/nixosModules.nix — NixOS module for janus-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: janus runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $JANUS_HOME/.env
# and read by janus at startup — no container recreation needed for env changes.
#
# Tool resolution: the janus wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.janus-agent;
    effectivePackage =
    ...
    janus-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-janus-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "janus-config-attrs";
      description = "Janus YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "janus-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "janus-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'JANUS_DOC_EOF'\n${value}\nJANUS_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "janus-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/janus";
    ...
    # janus user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "janus-container-entrypoint" ''
      set -eu

      JANUS_UID="''${JANUS_UID:?JANUS_UID must be set}"
      JANUS_GID="''${JANUS_GID:?JANUS_GID must be set}"

      # ── Group: ensure a group with GID=$JANUS_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$JANUS_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="janus"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$JANUS_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$JANUS_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$JANUS_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$JANUS_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="janus"
        TARGET_HOME="/home/janus"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$JANUS_UID" -g "$JANUS_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$JANUS_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$JANUS_UID:$JANUS_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure JANUS_HOME is owned by the target user.
      ...
      if [ -n "''${JANUS_HOME:-}" ] && [ -d "$JANUS_HOME" ]; then
        find "$JANUS_HOME" \! -user "$JANUS_UID" -exec chown "$JANUS_UID:$JANUS_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/janus-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/janus-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/janus ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/janus
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$JANUS_UID" --regid="$JANUS_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $JANUS_HOME/.env.
    ...
    # Default: /var/lib/janus/workspace → /data/workspace.
  ...
  in {
    options.services.janus-agent = with lib; {
      enable = mkEnableOption "Janus Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = janus-agent;
        description = "The janus-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/janus";
        description = "State directory. Contains .janus/ subdir (JANUS_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Janus config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $JANUS_HOME/.env at activation time.
          Janus reads this file on every startup via load_janus_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $JANUS_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $JANUS_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `janus gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the janus user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the janus plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Janus discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          janus_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as janus (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in janus-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the janus CLI to environment.systemPackages and export
          JANUS_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.janus symlink to the service
            stateDir. These users are automatically added to the janus group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.janus-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the janus CLI to system PATH and export JANUS_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.janus/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.JANUS_HOME = "${cfg.stateDir}/.janus";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.janus-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the janus user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.janus-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed janus
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure janus is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.janus        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.janus/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.janus/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.janus/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.janus/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.janus/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."janus-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.janus
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.janus ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.janus ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.janus -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.janus/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.janus/$_subdir"
            chmod 2770 "${cfg.stateDir}/.janus/$_subdir"
            find "${cfg.stateDir}/.janus/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.janus/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.janus/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.janus/config.yaml
            chmod 0640 ${cfg.stateDir}/.janus/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.janus/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.janus/.managed
          chmod 0644 ${cfg.stateDir}/.janus/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.janus/.container-mode <<'JANUS_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    janus_bin=${containerDataDir}/current-package/bin/janus
    JANUS_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.janus/.container-mode
            chmod 0644 ${cfg.stateDir}/.janus/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.janus/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.janus";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.janus" ]; then
                  rm -f "${symlinkPath}"
                  echo "janus-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.janus -> stateDir/.janus for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.janus";
                target = "${cfg.stateDir}/.janus";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "janus-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.janus/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.janus/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.janus/auth.json
              fi
            ''}
          ''}
          ...
          # Janus reads $JANUS_HOME/.env at startup via load_janus_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.janus/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'JANUS_NIX_ENV_EOF'
    ${envFileContent}
    JANUS_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.janus/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.janus/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.janus/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.janus-agent = {
          description = "Janus Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $JANUS_HOME/.env by the activation script. load_janus_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/janus"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the janus group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.janus-agent = {
          description = "Janus Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              JANUS_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              JANUS_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env JANUS_UID="$JANUS_UID" \
                --env JANUS_GID="$JANUS_GID" \
                --env JANUS_HOME=${containerDataDir}/.janus \
                --env JANUS_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/janus gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
