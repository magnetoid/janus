...
## 5. Janus Configuration

### Config location

`$JANUS_HOME/config.yaml` (defaults to `~/.janus/config.yaml` when `JANUS_HOME` is unset)
...
Restart the Janus session for changes to take effect. The MCP connection is
