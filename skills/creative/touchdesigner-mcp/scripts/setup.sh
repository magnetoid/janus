...
JANUS_HOME_DIR="${JANUS_HOME:-$HOME/.janus}"
JANUS_CFG="${JANUS_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Janus config has twozero_td MCP entry ──
if [[ ! -f "$JANUS_CFG" ]]; then
    echo -e " ${FAIL} Janus config not found at ${JANUS_CFG}"
    manual_steps+=("Create ${JANUS_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$JANUS_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Janus config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Janus config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${JANUS_CFG} manually"); }
    manual_steps+=("Restart Janus session to pick up config change")
fi
