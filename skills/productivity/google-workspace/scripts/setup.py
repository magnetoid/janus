#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Janus Agent.
...
# Ensure sibling modules (_janus_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _janus_home import display_janus_home, get_janus_home

JANUS_HOME = get_janus_home()
TOKEN_PATH = JANUS_HOME / "google_token.json"
CLIENT_SECRET_PATH = JANUS_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = JANUS_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Janus profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Janus Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Janus Docker image's "
        "uv-managed venv), install the optional extra instead:"
    )
    print("  pip install 'janus-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Janus home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_janus_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Janus")
