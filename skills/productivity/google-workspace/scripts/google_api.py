#!/usr/bin/env python3
"""Google Workspace API CLI for Janus Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Janus-facing JSON contract and falls back to the Python client
...
from _janus_home import get_janus_home
...
def _gws_binary() -> str | None:
    override = os.getenv("JANUS_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Janus Agent")
