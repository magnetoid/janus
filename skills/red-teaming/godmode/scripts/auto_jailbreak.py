...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("JANUS_HOME", os.path.expanduser("~/.janus")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("JANUS_HOME", Path.home() / ".janus")) / "skills" / "red-teaming" / "godmode"
...
JANUS_HOME = Path(os.getenv("JANUS_HOME", Path.home() / ".janus"))
CONFIG_PATH = JANUS_HOME / "config.yaml"
PREFILL_PATH = JANUS_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Janus models — already uncensored, just needs clean prompt
    "janus": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "janus" in model_lower or "nous" in model_lower:
        return "janus"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Janus config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.janus/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Janus for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Janus for changes to take effect.")
