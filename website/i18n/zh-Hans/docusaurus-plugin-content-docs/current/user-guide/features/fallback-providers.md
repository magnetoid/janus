...
| Janus Portal | `nous` | `janus setup --portal`（全新安装）或 `janus auth add nous`（OAuth） |
...
**以 Janus Portal 作为 OpenRouter 的备用：**
```yaml
model:
  provider: openrouter
...
```text
OpenRouter → Janus Portal → 自定义端点 → Codex OAuth →
...
```text
主提供商（若支持视觉）→ OpenRouter → Janus Portal →
...
| `"nous"` | 强制使用 Janus Portal | `janus auth` |
