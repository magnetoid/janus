...
| 提供商 | 配置方式 |
|----------|-------|
| **Janus Portal** | `janus model`（OAuth，订阅制） |
...
[Janus Portal](https://portal.imbalabs.com) 是 Cloud Industry 的统一订阅网关，也是**运行 Janus Agent 的推荐方式**。一次 OAuth 登录即可访问 300+ 前沿智能体模型（Claude、GPT、Gemini、DeepSeek、Qwen、Kimi、GLM、MiniMax、Grok 等），以及 [Tool Gateway](/user-guide/features/tool-gateway)（网页搜索、图像生成、TTS、浏览器自动化）和 [Nous Chat](https://chat.imbalabs.com)——费用从你的 Nous 订阅中扣除，无需单独管理各提供商账户。

```bash
janus setup --portal     # 全新安装——一条命令完成 OAuth + 提供商 + 网关配置
janus model              # 已有安装——从列表中选择"Janus Portal"
janus portal info        # 随时查看登录状态和路由信息
```

还没有订阅？前往 [portal.imbalabs.com/manage-subscription](https://portal.imbalabs.com/manage-subscription) 购买。

**完整详情：** 参见专属的 [Janus Portal 集成页面](/integrations/nous-portal)（订阅内容、模型目录、故障排查）以及分步指南[使用 Janus Portal 运行 Janus Agent](/guides/run-janus-with-nous-portal)。
...
:::warning
即使使用 Janus Portal、Codex 或自定义端点，某些工具（视觉、网页摘要、MoA）仍会使用单独的"辅助"模型。默认情况下（`auxiliary.*.provider: "auto"`），Janus 将这些任务路由到你的**主聊天模型**——即你在 `janus model` 中选择的同一模型。你可以单独覆盖每个任务，将其路由到更便宜/更快的模型（例如 OpenRouter 上的 Gemini Flash）——参见[辅助模型](/user-guide/configuration#auxiliary-models)。
:::

:::tip Nous Tool Gateway
付费 Janus Portal 订阅者还可访问 **[Tool Gateway](/user-guide/features/tool-gateway)**——网页搜索、图像生成、TTS 和浏览器自动化，均通过你的订阅路由。无需额外 API key。全新安装时，`janus setup --portal` 一条命令即可完成登录、设置 Nous 为提供商并开启网关。现有用户可通过 `janus model` 或 `janus tools` 按工具启用。随时使用 `janus portal info` 查看路由状态。
:::
...
7. **Janus Portal** — 将 Nous 模型 ID 后缀匹配到 OpenRouter 元数据
...
| 使用场景 | 推荐方案 |
|----------|-------------|
| **只想让它工作** | OpenRouter（默认）或 Janus Portal |
| **本地模型，简单配置** | Ollama |
