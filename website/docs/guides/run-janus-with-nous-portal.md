...
OAuth needs a browser, but the loopback callback runs on the machine where Janus is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
janus setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: manual paste (for Cloud Shell, Codespaces, EC2 Instance Connect)
janus auth add nous --type oauth --manual-paste
# Then re-run `janus setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "Janus Agent release notes" and summarize the top 3 hits.
```

You should see Janus call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

`janus setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
janus config set model.default anthropic/claude-sonnet-4.6
```
...
The Portal's own [info page](https://portal.nousresearch.com/info) carries this warning too — it's the official Nous guidance, not just a Janus-side opinion.
...
These rows appear in `janus tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, Janus runs the Portal login inline (without changing your inference provider or your other tools).
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Janus will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
```bash
janus cron create "every day at 9am" \
...
If you use [Janus profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.janus/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `janus portal info` shows "not logged in" after `janus setup --portal`
...
```bash
janus config set model.provider nous
```
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Janus doesn't replay it endlessly. Just log in again:
...
If a model is genuinely unavailable, [open an issue](https://github.com/magnetoid/janus/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`janus portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Janus profiles where you're using the wrong one (check `janus profile current`)

### Want to revoke and start clean

```bash
janus auth remove nous       # wipes the local refresh token
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple Janus configurations
