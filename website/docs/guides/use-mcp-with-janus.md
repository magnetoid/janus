---
sidebar_position: 6
title: "Use MCP with Janus"
description: "A practical guide to connecting MCP servers to Janus Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Janus

This guide shows how to actually use MCP with Janus Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Janus tool
- you want Janus to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Janus to internal APIs, databases, or company systems without modifying Janus core

Do not use MCP when:
- a built-in Janus tool already solves the job well
...
- Janus remains the agent
- MCP servers contribute tools
- Janus discovers those tools at startup or reload time
...
If you installed Janus with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Janus banner/status should show MCP integration when configured
- ask Janus what tools it has available
...
## WSL2: bridge Janus in WSL to Windows Chrome

This is the practical setup when:

- Janus runs inside WSL2
...
In this setup, Janus does **not** connect to Chrome directly. Instead:

- Janus runs in WSL
- Janus starts a local stdio MCP server
...
```text
Janus (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Janus stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Janus core browser transport
...
```bash
janus mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
janus mcp test chrome-devtools-win
```

Then start a fresh Janus session or run:
...
Once loaded, Janus can use the MCP-prefixed browser tools directly. For example:
...
If Janus runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Janus from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Janus from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Janus:
...
2. Janus-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Janus will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Janus to reason over a bounded workspace.
...
Now Janus can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Janus core.
...
Because Janus now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
