...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `janus send`."
---

# Pipe Script Output to Messaging Platforms

`janus send` is a small, scriptable CLI that pushes a message to any
messaging platform Janus is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | janus send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `janus
...
janus send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
janus send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
janus send --list

# Filter by platform
janus send --list telegram
```
...
Any platform Janus ships adapters for works as a target:
...
`janus send` resolves the message body in this order:

1. **Positional argument** — `janus send --to telegram "hi"`
2. **`--file PATH`** — `janus send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | janus send --to telegram`

When stdin is a TTY (no pipe), Janus does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  janus send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `janus send` reuses your Janus config, the same script works on
any host where Janus is installed — no need to export bot tokens into
...
`janus send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  janus send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | janus send \
...
```bash
./train.py --epochs 200 && \
  janus send --to telegram "training done" || \
  janus send --to telegram "training failed (exit $?)"
```
...
janus send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(janus send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `janus send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `janus send` calls
the platform's REST endpoint directly using credentials from
`~/.janus/.env` and `~/.janus/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `janus gateway start` and retry.
...
# Machine-readable
janus send --list --json
```

The listing is built from `~/.janus/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`janus gateway
...
| Approach | Multi-platform | Reuses Janus creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `janus send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`janus send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `janus send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `janus send` shares with cron delivery.
