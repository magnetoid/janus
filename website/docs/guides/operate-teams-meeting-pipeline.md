...
```bash
janus teams-pipeline token-health
janus teams-pipeline token-health --force-refresh
```
...
```bash
janus teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
janus teams-pipeline maintain-subscriptions
janus teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Janus cron (recommended if you already run the Janus gateway)

Janus ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.janus/scripts/`. First create the script:

```bash
mkdir -p ~/.janus/scripts
cat > ~/.janus/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec janus teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.janus/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
janus cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/janus-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Janus Teams pipeline subscription maintenance
...
ExecStart=/usr/local/bin/janus teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/janus-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Janus Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now janus-teams-pipeline-maintain.timer
systemctl list-timers janus-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/janus teams-pipeline maintain-subscriptions >> /var/log/janus/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.janus/.env` at the top of a wrapper script that crontab calls.
...
```bash
janus teams-pipeline subscriptions   # should show expirationDateTime advanced
janus teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
janus teams-pipeline list
janus teams-pipeline list --status failed
janus teams-pipeline show <job-id>
```

### Replay a stored job

```bash
janus teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
janus teams-pipeline fetch --meeting-id <meeting-id>
janus teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `janus teams-pipeline maintain-subscriptions --dry-run`
- inspect `janus teams-pipeline list --status failed`
...
- run `janus teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `janus teams-pipeline run`
...
- [ ] `janus teams-pipeline validate` returns an OK snapshot
- [ ] `janus teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Janus cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
