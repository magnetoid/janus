---
sidebar_position: 7
title: "Use SOUL.md with Janus"
description: "How to use SOUL.md to shape Janus Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Janus

`SOUL.md` is the **primary identity** for your Janus instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Janus to feel like the same assistant every time you talk to it — or if you want to replace the Janus persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Janus should be
- what Janus should avoid stylistically
- how Janus should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Janus is and how Janus speaks
...
Janus now uses only the global SOUL file for the current instance:

```text
~/.janus/SOUL.md
```

If you run Janus with a custom home directory, it becomes:
...
Janus automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Janus does not overwrite it
- if the file exists but is empty, Janus adds nothing from it to the prompt

## How Janus uses it

When Janus starts a session, it reads `SOUL.md` from `JANUS_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Janus feels.
...
Janus already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Janus should sound.

# Avoid
What Janus should not do.

# Defaults
How Janus should behave when ambiguity appears.
```
...
Then restart Janus or start a new session.
...
### I edited SOUL.md but Janus still sounds the same

Check:
- you edited `~/.janus/SOUL.md` or `$JANUS_HOME/SOUL.md`
...
### Janus is ignoring parts of my SOUL.md
