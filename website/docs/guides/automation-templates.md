...
Copy-paste recipes for common automation patterns. Each template uses Janus's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`janus webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `janus webhook subscribe`) |
...
```bash
janus cron create "0 2 * * *" \
  "You are a project manager triaging the magnetoid/janus GitHub repo.

1. Run: gh issue list --repo magnetoid/janus --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
janus webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
janus cron create "0 9 * * 1" \
  "Scan the magnetoid/janus repo for documentation drift.

1. Run: gh pr list --repo magnetoid/janus --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (janus_cli/commands.py, janus_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (janus_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
janus cron create "0 6 * * *" \
  "Run a dependency security audit on the janus-agent project.

1. cd ~/.janus/janus-agent && source .venv/bin/activate
...
```bash
janus webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
janus webhook subscribe alert-triage \
...
```python title="~/.janus/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Janus-Monitor/1.0"})
...
janus cron create "every 30m" \
  ...
  --script ~/.janus/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
janus cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Janus Agent development." \
...
```bash
janus webhook subscribe github-issues \
  --events "issues" \
...
```bash
janus webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
janus webhook subscribe stripe-payments \
...
```bash
janus cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the janus-agent codebase.
