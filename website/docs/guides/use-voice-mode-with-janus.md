---
sidebar_position: 8
title: "Use Voice Mode with Janus"
description: "A practical guide to setting up and using Janus voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Janus
...
:::tip
[Cloud Industry Portal](/integrations/nous-portal) bundles both the LLM and TTS through one OAuth — voice mode works end-to-end with no extra credentials.
:::
...
- you want Janus sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Janus.
...
## Step 1: make sure normal Janus works first
...
Janus supports both local and cloud speech stacks.
...
### If you use `janus setup`

If you choose NeuTTS in the setup wizard, Janus checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Janus transcribes and responds
...
- asking Janus to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Janus loop.

## Tuning CLI behavior

### Silence threshold

If Janus starts/stops too aggressively, tune:
...
Janus stays a normal chat bot, but can speak replies.
...
- you want Janus to function like a portable research or ops assistant
...
Janus joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Janus detects speech boundaries
- transcripts are posted in the associated text channel
- Janus responds in text and audio
...
1. get text Janus working
2. install `janus-agent[voice]`
