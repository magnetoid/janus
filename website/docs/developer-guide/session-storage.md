...
Janus Agent uses a SQLite database (`~/.janus/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `janus_state.py`


## Architecture Overview

```
~/.janus/state.db (SQLite, WAL mode)
...
Multiple janus processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.janus/state.db
...
Default path: `~/.janus/state.db`

This is derived from `janus_constants.get_janus_home()` which resolves to
`~/.janus/` by default, or the value of `JANUS_HOME` environment variable.
