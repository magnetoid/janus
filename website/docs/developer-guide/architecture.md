...
sidebar_position: 1
title: "Architecture"
description: "Janus Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Janus Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ janus_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # JanusCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── janus_state.py           # SQLite session/state database with FTS5
├── janus_constants.py       # JANUS_HOME, profile-aware paths
...
│
├── janus_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `janus` subcommands (large file)
...
│   ├── skills_config.py      # janus skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # janus tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # janus gateway start/stop
│
...
```text
User input → JanusCLI.process_input()
...
Three discovery sources: `~/.janus/plugins/` (user), `.janus/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `janus plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-janus-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Janus as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`janus -p <name>`) gets its own JANUS_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
