...
description: "How Janus stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `janus_cli/cron.py` | CLI `janus cron` subcommands |
...
Jobs are stored in `~/.janus/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `janus cron` commands are run or during active CLI sessions.
...
```python
# ~/.janus/scripts/check_competitors.py
...
2. **Environment variable** — `JANUS_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.janus/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `janus cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `janus cron` CLI provides direct job management:

```bash
janus cron list                    # Show all jobs
janus cron create                  # Interactive job creation (alias: add)
janus cron edit <job_id>           # Edit job configuration
janus cron pause <job_id>          # Pause a running job
janus cron resume <job_id>         # Resume a paused job
janus cron run <job_id>            # Trigger immediate execution
janus cron remove <job_id>         # Delete a job
```
