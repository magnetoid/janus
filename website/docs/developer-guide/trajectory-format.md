...
Janus Agent saves conversation trajectories in ShareGPT-compatible JSONL format
...
ALL possible tools (from the live tool registry, via
`registry.get_tool_to_toolset_map()`) with zero defaults, ensuring consistent
schema across entries for HuggingFace dataset loading.
...
It follows the Janus function-calling prompt template with:
