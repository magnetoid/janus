...
The ACP adapter wraps Janus' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
janus acp / janus-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.janus/.env
  -> configure stderr logging
  -> construct JanusACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'janus-agent[acp]==<version>' janus-acp`, pointed at the `janus-agent` PyPI release.
...
- `allow_once` -> Janus `once`
- `allow_always` -> Janus `always`
- reject options -> Janus `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Janus tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["janus-acp"])
...
Instead it reuses Janus' runtime resolver:

- `acp_adapter/auth.py`
- `janus_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Janus provider/credentials. It also always advertises a terminal setup auth method (`janus-setup`, args `--setup`) so first-run registry clients can open Janus' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.janus/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `janus-acp` toolset definition
- `janus_cli/main.py` — `janus acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `janus-acp` script
