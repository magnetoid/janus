...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Janus Agent terminal interface — commands, keybindings, personalities, and more"
---
...
One command — `janus setup --portal` — and you're ready to `janus chat`. See [Cloud Industry Portal](/integrations/nous-portal).
:::

:::tip
Janus also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `janus --tui` — see the [TUI](tui.md) guide.
:::
...
janus chat --provider nous        # Use Cloud Industry Portal
...
janus -s janus-agent-dev,github-auth
janus chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
janus --continue             # Resume the most recent CLI session (-c)
janus --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
janus chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
janus -w                         # Interactive mode in worktree
janus -w -z "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Janus CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Janus CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `JANUS_YOLO_MODE` is on (either `janus --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
When resuming a previous session (`janus -c` or `janus --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Janus to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Janus supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Janus replies |
...
```bash
janus -s janus-agent-dev,github-auth
janus chat -s github-pr-workflow -s github-auth
```

Janus loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.janus/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.janus/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Janus..."
  # Add your own!
```
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Janus — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while Janus is working, Janus prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Janus to the background — just like any terminal process. The shell prints a confirmation:

```
Janus Agent has been suspended. Run `fg` to bring Janus Agent back.
```
...
Resume this session with:
  janus --resume 20260225_143052_a1b2c3
...
```bash
janus --continue                          # Resume the most recent CLI session
janus -c                                  # Short form
janus -c "my project"                     # Resume a named session (latest in lineage)
janus --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
janus --resume "refactoring auth"         # Resume by title
janus -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `janus sessions rename <id> <title>` from the command line. Use `janus sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Janus's SQLite state database under `~/.janus/state.db`. The database keeps:
...
Janus immediately confirms the task and gives you back the prompt:
