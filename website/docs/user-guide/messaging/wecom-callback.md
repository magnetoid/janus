...
Connect Janus to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Janus supports two WeCom integration modes:
...
> Run `janus gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Janus decrypts the message, queues it for the agent
...
(Use `janus gateway start` only after `janus gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Janus and the token the
...
in `~/.janus/.env` values around `=` will also break signature checks. After
fixing, restart `janus gateway run`.
...
Check `janus gateway run` logs for the bound host/port. If the adapter bound to
