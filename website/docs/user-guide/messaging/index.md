...
description: "Chat with Janus from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Janus from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Janus](/guides/use-voice-mode-with-janus).

:::tip
Bots need both a model provider and tool providers (TTS, web). A [Cloud Industry Portal](/integrations/nous-portal) subscription bundles all of them.
:::
...
```bash
janus gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
janus gateway              # Run in foreground
janus gateway setup        # Configure messaging platforms interactively
janus gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo janus gateway install --system   # Linux only: install a boot-time system service
janus gateway start        # Start the default service
janus gateway stop         # Stop the default service
janus gateway status       # Check default service status
janus gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Janus Agent to the latest version |
...
Configure per-platform overrides in `~/.janus/gateway.json`:
...
janus pairing approve telegram XKGH5N7P

# Other pairing commands:
janus pairing list          # View pending + approved users
janus pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Janus appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.janus/config.yaml`:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.janus/config.yaml`:
...
```bash
JANUS_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
janus gateway install               # Install as user service
janus gateway start                 # Start the service
janus gateway stop                  # Stop the service
janus gateway status                # Check status
journalctl --user -u janus-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo janus gateway install --system
sudo janus gateway start --system
sudo janus gateway status --system
journalctl -u janus-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Janus will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Janus installations on the same machine (with different `JANUS_HOME` directories), each gets its own systemd service name. The default `~/.janus` uses `janus-gateway`; other installations use `janus-gateway-<hash>`. The `janus gateway` commands automatically target the correct service for your current `JANUS_HOME`.
:::

### macOS (launchd)

```bash
janus gateway install               # Install as launchd agent
janus gateway start                 # Start the service
janus gateway stop                  # Stop the service
janus gateway status                # Check status
tail -f ~/.janus/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.janus.gateway.plist`. It includes three environment variables:
...
- **JANUS_HOME** — scopes the gateway to your Janus installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `janus gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `JANUS_HOME` directory gets its own launchd label. The default `~/.janus` uses `ai.janus.gateway`; other installations use `ai.janus.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `janus-cli` | Full access |
| Telegram | `janus-telegram` | Full tools including terminal |
| Discord | `janus-discord` | Full tools including terminal |
| WhatsApp | `janus-whatsapp` | Full tools including terminal |
| Slack | `janus-slack` | Full tools including terminal |
| Google Chat | `janus-google_chat` | Full tools including terminal |
| Signal | `janus-signal` | Full tools including terminal |
| SMS | `janus-sms` | Full tools including terminal |
| Email | `janus-email` | Full tools including terminal |
| Home Assistant | `janus-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `janus-mattermost` | Full tools including terminal |
| Matrix | `janus-matrix` | Full tools including terminal |
| DingTalk | `janus-dingtalk` | Full tools including terminal |
| Feishu/Lark | `janus-feishu` | Full tools including terminal |
| WeCom | `janus-wecom` | Full tools including terminal |
| WeCom Callback | `janus-wecom-callback` | Full tools including terminal |
| Weixin | `janus-weixin` | Full tools including terminal |
| BlueBubbles | `janus-bluebubbles` | Full tools including terminal |
| QQBot | `janus-qqbot` | Full tools including terminal |
| Yuanbao | `janus-yuanbao` | Full tools including terminal |
| Microsoft Teams | `janus-teams` | Full tools including terminal |
| API Server | `janus-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `janus-webhook` | Full tools including terminal |
...
1. **Gateway log** (`~/.janus/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
