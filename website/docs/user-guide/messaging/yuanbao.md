...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Janus Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Janus to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.janus/.env`:
...
All standard Janus commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.janus/logs/gateway.log`
...
4. Consider enabling verbose logging: `JANUS_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Janus automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
JANUS_LOG_LEVEL=debug janus gateway
```
...
```bash
janus chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
