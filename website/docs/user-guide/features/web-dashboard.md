...
The web dashboard is a browser-based UI for managing your Janus Agent installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.

:::tip
Hosted-mode auth uses Cloud Industry Portal OAuth; if you also want the dashboard to talk to a real backend, `janus setup --portal` wires up the model and tool gateway too. See [Cloud Industry Portal](/integrations/nous-portal).
:::
...
```bash
# Custom port
janus dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
janus dashboard --host 0.0.0.0

# Start without opening browser
janus dashboard --no-open
```

## Prerequisites

The default `janus-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
pip install 'janus-agent[web,pty]'
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `pip install janus-agent[all]` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `janus dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `janus dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- Node.js (same requirement as `janus --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`pip install 'janus-agent[web,pty]'`, or `[all]` covers both)
...
```ini
[Service]
EnvironmentFile=%h/.janus/.env
ExecStart=/path/to/venv/bin/python -m janus_cli.main dashboard \
    --host 0.0.0.0 --port 9119 --no-open
```

with `~/.janus/.env` containing:

```bash
JANUS_DASHBOARD_BASIC_AUTH_USERNAME=admin
JANUS_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
JANUS_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `janus config set` and the gateway read from.
:::
...
Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.janus/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `janus skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
block in `config.yaml` that `janus mcp` reads from.
...
`janus mcp catalog` / `janus mcp install` use.
...
Connect Janus to any messaging platform from the browser — full parity with
`janus setup gateway`. The page lists every supported channel (Telegram,
...
- **Restart gateway** — credentials are written to `~/.janus/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page.
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and Janus versions, CPU core count + utilization, memory, disk usage of the Janus home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The Janus version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git or pip install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `janus update` in the background. On Docker/Nix/Homebrew installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **Cloud Industry Portal** — login status, the active inference provider, and the Tool Gateway routing table (which tools run via the Portal vs. locally), with a link to manage your subscription. Read-only mirror of `janus portal`.
- **Skill curator** — the background skill-maintenance status (active / paused, interval, last run) with pause/resume and a run-now button. Mirrors `janus curator`.
...
![System admin page — host stats and Cloud Industry Portal status](/img/dashboard/admin-system-top.png)
...
This re-reads `~/.janus/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
| `GET /api/janus/update/check` | Report update availability (commits behind, install method) without applying. `?force=1` busts the 6h cache |
| `GET /api/curator` · `PUT .../paused` · `POST .../run` | Skill-curator status + pause/resume + run |
| `GET /api/portal` | Cloud Industry Portal auth + Tool Gateway routing (read-only) |
...
When the dashboard is bound to a public or non-loopback address — anything other than `127.0.0.1` / `localhost` — Janus Agent engages an auth gate. Every request must carry a verified session cookie or it's bounced to the login page. Three providers ship in the box:
...
- **[OAuth (Cloud Industry Portal)](#default-provider-nous-research)** — for hosted deployments and any dashboard reachable over the public internet, and the recommended path for a [remote Janus Desktop connection](#connecting-janus-desktop-to-a-remote-backend). Every login is verified against your Nous account, so this is the provider suitable for internet-facing use.
- **[Self-hosted OIDC](#self-hosted-oidc-provider)** — for bringing your own identity provider via standard OpenID Connect (Keycloak, Auth0, Okta, Google, GitHub via an OIDC bridge, etc.). No Cloud Industry Portal involved; suitable for public-internet exposure when fronted by a conformant OIDC server.
...
|-------|-----------|----------|
| `janus dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `janus dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
Because every login is verified against Cloud Industry Portal and protected by your Nous account, **the Nous provider is the one suitable for exposing a dashboard to the public internet.**

#### Registering a dashboard

To use the Nous provider you need an OAuth client ID (shape `agent:{id}`). There are two ways to get one:

- **CLI — `janus dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing Nous login (run `janus setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `JANUS_DASHBOARD_OAUTH_CLIENT_ID` into `~/.janus/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).
...
- **GUI — the Local Dashboards page.** Open [`/local-dashboards`](https://portal.cloud-industry.com/local-dashboards) in the Cloud Industry Portal to register, name, manage, and revoke self-hosted dashboards from the browser. Copy the resulting `agent:{id}` client ID into `JANUS_DASHBOARD_OAUTH_CLIENT_ID` (env) or `dashboard.oauth.client_id` (config.yaml). This is also where you revoke a dashboard registered via the CLI.
...
|---------|-----------|--------|----------------|
| `JANUS_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `janus dashboard register` |

Per the Janus Agent convention (`~/.janus/.env` is for API keys / secrets only), **`config.yaml` is the recommended place to set these values** for local dev, on-prem, and any deployment you control directly. The environment-variable path exists so a hosting platform's secret injection can push per-deploy `client_id`s without anyone having to edit `config.yaml` inside the image — that's its primary purpose.
...
Bundled providers reported these issues:
  • nous: JANUS_DASHBOARD_OAUTH_CLIENT_ID is not set (and
    dashboard.oauth.client_id in config.yaml is empty). The Cloud Industry Portal
    provisions this env var (shape 'agent:{instance_id}') when it
    deploys a Janus Agent instance — set it to your provisioned
...
**1. Log in and register the dashboard.** `janus dashboard register` uses your existing Nous login to provision an OAuth client and writes `JANUS_DASHBOARD_OAUTH_CLIENT_ID` into `~/.janus/.env` for you:

```bash
janus setup            # if you're not already logged into Cloud Industry Portal
janus dashboard register
# ✓ Registered dashboard "swift_falcon"
# …writes JANUS_DASHBOARD_OAUTH_CLIENT_ID to ~/.janus/.env
```
...
`GET /api/auth/me` then returns the verified session (`provider: nous`). For an internet-facing host, register with `--redirect-uri https://janus.example.com/auth/callback` and set `JANUS_DASHBOARD_PUBLIC_URL` so the OAuth callback resolves to your public URL (see [Public URL override](#public-url-override)).
...
|---------|-----------|-------|
| `JANUS_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `JANUS_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `JANUS_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `JANUS_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `JANUS_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
**1. Set credentials in `~/.janus/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
cat >> ~/.janus/.env <<EOF
JANUS_DASHBOARD_BASIC_AUTH_USERNAME=admin
JANUS_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
JANUS_DASHBOARD_BASIC_AUTH_SECRET=$(openssl rand -base64 32)
EOF
chmod 600 ~/.janus/.env
```
...
If you run your own identity provider, the bundled `plugins/dashboard_auth/self_hosted` plugin authenticates the dashboard against it using **standard OpenID Connect** — no per-IDP code, no Cloud Industry Portal involved. It's verified against and works with any conformant OIDC server:
...
dashboard:
  oauth:
    ...
    self_hosted:
      issuer: https://auth.example.com/application/o/janus/   # required
      client_id: janus-dashboard                              # required
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `JANUS_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `JANUS_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `JANUS_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-janus.json` — it defines a `janus` realm, a **public PKCE client** (`janus-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
```json
{
  ...
  "clients": [
    {
      "clientId": "janus-dashboard",
      "name": "Janus Agent Dashboard",
      "enabled": true,
      "publicClient": true,
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-janus.json:/opt/keycloak/data/import/realm-janus.json:ro" \
  quay.io/keycloak/keycloak:26.0 \
  start-dev --import-realm
```

Once it's up, the realm advertises standard OIDC discovery at
`http://localhost:8080/realms/janus/.well-known/openid-configuration` (issuer
`http://localhost:8080/realms/janus`). The admin console is at
...
```bash
export JANUS_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/janus"
export JANUS_DASHBOARD_OIDC_CLIENT_ID="janus-dashboard"
export JANUS_DASHBOARD_PUBLIC_URL="http://localhost:9119"
janus dashboard --host 0.0.0.0 --port 9119 --no-open
```

`JANUS_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
> admin console (Clients → janus-dashboard → Settings). The same pattern works
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `JANUS_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:

```yaml
dashboard:
  public_url: "https://dashboard.example.com/janus"
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `JANUS_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `JANUS_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
The provider implements the [Cloud Industry Portal OAuth contract v1](https://github.com/ImbaLabs/nous-account-service/blob/main/docs/agent-dashboard-oauth-contract.md) — authorization-code grant with PKCE (S256):
...
3. Server stashes PKCE state in a short-lived cookie, redirects user to `https://portal.cloud-industry.com/oauth/authorize?…`.
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `janus_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `janus_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip |
| `janus_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
Every login start, success, failure, and session-verify failure is written as a JSON line to `$JANUS_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.
...
```python
# ~/.janus/plugins/dashboard-auth-myidp/__init__.py
from janus_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
```bash
# Quick env-var path.
JANUS_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  janus dashboard --host 0.0.0.0
...
```bash
# 1. Set the dashboard login credentials in ~/.janus/.env (secrets file, 0600).
cat >> ~/.janus/.env <<'EOF'
JANUS_DASHBOARD_BASIC_AUTH_USERNAME=admin
JANUS_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
janus dashboard --no-open --host 0.0.0.0 --port 9119
```

Prefer no plaintext at rest? Use `JANUS_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` with a scrypt hash instead — see [Username/password provider](#usernamepassword-provider-no-oauth-idp) for the full surface.

If you run the dashboard as a systemd service, `~/.janus/.env` is picked up automatically when the unit has `EnvironmentFile=%h/.janus/.env`, so the credentials are in the environment at boot.

:::warning
The dashboard reads and writes your `.env` (API keys, secrets) and can run agent commands. The **username/password** setup shown here is for a trusted network — never expose a password-protected dashboard directly to the open internet. Put it behind a VPN. [Tailscale](https://tailscale.com/) is the clean option: bind to the machine's tailscale IP (`--host <tailscale-ip>`) and use `http://<tailscale-ip>:9119` as the Remote URL. Only devices on your tailnet can reach it. To reach a backend over the public internet, use the **OAuth (Cloud Industry Portal)** provider instead.
:::
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/janus` are supported if you front it with a reverse proxy)
...
The session refreshes automatically and survives restarts when `JANUS_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `JANUS_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `JANUS_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |

### Troubleshooting

- **"Remote gateway incomplete"** — you haven't entered a remote URL.
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `JANUS_DASHBOARD_BASIC_AUTH_USERNAME` / `JANUS_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for unknown user and wrong password, so check both. Confirm the gate with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
...
- **Signed out on every restart** — set `JANUS_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `janus_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `janus update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `janus dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Janus Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Janus Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
