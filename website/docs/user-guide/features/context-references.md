...
Type `@` followed by a reference to inject content directly into your message. Janus expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Janus env: `$JANUS_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$JANUS_HOME/skills/.hub/`
