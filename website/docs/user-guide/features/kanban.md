...
description: "Durable SQLite-backed task board for coordinating multiple Janus profiles"
---
...
Janus Kanban is a durable task board, shared across all your Janus profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.janus/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.

### Two surfaces: the model talks through tools, you talk through the CLI

The board has two front doors, both backed by the same `~/.janus/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `janus kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `janus kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
For the full design rationale, comparative analysis against Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise, and the eight canonical collaboration patterns, see `docs/janus-kanban-v1-spec.pdf` in the repository.
...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), promotes ready tasks, atomically claims, spawns assigned profiles. Runs **inside the gateway** by default (`kanban.dispatch_in_gateway: true`). One dispatcher sweeps all boards per tick; workers are spawned with `JANUS_KANBAN_BOARD` pinned so they can't see other boards. After `kanban.failure_limit` consecutive spawn failures on the same task (default: 2) the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose profile doesn't exist, workspace can't mount, etc.
...
janus kanban boards list

# Create a new board.
janus kanban boards create atm10-server \
    --name "ATM10 Server" \
...
janus kanban --board atm10-server list
janus kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
janus kanban boards switch atm10-server
janus kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
janus kanban boards rename atm10-server "ATM10 (Prod)"
...
2. `JANUS_KANBAN_BOARD` env var (set by the dispatcher when spawning a
   worker, so workers can't see other boards).
3. `~/.janus/kanban/current` — the slug persisted by `janus kanban
   boards switch`.
4. `default`.
...
`janus dashboard` → Kanban tab shows a board switcher at the top as soon
...
- **Storage** — files land under
  `<janus-home>/kanban/attachments/<task_id>/` for the default board, or
  `<janus-home>/kanban/boards/<slug>/attachments/<task_id>/` for a named
  board. Set `JANUS_KANBAN_ATTACHMENTS_ROOT` to pin a custom location.
...
janus kanban create "research AI funding landscape" --assignee researcher
...
When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `janus kanban show t_abcd`.
...
Override the config flag at runtime via `JANUS_KANBAN_DISPATCH_IN_GATEWAY=0`
for debugging. Standard gateway supervision applies: run `janus gateway
...
until one comes up — `janus kanban create` warns about this at creation
time.

Running `janus kanban daemon` as a separate process is **deprecated**;
...
janus kanban create "nightly ops review" \
    --assignee ops \
...
```bash
janus kanban complete t_abc t_def t_hij --result "batch wrap"
janus kanban archive  t_abc t_def t_hij
janus kanban unblock  t_abc t_def
janus kanban block    t_abc "need input" --ids t_def t_hij
```

## How workers interact with the board

**Workers do not shell out to `janus kanban`.** When the dispatcher spawns a worker it sets `JANUS_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `janus kanban` CLI.
...
kanban_show()                                     # no args — uses JANUS_KANBAN_TASK
...
### Why tools instead of shelling to `janus kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `janus kanban complete` *inside* the container, where `janus` isn't installed and `~/.janus/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.janus/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `janus chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `JANUS_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
```json
{
  ...
  "verification": ["pytest tests/janus_cli/test_kanban_db.py -q"],
...
2. `cd $JANUS_KANBAN_WORKSPACE` (via the terminal tool) and do the work there.
...
```bash
janus -p <your-worker-profile> skills list | grep kanban-worker
```

If the bundled copy is missing, restore it for that profile:

```bash
janus -p <your-worker-profile> skills reset kanban-worker --restore
```
...
```bash
janus kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

janus kanban create "audit auth flow" \
    --assignee reviewer \
...
```bash
janus kanban create "Translate the docs site to French" \
...
```bash
janus -p orchestrator skills list | grep kanban-orchestrator
```

If the bundled copy is missing, restore it for that profile:

```bash
janus -p orchestrator skills reset kanban-orchestrator --restore
```
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `janus kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.

Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `janus kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `janus profile create --description "..."`, `janus profile describe <name> --text "..."`, `janus profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee` (or the active default profile if that's unset).
...
Config knobs (all under `kanban:` in `~/.janus/config.yaml`):
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `janus profile describe --auto`). |
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.janus/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
If you run `janus dashboard --host 0.0.0.0`, every plugin route — kanban included — becomes reachable from the network. **Don't do that on a shared host.** The board contains task bodies, comments, and workspace paths; an attacker reaching these routes gets read access to your entire collaboration surface and can also create / reassign / archive tasks.

Tasks in `~/.janus/kanban.db` are profile-agnostic on purpose (that's the coordination primitive). If you open the dashboard with `janus -p <profile> dashboard`, the board still shows tasks created by any other profile on the host. Same user owns all profiles, but this is worth knowing if multiple personas coexist.
...
```
janus kanban init                                     # create kanban.db + print daemon hint
janus kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
janus kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
janus kanban show <id> [--json]
janus kanban assign <id> <profile>                    # or 'none' to unassign
janus kanban reassign <id>... <profile>               # bulk re-assign tasks to a profile
janus kanban edit <id> [--title ...] [--body ...]     # edit task title / body / priority in place
        [--priority N]
janus kanban promote <id>...                          # move todo/blocked tasks to ready (recovery)
janus kanban schedule <id> --at <ISO8601>             # set/clear a task's scheduled_at start time
janus kanban diagnostics [--json]                     # board health snapshot (alias: diag)
janus kanban link <parent_id> <child_id>
janus kanban unlink <parent_id> <child_id>
janus kanban claim <id> [--ttl SECONDS]
janus kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
janus kanban complete <id>... [--result "..."]
janus kanban block <id> "<reason>" [--ids <id>...]
janus kanban unblock <id>...
janus kanban archive <id>...

janus kanban tail <id>                                # follow a single task's event stream
janus kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
janus kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
janus kanban runs <id> [--json]                       # attempt history (one row per run)
janus kanban assignees [--json]                       # profiles on disk + per-assignee task counts
janus kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
janus kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `janus gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
janus kanban stats [--json]                           # per-status + per-assignee counts
janus kanban log <id> [--tail BYTES]                  # worker log from ~/.janus/kanban/logs/
janus kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
janus kanban notify-list [<id>] [--json]
janus kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
janus kanban context <id>                             # what a worker sees
janus kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
janus kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
janus kanban create "nightly backup audit" \
...
Every `janus kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `janus chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `janus_cli.kanban.run_slash()` entry point that reuses the `janus kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `janus kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.janus/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`janus kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `janus kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |

For worked examples of each, see `docs/janus-kanban-v1-spec.pdf`.
...
```bash
janus kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
    --workspace dir:~/tenants/business-a/data/
```

Workers receive `$JANUS_TENANT` and namespace their memory writes by prefix. The board, the dispatcher, and the profile definitions are all shared; only the data is scoped.
...
```bash
janus kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
janus kanban notify-list
janus kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
janus kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
janus kanban runs t_abcd
...
**Bulk close caveat.** `janus kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`janus kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `janus kanban heartbeat $TASK` to signal liveness during long operations. |
...
`janus kanban tail <id>` shows these for a single task. `janus kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.janus/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.

## Design spec

The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/janus-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.
