...
Janus runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Janus
...
1. Janus captures a baseline of current diagnostics for the file.
...
…). Janus auto-detects the binary on PATH or in
`<JANUS_HOME>/lsp/bin/`.
...
`node_modules` tree — Janus installs both packages together when you
run `janus lsp install typescript` or auto-install fires on first
use.

## CLI

```
janus lsp status          # service state + per-server install status
janus lsp list            # registry, optionally --installed-only
janus lsp install <id>    # eagerly install one server
janus lsp install-all     # try every server with a known recipe
janus lsp restart         # tear down running clients
janus lsp which <id>      # print resolved binary path
```

`janus lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <JANUS_HOME>/lsp/bin
...
When `install_strategy: auto`, Janus installs binaries into
`<JANUS_HOME>/lsp/bin/`. NPM packages land in
`<JANUS_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Janus-owned and is
...
Servers are kept alive for the life of the Janus process. There's
...
**`janus lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<JANUS_HOME>/lsp/bin/`. Run
`janus lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `janus lsp status`**
...
When `janus lsp status` shows a `Backend warnings` section, install
...
`~/.janus/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.janus/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `janus lsp restart` to clear the set;
