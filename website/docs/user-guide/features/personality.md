...
description: "Customize Janus Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Janus Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `JANUS_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Janus is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Janus now seeds a default `SOUL.md` automatically in:

```text
~/.janus/SOUL.md
```

More precisely, it uses the current instance's `JANUS_HOME`, so if you run Janus with a custom home directory, it will use:
...
- Janus creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Janus loads `SOUL.md` only from `JANUS_HOME`
- Janus does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Janus falls back to a built-in default identity
...
If Janus loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `JANUS_HOME`, the personality belongs to the Janus instance itself.

That also makes it easier to teach users:
- "Edit `~/.janus/SOUL.md` to change Janus' default personality."
...
- how Janus should handle uncertainty, disagreement, or ambiguity
...
## What Janus injects into the prompt
...
Janus ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Janus, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Janus its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.janus/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.janus/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-janus)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Janus speaks
- `display.skin` and `/skin` affect how Janus looks in the terminal
