...
Janus Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
janus memory setup      # interactive picker + configuration
janus memory status     # check what's active
janus memory off        # disable external provider
```

You can also select the active memory provider via `janus plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.janus/config.yaml`:
...
When a memory provider is active, Janus automatically:
...
**Setup Wizard:**
```bash
janus memory setup        # select "honcho" — runs the Honcho-specific post-setup
```
...
:::tip Migrating from `janus honcho`
If you previously used `janus honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Janus profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Janus profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Janus profile. Host key `janus` → default; `janus.<profile>` for others. |
...
```bash
janus profile create coder --clone
```

`--clone` creates a `janus.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Janus profile, creates host blocks for any profile without one, inherits settings from the default `janus` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
janus memory setup    # select "openviking"
# Or manually:
janus config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.janus/.env
```
...
**Config:** `$JANUS_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `janus-user` | User identifier |
...
**Setup:**
```bash
janus memory setup    # select "hindsight"
# Or manually:
janus config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.janus/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p janus ui start`

**Config:** `$JANUS_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `janus` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Janus Agent and the User` | Context label for retained memories |
...
**Setup:**
```bash
janus memory setup    # select "holographic"
# Or manually:
janus config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.janus-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$JANUS_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
janus memory setup    # select "retaindb"
# Or manually:
janus config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.janus/.env
```
...
# Then configure Janus
janus memory setup    # select "byterover"
# Or manually:
janus config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$JANUS_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
janus memory setup    # select "supermemory"
# Or manually:
janus config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.janus/.env
```

**Config:** `$JANUS_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `janus` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `janus-{identity}` → `janus-coder`) to isolate memories per Janus profile
...
| **Requires** | `pip install janus-memori` + `janus-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install janus-memori
janus-memori install
janus config set memory.provider memori
janus memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `janus-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$JANUS_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$JANUS_HOME/` so each profile has its own credentials
