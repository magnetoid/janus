...
When Janus Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.janus/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.janus/config.yaml` (switchable per session
via `/personality`).
...
If you use Janus' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.janus/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Janus Agent's deliverable
