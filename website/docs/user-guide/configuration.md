...
Run `janus setup --portal` — one OAuth gets you a model provider and all four Tool Gateway tools without hand-editing YAML. Portal subscribers also get 10% off token-billed providers. See [Janus Portal](/integrations/nous-portal).
:::
...
├── auth.json       # OAuth provider credentials (Janus Portal, etc.)
