...
| | |
|---|---|
| Source | Optional — install with `janus skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Janus Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Janus terminal calls (desktop app flow only)
...
### Service Account (recommended for Janus)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.janus/.env` (the skill will prompt for this on first load).
...
## Janus Execution Pattern (desktop app flow)

Janus terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/janus-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/janus-op.sock"
