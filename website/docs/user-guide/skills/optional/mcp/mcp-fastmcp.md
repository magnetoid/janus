...
| | |
|---|---|
| Source | Optional — install with `janus skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Janus or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Janus. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.janus/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.janus/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Janus integration, either:

- configure the server in `~/.janus/config.yaml` using the `native-mcp` skill, or
...
### Janus cannot see the deployed server

The server-building part may be correct while the Janus config is not. Load the `native-mcp` skill and configure the server in `~/.janus/config.yaml`, then restart Janus.
