...
description: "Configure and use Honcho memory with Janus -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Janus -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `janus skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Janus

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Janus profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/janus#running-honcho-locally-with-janus

### Verify

```bash
janus honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Janus profile's AI peer
...
Honcho models conversations as interactions between **peers**. Janus creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Janus instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Janus run |
| `global` | Single session across all directories |

Manual override: `janus honcho map my-project-name`
...
Each Janus profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block janus.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `janus.coder` host block in `honcho.json`
...
```bash
janus honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Janus when Honcho memory is active.
...
Config file: `$JANUS_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `janus honcho setup`. Ensure `memory.provider: honcho` is in `~/.janus/config.yaml`.

### Memory not persisting across sessions
Check `janus honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `janus profile create <name> --clone`. For existing profiles: `janus honcho sync`.
...
| Command | Description |
|---------|-------------|
| `janus honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `janus honcho status` | Show resolved config, connection test, peer info for active profile |
| `janus honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `janus honcho disable` | Disable Honcho for the active profile |
| `janus honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `janus honcho peers` | Show peer identities across all profiles |
| `janus honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `janus honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `janus honcho sessions` | List known directory-to-session-name mappings |
| `janus honcho map <name>` | Map current working directory to a Honcho session name |
| `janus honcho identity` | Seed AI peer identity or show both peer representations |
| `janus honcho sync` | Create host blocks for all Janus profiles that don't have one yet |
| `janus honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Janus + Honcho |
| `janus memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `janus memory status` | Show active memory provider and config |
| `janus memory off` | Disable external memory provider |
