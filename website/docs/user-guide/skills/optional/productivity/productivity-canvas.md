...
| | |
|---|---|
| Source | Optional — install with `janus skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Janus Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.janus/.env`:
...
```bash
CANVAS="python $JANUS_HOME/skills/productivity/canvas/scripts/canvas_api.py"
