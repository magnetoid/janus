...
| | |
|---|---|
| Source | Optional — install with `janus skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${JANUS_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${JANUS_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${JANUS_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client janus
```
...
```bash
PUBLISH="${JANUS_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client janus
```
...
```bash
DRIVE="${JANUS_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `janus`)    |
