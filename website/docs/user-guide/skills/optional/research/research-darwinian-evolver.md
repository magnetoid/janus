...
| | |
|---|---|
| Source | Optional — install with `janus skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Janus Agent |
| License | MIT |
...
import upstream classes into Janus itself.
...
```bash
mkdir -p ~/.janus/cache/darwinian-evolver && cd ~/.janus/cache/darwinian-evolver
...
```bash
cd ~/.janus/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.janus/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.janus/skills/research/darwinian-evolver
DE_DIR=~/.janus/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Janus core.
   Custom driver scripts under `~/.janus/skills/...` are user-side and fine.
