...
- Give it a name like "janus-agent"
- Select scopes:
...
- Give it a title like "janus-agent-&lt;machine-name>"
...
elif [ -f ~/.janus/.env ] && grep -q "^GITHUB_TOKEN=" ~/.janus/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.janus/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
