...
title: "Git Worktrees"
description: "Run multiple Janus agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Janus Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Janus so each session has a clean, isolated working directory.

## Why Use Worktrees with Janus?

Janus treats the **current working directory** as the project root:

- CLI: the directory where you run `janus` or `janus chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.janus/config.yaml`
...
git worktree add ../repo-feature feature/janus-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/janus-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Janus there:

```bash
cd ../repo-feature

# Start Janus in the worktree
janus
```
...
git worktree add ../repo-experiment-a feature/janus-a
git worktree add ../repo-experiment-b feature/janus-b
```
...
- Works on its own branch (`feature/janus-a` vs `feature/janus-b`).
...
- Janus checkpoint data under `~/.janus/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Janus experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/janus-checkpoints-docs`, `feature/janus-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Janus from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `janus -w` (Automatic Worktree Mode)

Janus has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `janus/janus-<hash>`).
...
```bash
janus -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `janus -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Janus session its own clean checkout.
