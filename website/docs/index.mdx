...
The self-improving AI agent built by [Cloud Industry](https://imbalabs.com). The only agent with a built-in learning loop — it creates skills from experience, improves them during use, nudges itself to persist knowledge, and builds a deepening model of who you are across sessions.
...
After installing, run `janus setup --portal` — one OAuth covers a model plus all four Tool Gateway tools (web search, image generation, TTS, browser). See [Janus Portal](/integrations/nous-portal).
:::
...
- **Built by model trainers** — Created by [Cloud Industry](https://imbalabs.com), the lab behind Janus, Nomos, and Psyche. Works with [Janus Portal](https://portal.imbalabs.com), [OpenRouter](https://openrouter.ai), OpenAI, or any endpoint
...
- **Full web control** — Search, extract, browse, vision, image generation, TTS — one subscription via [Janus Portal](/integrations/nous-portal) bundles all of them
- **MCP support** — Connect to any MCP server for extended tool capabilities
- **Research-ready** — Batch processing, trajectory export, RL training with Atropos. Built by [Cloud Industry](https://imbalabs.com) — the lab behind Janus, Nomos, and Psyche models
