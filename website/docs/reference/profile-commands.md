...
This page covers all commands related to [Janus profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `janus profile` without a subcommand shows help.
...
```bash
janus profile use <name>
```

Sets `<name>` as the active profile. All subsequent `janus` commands (without `-p`) will use this profile.
...
janus profile use default
```

## `janus profile create`

```bash
janus profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `janus profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `janus update` runs won't re-seed the bundled set, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.janus`), use `janus skills opt-out` / `janus skills opt-in`. |
...
janus profile create mybot

# Clone config only from current profile
janus profile create work --clone

# Clone everything from current profile
janus profile create backup --clone-all

# Clone config from a specific profile
janus profile create work2 --clone --clone-from work
```

## `janus profile describe`

```bash
janus profile describe [<name>] [options]
```
...
janus profile describe researcher

# Set it explicitly
janus profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
janus profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
janus profile describe --all --auto
```

## `janus profile delete`

```bash
janus profile delete <name> [options]
```
...
```bash
janus profile delete mybot
janus profile delete mybot --yes
```
...
```bash
janus profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Janus home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ janus profile show work
Profile: work
Path:    ~/.janus/profiles/work
...
```bash
janus profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Janus installation.
...
```bash
janus profile alias work
# Creates/updates ~/.local/bin/work

janus profile alias work --name mywork
# Creates ~/.local/bin/mywork

janus profile alias work --remove
# Removes the wrapper script
```

## `janus profile rename`

```bash
janus profile rename <old-name> <new-name>
```
...
```bash
janus profile rename mybot assistant
# ~/.janus/profiles/mybot → ~/.janus/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `janus profile export`

```bash
janus profile export <name> [options]
```
...
```bash
janus profile export work
# Creates work.tar.gz in the current directory

janus profile export work -o ./work-2026-03-29.tar.gz
```

## `janus profile import`

```bash
janus profile import <archive> [options]
```
...
```bash
janus profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

janus profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`janus profile export` / `import` are still the right commands for
...
### `janus profile install`

```bash
janus profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `janus -p telemetry`). |
...
janus profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
janus profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
janus profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
janus profile install ./telemetry/
```

### `janus profile update`

```bash
janus profile update <name> [--force-config] [--yes]
```
...
```bash
janus profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Janus version, author, env var requirements, the source URL/path, and
...
`janus profile list` also shows the distribution name and version in a
`Distribution` column, and `janus profile show <name>` / `delete <name>`
...
janus profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
janus profile install https://github.com/your-org/internal-assistant.git
```
...
`janus_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Janus version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Janus can clone from.
3. Tell recipients to run `janus profile install <your-repo-url>`.
...
## `janus -p` / `janus --profile`

```bash
janus -p <name> <command> [options]
janus --profile <name> <command> [options]
```

Global flag to run any Janus command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
janus -p work chat -q "Check the server status"
janus --profile dev gateway start
janus -p personal skills list
janus -p work config edit
```

## `janus completion`

```bash
janus completion <shell>
```
...
- `janus profile <TAB>` — subcommands (list, use, create, etc.)
- `janus profile use <TAB>` — profile names
- `janus -p <TAB>` — profile names
