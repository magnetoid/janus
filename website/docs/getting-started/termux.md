...
description: "Run Janus Agent directly on an Android phone with Termux"
---

# Janus on Android with Termux

This is the tested path for running Janus Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Janus from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
- links `janus` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `janus` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/janus" "$PREFIX/bin/janus"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `janus` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.janus/.env`.
...
### `janus doctor` says ripgrep or Node is missing
