...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Janus Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Janus setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Janus Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Janus Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Janus for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Janus working on my machine | `janus setup` | Run a real chat and verify it responds |
| I already know my provider | `janus model` | Save the config, then start chatting |
| I want a bot or always-on setup | `janus gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `janus model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `janus model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Janus cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Janus Agent
...
The single most important setup step. Use `janus model` to walk through the choice interactively:

```bash
janus model
```

:::tip Easiest path: Cloud Industry Portal
...
|----------|-----------|---------------|
| **Cloud Industry Portal** | Subscription-based, zero-config | OAuth login via `janus model` |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `janus_cli/models.py` may change between releases) | `janus model` → MiniMax (OAuth) |
...
| **Google Gemini (OAuth)** | Gemini via the `google-gemini-cli` OAuth flow — no key needed | `janus model` → Google Gemini (OAuth) |
| **xAI** | Grok models via direct API | Set `XAI_API_KEY` |
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `janus model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `janus model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `janus model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `janus model` (requires `copilot` CLI + `copilot login`) |
...
Janus Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `janus model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Janus separates secrets from normal config:

- **Secrets and tokens** → `~/.janus/.env`
- **Non-secret settings** → `~/.janus/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
janus config set model anthropic/claude-opus-4.6
janus config set terminal.backend docker
janus config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
janus            # classic CLI
janus --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Janus ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `janus` vs `janus --tui`.
:::
...
```bash
janus --continue    # Resume the most recent session
janus -c            # Short form
```
...
```bash
janus gateway setup    # Interactive platform configuration
```
...
- `janus tools` — tune tool access per platform
- `janus skills` — browse and install reusable workflows
...
```bash
janus config set terminal.backend docker    # Docker isolation
janus config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Janus install directory (the curl installer placed it at
# ~/.janus/janus-agent on Linux/macOS or %LOCALAPPDATA%\janus\janus-agent on Windows):
...
Skills are on-demand instruction documents that teach Janus how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Janus ships with a catalog of bundled skills already installed in `~/.janus/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
janus skills browse                      # list everything available
janus skills search kubernetes           # find skills by keyword
janus skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `janus skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Janus ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.janus/janus-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Janus opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `janus model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `janus gateway setup` and check `janus gateway status` |
| `janus --continue` can't find old session | Switched profiles or session never saved | Check `janus sessions list` and confirm you're in the right profile |
...
| `janus doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `janus model`
3. `janus setup`
4. `janus sessions list`
5. `janus --continue`
6. `janus gateway status`
...
| `janus tools` | Configure which tools are enabled per platform |
| `janus setup` | Full setup wizard (configures everything at once) |
| `janus doctor` | Diagnose issues |
| `janus update` | Update to latest version |
| `janus gateway` | Start the messaging gateway |
| `janus --continue` | Resume last session |
