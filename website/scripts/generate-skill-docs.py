...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/magnetoid/janus/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    janus_meta = (fm.get("metadata") or {}).get("janus") or {}
    tags = janus_meta.get("tags") or []
    related = janus_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `janus skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Janus Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Janus ships with a large built-in skill library copied into `~/.janus/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Janus also syncs bundled skills on `janus update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.janus/skills/` tree, it is still shipped with Janus; restore it with `janus skills reset <name> --restore`.",
        "",
