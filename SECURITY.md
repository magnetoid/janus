# Janus Agent Security Policy

This document describes Janus Agent's trust model, names the one
...
- Environment details (`janus version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Janus Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Janus Agent's documentation
...
Janus Agent supports two OS-level isolation postures. They address
...
Janus Agent supports this in two ways:

- **Janus Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Janus Agent's in-process heuristics
...
Janus Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Janus Agent
itself. Bugs in Janus Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Janus Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Janus Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Janus Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  janus-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Janus Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Janus Agent. Bugs in Janus
