/**
 * ChatPage — embeds `janus --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (janus_cli/web_server.py)                          .
 *          ▼                                                              .
...
function buildWsUrl(
  ...
  return `${proto}//${window.location.host}${JANUS_BASE_PATH}/api/pty?${qs.toString()}`;
}
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__JANUS_SESSION_TOKEN__ &&
    !window.__JANUS_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `janus dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__JANUS_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Janus TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[janus-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__JANUS_AUTH_REQUIRED__``) this
    ...
    // forwarding them into Janus. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="janus-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
