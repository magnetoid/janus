...
``{title, url, description, position}`` rows every other Janus web
provider produces.
...
prefers Janus-managed xAI Grok OAuth (via ``janus auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.janus/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``janus tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `janus auth` to sign in with "
