...
three virtual model IDs so the ``janus tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not os.environ.get("OPENAI_API_KEY"):
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `janus tools` → Image "
                    "Generation → OpenAI to configure, or `janus setup` "
                    "to add the key."
                ),
