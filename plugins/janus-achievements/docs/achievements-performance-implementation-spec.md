# Janus Achievements Implementation Spec (Detailed)
...
- persisted runtime files:
  - `~/.janus/plugins/janus-achievements/state.json` (existing unlock state)
  - `~/.janus/plugins/janus-achievements/scan_snapshot.json` (new)
  - `~/.janus/plugins/janus-achievements/scan_checkpoint.json` (new)
