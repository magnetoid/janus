# Janus Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: janus-achievements plugin
...
### `GET /api/plugins/janus-achievements/achievements`
...
### `POST /api/plugins/janus-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/janus-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/janus-achievements/overview`
...
- Persist plugin data under:
  - `~/.janus/plugins/janus-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.janus/plugins/janus-achievements/scan_checkpoint.json`
...
- `~/.janus/plugins/janus-achievements/`
