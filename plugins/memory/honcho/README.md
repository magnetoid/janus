...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/janus>
...
```bash
janus memory setup honcho   # configure Honcho directly (works on a fresh install)
janus memory setup          # generic picker, choose Honcho from the list
```

Or manually:
```bash
janus config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.janus/.env
```

> `janus honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `janus memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$JANUS_HOME/honcho.json` | Profile-local (isolated Janus instances) |
| 2 | `~/.janus/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Janus profile: `janus` (default) or `janus_<profile>`.
...
**Deployment shapes** (`janus memory setup honcho` asks one prompt to set these):

- **Single-operator** — `pinUserPeer: true`. All gateway users → `peerName`. Recommended for personal use where you connect Janus to your own Telegram/Discord/etc.
...
| 4 | `per-session` strategy | Janus session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`janus-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`janus`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-janus-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening janus in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Janus session ID (timestamp + hex). Every `janus` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Janus profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`janus`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Janus profile: `janus` (default) or `janus_<profile>` (e.g. `janus -p coder` -> host key `janus_coder`). Older `janus.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `JANUS_HONCHO_HOST` | Host key override |
...
|---------|-------------|
| `janus memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `janus honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `janus memory setup`) |
| `janus honcho status` | Show resolved config for active profile |
| `janus honcho enable` / `disable` | Toggle Honcho for active profile |
| `janus honcho mode <mode>` | Change recall or observation mode |
| `janus honcho peer --user <name>` | Update user peer name |
| `janus honcho peer --ai <name>` | Update AI peer name |
| `janus honcho tokens --context <N>` | Set context token budget |
| `janus honcho tokens --dialectic <N>` | Set dialectic max chars |
| `janus honcho map <name>` | Map current directory to a session name |
| `janus honcho sync` | Create host blocks for all Janus profiles |
