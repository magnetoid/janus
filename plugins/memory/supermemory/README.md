...
Config file: `$JANUS_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `janus` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `janus-{identity}` → `janus-coder`). |
...
All Supermemory API calls send `x-sm-source: janus`, and document writes stamp
`metadata.sm_source: janus`. This is a **functional routing key, not telemetry**:
it groups Janus-written memories into a dedicated "Janus" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Janus profile:

```json
{
  "container_tag": "janus-{identity}"
}
```

For a profile named `coder`, this resolves to `janus-coder`. The default profile resolves to `janus-default`. Without `{identity}`, all profiles share the same container.
