...
def _load_supermemory_config(janus_home: str) -> dict:
    config = _default_config()
    config_path = Path(janus_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, janus_home: str) -> None:
    config_path = Path(janus_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "janus"},
        )
    ...
    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        ...
        merged = {"sm_source": "janus", **(metadata or {})}
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `janus memory setup`.
        # All other options are documented for $JANUS_HOME/supermemory.json
    ...
    def save_config(self, values, janus_home):
        ...
        _save_supermemory_config(sanitized, janus_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from janus_constants import get_janus_home
        self._janus_home = kwargs.get("janus_home") or str(get_janus_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._janus_home)
