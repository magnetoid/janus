...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: janus)
...
Or via $JANUS_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Janus can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $JANUS_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_janus_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "janus": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "janus"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Janus config."""
    profile = config.get("profile", "janus")
    return str(profile or "janus")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Janus profile name (from agent_identity)
      {workspace} — Janus workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``janus-{user}`` with no user becomes ``janus``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Janus Agent and the User"
    ...
    def save_config(self, values, janus_home):
        """Write config to $JANUS_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(janus_home) / "hindsight"
    ...
    def post_setup(self, janus_home: str, config: dict) -> None:
        ...
        from janus_cli.memory_setup import _curses_select
        ...
        provider_config.setdefault("bank_id", "janus")
        ...
        self.save_config(provider_config, janus_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(janus_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(janus_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "janus"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: janus-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Janus Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "janus"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "janus"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "janus", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "janus")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Janus Agent and the User")
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Janus created/used that client on the shared async loop,
