...
Key contract points encoded here:
  ...
  - audience claim is the bare ``client_id`` (no ``janus-cli:`` prefix).
...
"state": …}`` and the route serializes those into the ``janus_session_pkce``
cookie.
...
Skip reasons:
  ...
  ("Set JANUS_DASHBOARD_OAUTH_CLIENT_ID …") instead of the bare "no
...
class NousDashboardAuthProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # The auth-route layer expects ``cookie_payload[\"janus_session_pkce\"]``
    ...
    def _verify_jwt(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # between JANUS_DASHBOARD_PORTAL_URL / JANUS_DASHBOARD_OAUTH_CLIENT_ID
...
def _resolve_client_id() -> str:
    ...
    Order:
      1. ``JANUS_DASHBOARD_OAUTH_CLIENT_ID`` env var (when non-empty
    ...
    """
    env = os.environ.get("JANUS_DASHBOARD_OAUTH_CLIENT_ID", "").strip()
    if env:
        return env
...
def _resolve_portal_url() -> str:
    ...
    Order:
      1. ``JANUS_DASHBOARD_PORTAL_URL`` env var (non-empty after strip).
      2. ``dashboard.oauth.portal_url`` in ``config.yaml``.
      3. :data:`_DEFAULT_PORTAL_URL` (production Portal).
    """
    env = os.environ.get("JANUS_DASHBOARD_PORTAL_URL", "").strip()
    if env:
        return env
    cfg_value = str(
...
def register(ctx) -> None:
    ...
    configured (either via ``JANUS_DASHBOARD_OAUTH_CLIENT_ID`` env var
    ...
    can surface "Set JANUS_DASHBOARD_OAUTH_CLIENT_ID …" instead of the
    ...
    engagement layer (``janus_cli.web_server.should_require_auth`` +
