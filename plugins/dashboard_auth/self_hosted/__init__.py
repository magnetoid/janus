...
A standards-compliant OpenID Connect Relying Party for the ``janus dashboard``
OAuth gate. Unlike the bundled ``nous`` provider (which encodes Cloud Industry Portal's
...
:class:`~janus_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``janus_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~janus_cli.dashboard_auth.Session`.
...
provider verifies its *access* token because Cloud Industry Portal mints a custom JWT
...
same precedence convention as the ``nous`` plugin)::
    ...
    JANUS_DASHBOARD_OIDC_ISSUER
    JANUS_DASHBOARD_OIDC_CLIENT_ID
    JANUS_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        cookie_payload = {
            "janus_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Janus does not call
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``JANUS_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "JANUS_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "JANUS_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("JANUS_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(JANUS_DASHBOARD_OIDC_ISSUER + JANUS_DASHBOARD_OIDC_CLIENT_ID) "
