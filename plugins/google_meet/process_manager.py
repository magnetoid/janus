...
session-scoped state file under ``$JANUS_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $JANUS_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_janus_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this janus install, leave it first —
    ...
    env = os.environ.copy()
    env["JANUS_MEET_URL"] = url
    env["JANUS_MEET_OUT_DIR"] = str(out)
    env["JANUS_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["JANUS_MEET_HEADED"] = "1"
    if auth_state:
        env["JANUS_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["JANUS_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if JANUS_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["JANUS_MEET_MODE"] = mode
    if realtime_model:
        env["JANUS_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["JANUS_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["JANUS_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["JANUS_MEET_REALTIME_KEY"] = realtime_api_key
