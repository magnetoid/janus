...
Let the janus agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where janus runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `janus meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `janus meet` CLI |
...
| `cli.py` | `janus meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$JANUS_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `janus meet node {run,list,approve,remove,status,ping}` |
...
janus meet install                                      # pip + Chromium
janus meet setup                                        # preflight
janus meet auth                                         # optional
janus meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
janus meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.janus/.env
janus meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
janus meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
janus meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.janus/.env
janus meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, janus will **not** switch your system audio input automatically — the
...
janus plugins enable google_meet
janus meet node run --display-name my-mac --host 0.0.0.0 --port 18789
