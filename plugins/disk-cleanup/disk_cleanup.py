"""disk_cleanup — ephemeral file cleanup for Janus Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under JANUS_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly JANUS_HOME and /tmp/janus-*
Never touches: ~/.janus/logs/ or any system directory.
"""
...
try:
    ...
    def get_janus_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("JANUS_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".janus").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$JANUS_HOME/logs/``."""
    return get_janus_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$JANUS_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under JANUS_HOME or ``/tmp/janus-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    janus_home = get_janus_home()
    try:
        path.resolve().relative_to(janus_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/janus-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("janus-"):
        return True
    return False
...
# Paths under $JANUS_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so JANUS_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        janus_home = get_janus_home()
        for parent in ("cron", "cronjobs"):
            base = janus_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside JANUS_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under JANUS_HOME (but leave JANUS_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "janus-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(janus_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == janus_home:
                continue
            try:
                rel_parts = dirpath.relative_to(janus_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    janus_home = get_janus_home()
    try:
        rel = path.resolve().relative_to(janus_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "janus-agent",
        }:
            return None
        ...
        # Path isn't under JANUS_HOME (e.g. /tmp/janus-*) — fall through.
        pass
