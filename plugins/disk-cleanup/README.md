...
Auto-tracks and cleans up ephemeral files created during Janus Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$JANUS_HOME` and `/tmp/janus-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `JANUS_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `JANUS_HOME` or `/tmp/janus-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$JANUS_HOME/disk-cleanup/` is itself excluded
- `$JANUS_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
