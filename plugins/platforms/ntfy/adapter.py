"""ntfy platform adapter (Janus plugin).
...
``plugins/platforms/ntfy/``. The Janus plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "janus-in"              # subscribe topic (incoming)
          publish_topic: "janus-out"     # optional — defaults to topic
...
_ECHO_TAG = "janus-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``janus cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Janus dependency",
