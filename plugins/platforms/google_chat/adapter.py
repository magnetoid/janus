...
# ``model_tools`` import time, so ``janus status``, ``janus chat``, etc.
...
# ``janus_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Janus is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a janus MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${JANUS_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _janus_home = _get_janus_home()
        except (ModuleNotFoundError, ImportError):
            _janus_home = _Path.home() / ".janus"
        self._thread_count_store = _ThreadCountStore(
            _janus_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Janus is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("JANUS_HOME", str(_Path.home() / ".janus"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${JANUS_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a janus MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Janus is thinking…' marker message.
        ...
        "Janus is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Janus is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Janus is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Janus is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Janus is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``janus setup``.

    The setup wizard at ``janus_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.janus/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. janus-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.janus/.env")
    print_info("Restart the gateway: janus gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``janus cron`` running as a
    separate process from ``janus gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Janus plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'janus-agent[google_chat]'",
        ...
        # the "Janus is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Janus is thinking…' marker message "
