"""SimpleX Chat platform adapter (Janus plugin).
...
This adapter ships as a Janus platform plugin under
``plugins/platforms/simplex/``. The Janus plugin loader scans the
...
discoverable and `janus setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Janus — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"janus-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``janus setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.janus/.env`` via ``janus_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("janus_cli.config not available; set SIMPLEX_* vars manually in ~/.janus/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `janus gateway status` without
