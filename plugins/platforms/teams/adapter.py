"""
Microsoft Teams platform adapter for Janus Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Janus"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        janus_action = data.get("janus_action", "")
        session_key = data.get("session_key", "")

        if not janus_action or not session_key:
        ...
        choice = choice_map.get(janus_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="janus_approve",
                    data={**btn_data_base, "janus_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="janus_approve",
                    data={**btn_data_base, "janus_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="janus_approve",
                    data={**btn_data_base, "janus_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="janus_approve",
                    data={**btn_data_base, "janus_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Janus\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.janus/.env")
