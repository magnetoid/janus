"""
LINE Messaging API platform adapter for Janus Agent.
...
to Janus Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``JANUS_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            janus_home = Path.home().joinpath(".janus").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``janus status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``janus status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``janus setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.janus/.env`` via ``janus_cli.config``.
    """
    print()
    ...
    try:
        from janus_cli.config import get_env_var, set_env_var
    except ImportError:
        print("janus_cli.config not available; set LINE_* vars manually in ~/.janus/.env")
        return
