...
Override the image with ``JANUS_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("JANUS_TEST_IMAGE", "janus-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``JANUS_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("JANUS_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged janus user
# ---------------------------------------------------------------------------
#
# Background: every Janus runtime path inside the container drops to UID
# 10000 (the ``janus`` user) via ``s6-setuidgid janus``. ``docker exec``
...
# fails (PermissionError swallowed) for janus, so a test that ran as root
...
# helpers below run every probe under ``-u janus`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: janus).
