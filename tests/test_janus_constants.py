"""Tests for janus_constants module."""
...
class TestGetDefaultJanusRoot:
    """Tests for get_default_janus_root() — Docker/custom deployment awareness."""

    def test_no_janus_home_returns_native(self, tmp_path, monkeypatch):
        """When JANUS_HOME is not set, returns ~/.janus."""
        monkeypatch.delenv("JANUS_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_janus_root() == tmp_path / ".janus"

    def test_janus_home_is_native(self, tmp_path, monkeypatch):
        """When JANUS_HOME = ~/.janus, returns ~/.janus."""
        native = tmp_path / ".janus"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JANUS_HOME", str(native))
        assert get_default_janus_root() == native

    def test_janus_home_is_profile(self, tmp_path, monkeypatch):
        """When JANUS_HOME is a profile under ~/.janus, returns ~/.janus."""
        native = tmp_path / ".janus"
    ...
    def test_janus_home_is_docker(self, tmp_path, monkeypatch):
        """When JANUS_HOME points outside ~/.janus (Docker), returns JANUS_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JANUS_HOME", str(docker_home))
        assert get_default_janus_root() == docker_home

    def test_janus_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any JANUS_HOME outside ~/.janus is treated as the root."""
        custom = tmp_path / "my-janus-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JANUS_HOME", str(custom))
        assert get_default_janus_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (JANUS_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JANUS_HOME", str(profile))
        assert get_default_janus_root() == docker_root

    def test_no_janus_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\janus, not ~/.janus."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("JANUS_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(janus_constants.sys, "platform", "win32")

        assert get_default_janus_root() == local_appdata / "janus"

    def test_no_janus_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/janus without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("JANUS_HOME", raising=False)
        ...
        assert get_default_janus_root() == home / "AppData" / "Local" / "janus"


class TestGetJanusHome:
    """Tests for get_janus_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When JANUS_HOME is unset on Windows, use %LOCALAPPDATA%\\janus."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("JANUS_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(janus_constants.sys, "platform", "win32")
        monkeypatch.setattr(janus_constants, "_profile_fallback_warned", False)

        assert get_janus_home() == local_appdata / "janus"
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(janus_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(janus_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".janus"
