...
``janus_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``janus --help``, ``janus version``,
``janus logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``janus --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from janus_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["janus", "-w", "--tui"], None),
        (["janus", "version"], "version"),
        (["janus", "--tui", "chat"], "chat"),
        (["janus", "-w", "logs"], "logs"),
        (["janus", "chat", "hello world"], "chat"),
        (["janus", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["janus", "-m", "gpt5", "chat"], "chat"),
        (["janus", "--model", "gpt5", "chat", "hi"], "chat"),
        (["janus", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["janus", "-z", "hello world"], None),
        (["janus", "-z", "hello", "chat"], "chat"),
        (["janus", "--model=gpt5", "chat"], "chat"),     # inline form
        (["janus", "--", "chat"], "chat"),               # -- terminator
        (["janus", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["janus", "some-plugin-cmd"], "some-plugin-cmd"),
        (["janus", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["janus"],                          # bare → chat
        ["janus", "--help"],                # top-level help
        ["janus", "-h"],
        ["janus", "version"],               # known built-in
        ["janus", "logs"],
        ["janus", "gateway", "run"],
        ["janus", "--tui"],
        ["janus", "-w", "--tui"],
        ["janus", "chat", "hi"],
        ["janus", "help"],                  # accepted built-in-ish
        ["janus", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["janus", "meet", "join"],          # potential google_meet plugin
        ["janus", "honcho", "status"],      # potential memory plugin
        ["janus", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``janus help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"janus_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
