...
def test_pip_install_detected_when_no_git_dir(tmp_path):
    ...
    with patch("janus_cli.config.get_managed_system", return_value=None), \
         patch("janus_cli.config.get_janus_home", return_value=tmp_path):
...
def test_managed_install_takes_precedence(tmp_path):
    """When JANUS_MANAGED is set, that takes precedence over git detection."""
    (tmp_path / ".git").mkdir()
    with patch("janus_cli.config.get_managed_system", return_value="NixOS"), \
         patch("janus_cli.config.get_janus_home", return_value=tmp_path):
        from janus_cli.config import detect_install_method
        method = detect_install_method(project_root=tmp_path)
        assert method == "nixos"
...
def test_recommended_update_command_pip():
    ...
    from janus_cli.config import recommended_update_command_for_method
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    image, so ``janus update`` refused to run. With a ``.git`` checkout it
    must resolve to ``git``.
    """
    (tmp_path / ".git").mkdir()
    with patch("janus_cli.config.get_managed_system", return_value=None), \
         patch("janus_cli.config.get_janus_home", return_value=tmp_path), \
         patch("janus_constants.is_container", return_value=True):
        from janus_cli.config import detect_install_method
...
def test_banner_warns_on_pip_install(tmp_path):
    ...
    with patch("janus_cli.config.get_janus_home", return_value=hh), \
         patch("janus_constants.get_janus_home", return_value=hh):
        buf = io.StringIO()
