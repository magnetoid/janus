...
Covers:
- Defense 1: gateway stop/restart refuse when _JANUS_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.janus.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.janus.gateway.plist",
        "launchctl stop ai.janus.gateway",
        "systemctl restart janus-gateway",
        "systemctl stop janus-gateway.service",
    ...
    @pytest.mark.parametrize("text", [
        "kill janus gateway process",
        "pkill -f janus.*gateway",
    ])
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_janus_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade janus then run janus gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.janus.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestGatewaySelfTargetingGuard:
    """Verify janus gateway stop/restart refuse when _JANUS_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_JANUS_GATEWAY", "1")
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_JANUS_GATEWAY", raising=False)
