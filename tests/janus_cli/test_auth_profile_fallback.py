...
When ``JANUS_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.janus/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.janus            (has its own auth.json fixture)
    * Profile     -> tmp_path/.janus/profiles/coder   (active, JANUS_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Janus is not
# logged into Cloud Industry Portal"`` even though the user has a valid global Nous
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from janus_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    janus_home = tmp_path / "classic"
    janus_home.mkdir()
    monkeypatch.setenv("JANUS_HOME", str(janus_home))

    _write(janus_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (JANUS_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.janus as the "real home" — which is NOT equal
    # to the JANUS_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    janus_home = tmp_path / "classic"
    ...
    _write(janus_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from janus_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: JANUS_HOME is set to a custom path that is NOT under
    # ~/.janus/profiles/ — get_default_janus_root() returns JANUS_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from janus_cli.auth import read_credential_pool, write_credential_pool
