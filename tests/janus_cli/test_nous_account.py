"""Tests for normalized Cloud Industry Portal account entitlement helpers."""
    ...
    token = _jwt(
        {
            ...
            "nous_client": "janus-agent",
            "exp": int(time.time()) + 900,
            "paid_access": True,
            "subscription_tier": 2,
        }
    )
    monkeypatch.setattr("janus_cli.auth.get_provider_auth_state", lambda provider: _state(token))
...
def test_expired_jwt_falls_back_to_fresh_account(monkeypatch):
    ...
    monkeypatch.setattr("janus_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    monkeypatch.setattr("janus_cli.auth.resolve_nous_access_token", lambda: "fresh-token")
    monkeypatch.setattr("janus_cli.nous_account._fetch_nous_account_info", lambda *a, **kw: payload)
...
def test_no_oauth_token_reports_inference_key_present(monkeypatch):
    monkeypatch.setattr("janus_cli.auth.get_provider_auth_state", lambda provider: {})
...
def test_entitlement_message_for_inference_key_without_portal_login():
    ...
    assert "cannot verify your Cloud Industry Portal paid access" in message
...
def test_entitlement_message_for_unknown_entitlement_is_explicit():
    ...
    assert "Run `janus model`" in message
...
def test_entitlement_message_for_account_missing():
    ...
    assert message is not None
    assert "could not find a Cloud Industry Portal account or organisation" in message
