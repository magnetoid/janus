"""Tests for janus_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("janus_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from janus_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("janus_cli.config.load_config")
    ...
    @patch("janus_cli.config.load_config")
    @patch.dict("os.environ", {"JANUS_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """JANUS_SESSION_PLATFORM should be used when JANUS_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_janus_platform_takes_precedence(self, tmp_path, monkeypatch):
        """JANUS_PLATFORM should win over JANUS_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("JANUS_HOME", str(tmp_path))
        monkeypatch.setenv("JANUS_PLATFORM", "telegram")
        monkeypatch.setenv("JANUS_SESSION_PLATFORM", "telegram")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from janus_cli.skills_config import _get_categories
        skills = [
