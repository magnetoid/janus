"""Tests for ``janus dashboard register``.
...
import janus_cli.dashboard_register as dr
...
class TestFastFails:
    def test_not_logged_in_exits_1_with_setup_hint(self, capsys):
        ...
        with patch.object(dr, "cmd_dashboard_register", dr.cmd_dashboard_register):
            with patch(
                "janus_cli.auth.resolve_nous_access_token", side_effect=err
            ), patch("janus_cli.config.is_managed", return_value=False):
                with pytest.raises(SystemExit) as exc:
                    dr.cmd_dashboard_register(_ns())
        assert exc.value.code == 1
        out = capsys.readouterr().out
        assert "not logged into Cloud Industry Portal" in out
...
class TestHappyPath:
    def _run(self, *, args, account_token="tok_abc", portal="https://portal.cloud-industry.com",
        ...
        with patch(
            "janus_cli.auth.resolve_nous_access_token", return_value=account_token
        ), patch("janus_cli.config.is_managed", return_value=False), patch.object(
            dr, "_resolve_portal_base_url", return_value=portal
        ), patch(
            "janus_cli.config.get_env_value", return_value=None
        ), patch(
            "janus_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
    ...
    def test_writes_client_id_and_posts_generated_name(self, capsys):
        ...
        assert saved["JANUS_DASHBOARD_OAUTH_CLIENT_ID"] == "agent:selfhost-1"
        assert "JANUS_DASHBOARD_PORTAL_URL" not in saved
    ...
    def test_custom_redirect_uri_is_forwarded(self, capsys):
        captured: dict = {}
        self._run(
            args=_ns(redirect_uri="https://janus.example.com/auth/callback"),
            captured=captured,
        )
        assert (
            captured["body"]["custom_redirect_uri"]
            == "https://janus.example.com/auth/callback"
        )
    ...
    def test_non_default_portal_is_persisted(self, capsys):
        ...
        assert (
            saved["JANUS_DASHBOARD_PORTAL_URL"]
...
class TestPortalResolution:
    ...
    def test_falls_back_to_stored_login_portal(self):
        with patch(
            ...
            return_value={"portal_base_url": "https://portal.staging-cloud-industry.com"},
        ):
            assert (
                dr._resolve_portal_base_url(None)
                == "https://portal.staging-cloud-industry.com"
            )
...
class TestPortalErrors:
    def _run_http_error(self, code, body):
        err = urllib.error.HTTPError(
            url="https://portal.cloud-industry.com/api/oauth/self-hosted-client",
            code=code,
            msg="err",
            hdrs=None,
        ...
        with patch(
            ...
            dr, "_resolve_portal_base_url", return_value="https://portal.cloud-industry.com"
