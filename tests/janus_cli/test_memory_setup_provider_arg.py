"""Tests for `janus memory setup [provider]` routing.
...
``janus memory setup honcho``) without the interactive picker — which matters
because the per-provider ``janus <provider>`` subcommand is only registered
...
from janus_cli import memory_setup
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "janus memory setup" in out
