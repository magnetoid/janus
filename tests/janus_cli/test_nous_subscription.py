...
from janus_cli.nous_account import NousPortalAccountInfo, NousToolAccessInfo
from janus_cli import nous_subscription as ns
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    monkeypatch.setattr(
        "janus_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
...
# ---------------------------------------------------------------------------
# ensure_nous_portal_access — inline login gate for `janus tools`
# ---------------------------------------------------------------------------
