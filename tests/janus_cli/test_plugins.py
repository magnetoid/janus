"""Tests for the Janus plugin system (janus_cli.plugins)."""
...
from janus_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<janus_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<janus_home>/plugins/``; we derive
    ``<janus_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <JANUS_HOME>/config.yaml.
        # Config is always read from JANUS_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        janus_home_str = os.environ.get("JANUS_HOME")
        if janus_home_str:
            janus_home = Path(janus_home_str)
        else:
            janus_home = base.parent
        janus_home.mkdir(parents=True, exist_ok=True)
        cfg_path = janus_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.janus/plugins/ are discovered."""
        plugins_dir = tmp_path / "janus_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("JANUS_HOME", str(tmp_path / "janus_test"))
    ...
    def test_execution_middleware_does_not_retry_downstream_failure(self, monkeypatch):
        ...
        monkeypatch.setattr("janus_cli.plugins.get_plugin_manager", lambda: manager)
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("JANUS_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".janus" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        janus_home = tmp_path / "janus_test"
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under janus_plugins.<name>."""
        plugins_dir = tmp_path / "janus_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("JANUS_HOME", str(tmp_path / "janus_test"))

        # Clean up any prior namespace module
        sys.modules.pop("janus_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "janus_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="janus_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import janus_cli.plugins as plugins_mod
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("janus_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        janus_home = tmp_path / "janus_test"
        plugins_dir = janus_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "janus_test" / "plugins"
        # Set JANUS_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("janus_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("janus_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("janus_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("janus_cli.plugins.PluginContext.dispatch_tool.__module__", "janus_cli.plugins"):
...
class TestPluginDebugLogging:
    """JANUS_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("JANUS_PLUGINS_DEBUG", raising=False)
        from janus_cli import plugins as plugins_mod
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With JANUS_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("JANUS_PLUGINS_DEBUG", "1")
        from janus_cli import plugins as plugins_mod
