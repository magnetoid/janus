"""Tests for the update check mechanism in janus_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    with patch("janus_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_invalidates_on_version_change(tmp_path, monkeypatch):
    ...
    fake_banner = tmp_path / "janus_cli" / "banner.py"
    ...
    monkeypatch.setenv("JANUS_HOME", str(tmp_path))
    monkeypatch.delenv("JANUS_REVISION", raising=False)
    with patch("janus_cli.banner.subprocess.run") as mock_run, \
         patch("janus_cli.banner.check_via_pypi", return_value=0) as mock_pypi:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("JANUS_HOME", str(tmp_path))
    with patch("janus_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("JANUS_HOME", str(tmp_path))
    with patch("janus_cli.banner.subprocess.run") as mock_run:
        with patch("janus_cli.banner.check_via_pypi", return_value=0):
            result = banner.check_for_updates()
    assert result == 0
    mock_run.assert_not_called()
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when JANUS_HOME has no git repo."""
    import janus_cli.banner as banner
    ...
    # Point JANUS_HOME at a temp dir with no janus-agent/.git
    monkeypatch.setenv("JANUS_HOME", str(tmp_path))
    with patch("janus_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_docker_returns_none(tmp_path, monkeypatch):
    ...
    JANUS_REVISION (nix-only), so without a docker guard check_for_updates()
    ...
    container, and `janus update` correctly refusing to run there. The guard
    ...
    with patch("janus_cli.config.detect_install_method", return_value="docker"), \
         patch("janus_cli.banner.subprocess.run") as mock_run, \
         patch("janus_cli.banner.check_via_pypi") as mock_pypi:
        result = banner.check_for_updates()
...
def test_check_for_updates_non_docker_still_checks(tmp_path, monkeypatch):
    ...
    with patch("janus_cli.config.detect_install_method", return_value="pip"), \
         patch("janus_cli.banner.subprocess.run") as mock_run, \
         patch("janus_cli.banner.check_via_pypi", return_value=1) as mock_pypi:
        result = banner.check_for_updates()
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from janus_cli.main import _invalidate_update_cache

    # Build a fake ~/.janus with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"JANUS_HOME": str(default_home)}):
