...
def container_env(tmp_path, monkeypatch):
    """Set up a fake JANUS_HOME with .container-mode file."""
    janus_home = tmp_path / ".janus"
    janus_home.mkdir()
    monkeypatch.setenv("JANUS_HOME", str(janus_home))
    monkeypatch.delenv("JANUS_DEV", raising=False)

    container_mode = janus_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=janus-agent\n"
        "exec_user=janus\n"
        "janus_bin=/data/current-package/bin/janus\n"
    )
    return janus_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("janus_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "janus-agent"
    assert info["exec_user"] == "janus"
    assert info["janus_bin"] == "/data/current-package/bin/janus"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("janus_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_janus_dev(container_env, monkeypatch):
    """Returns None when JANUS_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("JANUS_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_janus_dev_zero(container_env, monkeypatch):
    """JANUS_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("JANUS_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        janus_home = Path(tmpdir) / ".janus"
        janus_home.mkdir()
        (janus_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("janus_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_janus_home": lambda: janus_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("JANUS_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "janus-custom"
    assert info["exec_user"] == "myuser"
    assert info["janus_bin"] == "/opt/janus/bin/janus"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from janus_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "janus"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "janus-agent" in cmd
    assert "/data/current-package/bin/janus" in cmd
    assert "chat" in cmd
