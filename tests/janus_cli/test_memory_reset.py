"""Tests for the `janus memory reset` CLI command.
...
- Profile-scoped reset (uses JANUS_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake JANUS_HOME with memory files."""
    janus_home = tmp_path / ".janus"
    memories = janus_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("JANUS_HOME", str(janus_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nJanus repo is at ~/.janus/janus-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `janus memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        janus_home = tmp_path / ".janus"
        (janus_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("JANUS_HOME", str(janus_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's JANUS_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_janus_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
