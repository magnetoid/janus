"""Regression tests for #34107 — Docker UID/GID handling in ensure_janus_home.

When Janus runs in Docker with ``JANUS_UID=1000`` / ``JANUS_GID=911``,
the entrypoint chowns the top-level ``JANUS_HOME`` once at startup. But
...
The fix is a ``_chown_to_janus_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_janus_uid_gid
# ---------------------------------------------------------------------------


class TestResolveJanusUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("JANUS_UID", "1000")
        monkeypatch.setenv("JANUS_GID", "911")
        from janus_cli.config import _resolve_janus_uid_gid
        uid, gid = _resolve_janus_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("JANUS_UID", raising=False)
        monkeypatch.delenv("JANUS_GID", raising=False)
        from janus_cli.config import _resolve_janus_uid_gid
        uid, gid = _resolve_janus_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("JANUS_UID", "not-a-number")
        monkeypatch.setenv("JANUS_GID", "911")
        from janus_cli.config import _resolve_janus_uid_gid
        uid, gid = _resolve_janus_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("JANUS_UID", "")
        monkeypatch.setenv("JANUS_GID", "")
        from janus_cli.config import _resolve_janus_uid_gid
        uid, gid = _resolve_janus_uid_gid()
        assert uid is None
        assert gid is None
...
class TestChownToJanusUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_janus_uid(d)  # must not raise
