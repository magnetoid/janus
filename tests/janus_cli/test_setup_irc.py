"""Tests for IRC gateway configuration via `janus setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Janus install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Janus install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#janus")
            monkeypatch.setenv("IRC_NICKNAME", "janus-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `janus setup gateway` experience with IRC present."""
