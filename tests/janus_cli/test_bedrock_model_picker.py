...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (JANUS_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via JANUS_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in JANUS_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from janus_cli.providers import JANUS_OVERLAYS
        assert "bedrock" in JANUS_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from janus_cli.providers import JANUS_OVERLAYS
        assert JANUS_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from janus_cli.providers import JANUS_OVERLAYS
        assert JANUS_OVERLAYS["bedrock"].auth_type == "aws_sdk"
