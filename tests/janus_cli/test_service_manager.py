"""Tests for janus_cli.service_manager — the abstract ServiceManager
...
def test_s6_running_true_when_comm_and_basedir_match(
    ...
    from janus_cli.service_manager import _s6_running
...
def test_s6_running_false_when_comm_unreadable(
    ...
    always-False inside the container under the janus user. The new
...
def test_systemd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "janus_cli.gateway.systemd_start", lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "janus_cli.gateway.systemd_stop", lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "janus_cli.gateway.systemd_restart", lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "janus_cli.gateway._probe_systemd_service_running",
...
def test_launchd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "janus_cli.gateway.launchd_start", lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "janus_cli.gateway.launchd_stop", lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "janus_cli.gateway.launchd_restart", lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "janus_cli.gateway._probe_launchd_service_running", lambda: False,
    )
    mgr = LaunchdServiceManager()
    mgr.start("ignored")
    mgr.stop("ignored")
...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # against the `janus_cli` package succeeds — gateway_windows is
    # imported lazily inside the wrapper and may not yet be loaded.
    import janus_cli.gateway_windows  # noqa: F401
    ...
    monkeypatch.setattr("janus_cli.gateway_windows", _FakeWindowsModule)
    monkeypatch.setattr(
        "janus_cli.gateway.find_gateway_pids",
        lambda **kw: [12345],
    )
    mgr = WindowsServiceManager()
    mgr.start("ignored")
...
def test_get_service_manager_returns_correct_backend(
    ...
    monkeypatch.setattr(
        "janus_cli.service_manager.detect_service_manager", lambda: kind,
    )
...
def test_get_service_manager_raises_when_unsupported(
    ...
    monkeypatch.setattr(
        "janus_cli.service_manager.detect_service_manager", lambda: "none",
    )
...
def test_get_service_manager_returns_s6_instance(
    ...
    monkeypatch.setattr(
        "janus_cli.service_manager.detect_service_manager", lambda: "s6",
    )
...
# unprivileged janus user from every lifecycle op. These tests run
...
def _expected_event_mode(base) -> int:
    """The mode ``event/`` can actually end up with on this host.

    ``_seed_supervise_skeleton`` requests ``0o3730`` (setgid + g+rwx + sticky).
    A new directory inherits its parent's gid, and when that gid is not one of
    ours the kernel silently drops ``S_ISGID`` on chmod. The test wrapper runs
    with ``TMPDIR=/tmp``, which on macOS is ``root:wheel`` — so the bit vanishes
    there while surviving on the Linux CI runner and under a user-owned TMPDIR.
    That made these assertions pass or fail purely on where pytest placed its
    tmp dir. Probe the actual filesystem instead of assuming.
    """
    import os
    import stat as _stat

    probe = base / "_setgid_probe"
    probe.mkdir()
    try:
        os.chmod(probe, 0o3730)
        kept = bool(_stat.S_IMODE(probe.stat().st_mode) & _stat.S_ISGID)
    finally:
        probe.rmdir()
    return 0o3730 if kept else 0o1730
...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    from janus_cli.service_manager import _seed_supervise_skeleton
    ...
    want_event_mode = _expected_event_mode(tmp_path)
    assert stat.S_IMODE(event.stat().st_mode) == want_event_mode, (
        f"event/ mode = {oct(event.stat().st_mode)}, want {oct(want_event_mode)}"
    )
    ...
    assert stat.S_IMODE(supervise_event.stat().st_mode) == want_event_mode
...
def test_seed_supervise_skeleton_handles_log_subservice(tmp_path) -> None:
    ...
    assert stat.S_IMODE(log_event.stat().st_mode) == _expected_event_mode(tmp_path)
...
def test_s6_register_creates_service_dir_and_triggers_scan(
    ...
    assert "janus -p coder gateway run" in run_text
    assert "s6-setuidgid janus" in run_text
...
def test_render_run_script_resets_home_before_exec() -> None:
    ...
    assert "export HOME=/opt/data" in run_text
    assert "exec s6-setuidgid janus janus -p coder gateway run" in run_text
...
def test_lifecycle_raises_gateway_not_registered_for_missing_slot(
    ...
    assert "'typo'" in msg
    assert "janus profile create typo" in msg
...
def test_lifecycle_raises_s6_command_error_on_subprocess_failure(
    ...
    import subprocess as _sp
    from janus_cli.service_manager import S6CommandError
