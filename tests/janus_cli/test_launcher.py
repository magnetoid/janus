"""Tests for the top-level `./janus` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./janus` should use `janus_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "janus"
    called = []

    fake_main_module = types.ModuleType("janus_cli.main")

    def fake_main():
        called.append("janus_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "janus_cli.main", fake_main_module)
