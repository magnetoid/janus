...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    janus_state = ModuleType("janus_state")
    janus_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "janus_state", janus_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Janus"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Janus"
