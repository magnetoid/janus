...
def server():
    with patch.dict(
        "sys.modules",
        {
            "janus_constants": MagicMock(
                get_janus_home=MagicMock(return_value="/tmp/janus_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'janus-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'janus-release' patched"
    }
