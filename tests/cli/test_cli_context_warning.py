...
def _isolate(tmp_path, monkeypatch):
    """Isolate JANUS_HOME so tests don't touch real config."""
    home = tmp_path / ".janus"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal JanusCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = JanusCLI.__new__(JanusCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["janus-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Janus' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Janus' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Janus' minimum context length."""
