...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("janus/janus-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned janus/* and pr-* branches."""

    def test_prunes_orphaned_janus_branch(self, git_repo):
        """janus/janus-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "janus/janus-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "janus/janus-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "janus/janus-deadbeef" in result.stdout
        ...
        assert "janus/janus-deadbeef" in orphaned
        ...
        assert "janus/janus-deadbeef" not in result.stdout
