...
- Path resolution (absolute, relative to JANUS_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp JANUS_HOME."""
    janus_home = tmp_path / ".janus"
    janus_home.mkdir()
    (janus_home / "cron").mkdir()
    (janus_home / "cron" / "output").mkdir()
    (janus_home / "scripts").mkdir()
    monkeypatch.setenv("JANUS_HOME", str(janus_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "JANUS_DIR", janus_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", janus_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", janus_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", janus_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.janus/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("JANUS_SESSION_PLATFORM") is None
        assert os.environ.get("JANUS_SESSION_CHAT_ID") is None
        assert os.environ.get("JANUS_SESSION_CHAT_NAME") is None
