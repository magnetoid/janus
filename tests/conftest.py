"""Shared fixtures for the janus-agent test suite.
...
2. **Isolated JANUS_HOME.** JANUS_HOME points to a per-test tempdir so
   code reading ``~/.janus/*`` via ``get_janus_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".janus"`` instead of the canonical
   ``get_janus_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No JANUS_SESSION_* inheritance** — the agent's current gateway
...
# JANUS_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_JANUS_BEHAVIORAL_VARS = frozenset({
    "JANUS_YOLO_MODE",
    "JANUS_INTERACTIVE",
    "JANUS_QUIET",
    "JANUS_TOOL_PROGRESS",
    "JANUS_TOOL_PROGRESS_MODE",
    "JANUS_MAX_ITERATIONS",
    "JANUS_SESSION_PLATFORM",
    "JANUS_SESSION_CHAT_ID",
    "JANUS_SESSION_CHAT_NAME",
    "JANUS_SESSION_THREAD_ID",
    "JANUS_SESSION_SOURCE",
    "JANUS_SESSION_KEY",
    "JANUS_GATEWAY_SESSION",
    "JANUS_CRON_SESSION",
    "_JANUS_GATEWAY",
    "JANUS_PLATFORM",
    "JANUS_MODEL",
    "JANUS_INFERENCE_MODEL",
    "JANUS_INFERENCE_PROVIDER",
    "JANUS_TUI_PROVIDER",
    "JANUS_MANAGED",
    "JANUS_DEV",
    "JANUS_CONTAINER",
    ...
    "JANUS_TIMEZONE",
    "JANUS_REDACT_SECRETS",
    "JANUS_BACKGROUND_NOTIFICATIONS",
    "JANUS_EXEC_ASK",
    "JANUS_HOME_MODE",
    "JANUS_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.janus/kanban.db instead of the per-test JANUS_HOME.
    "JANUS_KANBAN_DB",
    "JANUS_KANBAN_BOARD",
    "JANUS_KANBAN_HOME",
    "JANUS_KANBAN_WORKSPACES_ROOT",
    "JANUS_KANBAN_LOGS_ROOT",
    "JANUS_KANBAN_TASK",
    "JANUS_KANBAN_WORKSPACE",
    "JANUS_KANBAN_RUN_ID",
    "JANUS_KANBAN_CLAIM_LOCK",
    "JANUS_KANBAN_DISPATCH_IN_GATEWAY",
    "JANUS_TENANT",
    ...
    "JANUS_DASHBOARD_OAUTH_CLIENT_ID",
    "JANUS_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and JANUS_HOME to per-test tempdirs so code that
    reads ``~/.janus/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral JANUS_* vars that could change test semantics.
    for name in _JANUS_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect JANUS_HOME to a per-test tempdir. Code that reads
    #    ``~/.janus/*`` via ``get_janus_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.janus/*`` via
    #    ``Path.home() / ".janus"`` instead of ``get_janus_home()``
    #    is a bug to fix at the callsite.
    fake_janus_home = tmp_path / "janus_test"
    fake_janus_home.mkdir()
    (fake_janus_home / "sessions").mkdir()
    (fake_janus_home / "cron").mkdir()
    (fake_janus_home / "memories").mkdir()
    (fake_janus_home / "skills").mkdir()
    monkeypatch.setenv("JANUS_HOME", str(fake_janus_home))
    ...
    #    ~/.janus/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import janus_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_janus_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal janus config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``janus-gateway`` process
...
# correlated with a ``tests/janus_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> janus-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart janus-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting janus/python patterns are also blocked.
    """
    ...
    def _matches_janus_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _JANUS_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_janus_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch janus-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m janus_cli.main".
                if (
                    "janus" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live janus-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting janus/python could hit the live gateway. "
