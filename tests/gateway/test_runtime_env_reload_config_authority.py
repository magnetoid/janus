...
JANUS_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env JANUS_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_keeps_env_max_iterations_when_config_omits_key(
    ...
    janus_home = tmp_path / ".janus"
    janus_home.mkdir()
    (janus_home / "config.yaml").write_text(yaml.safe_dump({"agent": {}}), encoding="utf-8")
    (janus_home / ".env").write_text("JANUS_MAX_ITERATIONS=123\n", encoding="utf-8")
    ...
    assert os.environ["JANUS_MAX_ITERATIONS"] == "123"
