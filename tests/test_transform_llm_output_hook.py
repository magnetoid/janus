...
from janus_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(janus_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <janus_home>/plugins/<name> and opt it in."""
    plugin_dir = janus_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("JANUS_HOME", str(tmp_path / "janus_empty"))
