...
class TestJanusConstantsFallback:
    """Tests for _janus_home.py fallback when janus_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_janus_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _janus_home.py with janus_constants blocked."""
        monkeypatch.setitem(sys.modules, "janus_constants", None)
        spec = importlib.util.spec_from_file_location("_janus_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_janus_home_env_var(self, monkeypatch, tmp_path):
        """When janus_constants is missing, JANUS_HOME comes from env var."""
        monkeypatch.setenv("JANUS_HOME", str(tmp_path / "custom-janus"))
        module = self._load_helper(monkeypatch)
        assert module.get_janus_home() == tmp_path / "custom-janus"

    def test_fallback_defaults_to_dot_janus(self, monkeypatch):
        """When janus_constants is missing and JANUS_HOME unset, default to ~/.janus."""
        monkeypatch.delenv("JANUS_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_janus_home() == Path.home() / ".janus"

    def test_fallback_ignores_empty_janus_home(self, monkeypatch):
        """Empty/whitespace JANUS_HOME is treated as unset."""
        monkeypatch.setenv("JANUS_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_janus_home() == Path.home() / ".janus"

    def test_fallback_display_janus_home_shortens_path(self, monkeypatch):
        """Fallback display_janus_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("JANUS_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_janus_home() == "~/.janus"

    def test_fallback_display_janus_home_profile_path(self, monkeypatch):
        """Fallback display_janus_home() handles profile paths under ~/."""
        monkeypatch.setenv("JANUS_HOME", str(Path.home() / ".janus/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_janus_home() == "~/.janus/profiles/coder"

    def test_fallback_display_janus_home_custom_path(self, monkeypatch):
        """Fallback display_janus_home() returns full path for non-home locations."""
        monkeypatch.setenv("JANUS_HOME", "/opt/janus-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_janus_home() == "/opt/janus-custom"

    def test_delegates_to_janus_constants_when_available(self):
        """When janus_constants IS importable, _janus_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_janus_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import janus_constants
        assert module.get_janus_home is janus_constants.get_janus_home
        assert module.display_janus_home is janus_constants.display_janus_home
...
class TestInstallDeps:
    ...
    Regression coverage for the Janus Docker image, whose venv is built with
    ...
    def test_returns_false_when_no_pip_and_no_uv(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "janus-agent[google]" in out
