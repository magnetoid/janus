"""
Tests for timezone support (janus_time module + integration points).
...
def _reset_janus_time_cache():
    """Reset the janus_time module cache (replacement for removed reset_cache)."""
    janus_time._cached_tz = None
    janus_time._cached_tz_name = None
    janus_time._cache_resolved = False


# =========================================================================
# janus_time.now() — core helper
# =========================================================================

class TestJanusTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_janus_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["JANUS_TIMEZONE"] = "Asia/Kolkata"
        result = janus_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["JANUS_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="janus_time"):
            result = janus_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["JANUS_TIMEZONE"] = "Europe/London"
        tz = janus_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["JANUS_TIMEZONE"] = "Not/A/Timezone"
        tz = janus_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When JANUS_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) JANUS_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("JANUS_TIMEZONE=" + os.environ.get("JANUS_TIMEZONE", "NOT_SET"))'
        )
    ...
    def test_tz_not_injected_when_empty(self):
        """When JANUS_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("JANUS_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=janus_tz) which shifted
        absolute time when system-local tz != Janus tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_janus_tz(self):
        """Already-aware datetimes should be normalized to Janus tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Janus tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Janus tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Janus is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Janus tz
        ...
        # Use a Janus timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _janus_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["JANUS_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_janus_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Janus timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["JANUS_TIMEZONE"] = "US/Eastern"
        _reset_janus_time_cache()
