...
Run from the PR worktree:
    ...
    cd ~/.janus/janus-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.janus/janus-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.janus/janus-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a janus-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a janus-agent checkout"
)
...
# Isolated JANUS_HOME for the config write.
