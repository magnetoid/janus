...
""",
        ...
        from janus_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        janus_home = tmp_path / ".janus"
        janus_home.mkdir()
        (janus_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("JANUS_HOME", str(janus_home))
        from janus_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from janus_cli.plugins import PluginContext, PluginManifest, PluginManager
