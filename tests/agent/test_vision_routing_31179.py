...
def isolated_home(monkeypatch):
    """Temp JANUS_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="janus_test_31179_")
    janus_home = os.path.join(test_home, ".janus")
    os.makedirs(janus_home)
...
def _fresh_modules():
    """Drop cached janus modules so each test reloads against current env."""
