...
engine plugins (e.g. janus-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``janus plugins`` picker."""
    from janus_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "janus_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "janus_cli.plugins.get_plugin_context_engine",
