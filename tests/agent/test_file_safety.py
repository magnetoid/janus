...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_janus_env(self):
        """Janus' own .env inside JANUS_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: janus internal .env is in ~/.janus/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.janus/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.janus/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Janus cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        janus_home = tmp_path / ".janus"
        cache = janus_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._janus_home_path", return_value=janus_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Janus cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        janus_home = tmp_path / ".janus"
        hub = janus_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_janus_home(self, tmp_path):
        """The env basename guard does not depend on JANUS_HOME resolution."""
        janus_home = tmp_path / ".janus"
        janus_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        janus_home = tmp_path / ".janus"
        cache = janus_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
