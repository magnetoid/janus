...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Janus passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("JANUS_HOME", "/users/alice/.janus/profiles/backend-worker")
        monkeypatch.setenv("JANUS_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "JANUS_KANBAN_DB",
            "/users/alice/.janus/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.janus/kanban/boards/smoke"]'
            in cmd
        )
