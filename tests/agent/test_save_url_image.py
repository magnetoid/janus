...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate JANUS_HOME under tmp_path."""
    monkeypatch.setenv("JANUS_HOME", str(tmp_path / ".janus"))
    (tmp_path / ".janus").mkdir()

    # Force the constants/image cache helpers to re-read JANUS_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("janus_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_janus_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under JANUS_HOME — gateway cleanup
