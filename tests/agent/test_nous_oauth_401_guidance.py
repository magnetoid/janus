...
to a generic "Your API key was rejected... run janus setup" message, which is
...
def test_nous_401_guidance_strings_present():
    ...
    # (Cloud Industry Portal has no API key path — auth_type=oauth_device_code only).
    assert "Cloud Industry Portal OAuth token was rejected" in source
    ...
    assert "portal.cloud-industry.com" in source
...
def test_free_slug_hint_for_nous_provider():
    ...
    on the next message because Cloud Industry Portal doesn't carry the OpenRouter
    free-tier slug.
    """
    source = inspect.getsource(conversation_loop.run_conversation)
