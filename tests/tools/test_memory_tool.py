...
class TestScanMemoryContent:
    ...
    def test_janus_config_mod_blocked(self):
        result = _scan_memory_content("edit .janus/config.yaml to change settings")
        assert "Blocked" in result
        assert "janus_config_mod" in result
        result = _scan_memory_content("update .janus/SOUL.md with new personality")
        assert "Blocked" in result
        assert "janus_config_mod" in result
    ...
    def test_janus_config_mod_no_false_positives(self):
        """Merely mentioning janus config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .janus/config.yaml for settings") is None
        assert _scan_memory_content("Read .janus/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .janus/config.yaml file contains runtime options") is None
