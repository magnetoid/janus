...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._janus_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(janus_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(janus_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_janus_bin(self):
        """After _INSTALL_FAILED, manual install in JANUS_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        janus_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(janus_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(janus_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._janus_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == janus_bin
            assert _tirith_mod._resolved_path == janus_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# JANUS_HOME isolation
# ---------------------------------------------------------------------------

class TestJanusHomeIsolation:
    def test_janus_bin_dir_respects_janus_home(self):
        """_janus_bin_dir must use JANUS_HOME, not hardcoded ~/.janus."""
        from tools.tirith_security import _janus_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"JANUS_HOME": tmpdir}):
            result = _janus_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_janus_home(self):
        """_failure_marker_path must use JANUS_HOME, not hardcoded ~/.janus."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"JANUS_HOME": "/custom/janus"}):
            result = _failure_marker_path()
        assert result == "/custom/janus/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets JANUS_HOME; verify it's active."""
        janus_home = os.getenv("JANUS_HOME")
        assert janus_home is not None, "JANUS_HOME should be set by conftest"
        assert "janus_test" in janus_home, "Should point to test temp dir"

    def test_get_janus_home_fallback(self):
        """Without JANUS_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_janus_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove JANUS_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".janus")
            result = _get_janus_home()
