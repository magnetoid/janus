...
class TestWriteDenyExactPaths:
    ...
    def test_janus_env(self):
        # ``.env`` under the active JANUS_HOME (profile-aware, not just
        # ``~/.janus``) must be write-denied. The hermetic test conftest
        # points JANUS_HOME at a tempdir — resolve via get_janus_home()
        # to match the denylist.
        from janus_constants import get_janus_home
        path = str(get_janus_home() / ".env")
        assert _is_write_denied(path) is True

    def test_janus_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.janus/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "janus_root"
        ...
        # Sanity check: JANUS_HOME does point to the profile dir, not the root.
        from janus_constants import get_janus_home, get_default_janus_root
        assert get_janus_home() == profile_home
        assert get_default_janus_root() == root
...
class TestWriteAllowed:
    ...
    def test_janus_config_not_env(self):
        path = os.path.join(str(Path.home()), ".janus", "config.yaml")
