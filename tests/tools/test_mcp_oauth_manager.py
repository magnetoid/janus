...
def test_janus_provider_subclass_exists():
    """JanusMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _JANUS_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _JANUS_PROVIDER_CLS is not None
    assert issubclass(_JANUS_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_janus_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns JanusMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _JANUS_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("JANUS_HOME", str(tmp_path))
    ...
    assert _JANUS_PROVIDER_CLS is not None
    assert isinstance(provider, _JANUS_PROVIDER_CLS)
    assert provider._janus_server_name == "srv"
