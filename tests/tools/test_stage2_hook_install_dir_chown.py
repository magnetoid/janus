...
When JANUS_UID is remapped at container boot, ``usermod -u`` only rewrites
files under the janus user's home directory ($JANUS_HOME == /opt/data).
Runtime-writable trees under ``/opt/janus`` must be explicitly chowned to the
new UID before services drop privileges. ``/opt/janus/gateway`` is one such
