...
def _restore_tool_modules():
    original_janus_home = os.environ.get("JANUS_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_janus_home is None:
            os.environ.pop("JANUS_HOME", None)
        else:
            os.environ["JANUS_HOME"] = original_janus_home
        _reset_modules(("tools", "janus_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    os.environ["JANUS_HOME"] = str(janus_home)
    sys.modules["janus_cli.config"] = types.SimpleNamespace(
        get_janus_home=lambda: janus_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="janus-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": janus_home / "modal_snapshots.json",
