...
def _clean_state():
    ...
    saved = {}
    for k in ("JANUS_INTERACTIVE", "JANUS_GATEWAY_SESSION", "JANUS_EXEC_ASK", "JANUS_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["JANUS_INTERACTIVE"] = "1"
        session_key = os.getenv("JANUS_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No JANUS_INTERACTIVE or JANUS_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
