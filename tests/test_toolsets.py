...
class TestToolsetConsistency:
    ...
    def test_janus_platforms_share_core_tools(self):
        """All janus-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on janus-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["janus-cli", "janus-telegram", "janus-discord", "janus-whatsapp", "janus-slack", "janus-signal", "janus-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_janus_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("janus-whatsapp")
