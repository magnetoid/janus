...
binary download are exercised manually by `janus secrets bitwarden
...
def janus_home(tmp_path, monkeypatch):
    """Point Janus at an isolated home directory."""
    home = tmp_path / ".janus"
    home.mkdir()
    monkeypatch.setenv("JANUS_HOME", str(home))
    # Some modules cache get_janus_home; clear if needed.
    import janus_constants
    if hasattr(janus_constants, "_JANUS_HOME_CACHE"):
        janus_constants._JANUS_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(janus_home, monkeypatch):
...
def test_install_bws_happy_path(janus_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(janus_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(janus_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from janus_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under janus_home/cache/."""
    home = tmp_path / ".janus"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
