"""Tests for ACP Registry metadata shipped with Janus."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "janus-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Janus Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/magnetoid/janus"
    assert data["website"].startswith("https://github.com/magnetoid/janus")
    assert data["authors"] == ["Cloud Industry"]
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"janus-agent[acp]=={data['version']}"
    assert uvx["args"] == ["janus-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"janus-agent[acp]=={_pyproject_version()}"
    )
