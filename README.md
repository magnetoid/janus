...
Use any model you want — [Cloud Industry Portal](https://portal.cloud-industry.com), [OpenRouter](https://openrouter.ai) (200+ models), [NovitaAI](https://novita.ai) (AI-native cloud for Model API, Agent Sandbox, and GPU Cloud), [NVIDIA NIM](https://build.nvidia.com) (Nemotron), [Xiaomi MiMo](https://platform.xiaomimimo.com), [z.ai/GLM](https://z.ai), [Kimi/Moonshot](https://platform.moonshot.ai), [MiniMax](https://www.minimax.io), [Hugging Face](https://huggingface.co), OpenAI, or your own endpoint. Switch with `janus model` — no code changes, no lock-in.
...
janus migrate hermes # Import a previous install (~/.hermes)
...
| **Reflexion lessons** | A failed session distills one transferable *"do X instead"* lesson keyed to the task type; the agent recalls it before the next similar task, so mistakes compound into improvement. | `recall_lessons` tool · auto on failure |
...
| **Consensus / smart routing** | Routes each task to the **right-cost model** — the cheapest model for simple work, a strong model for hard work, and an **ensemble of top models** (synthesized via MoA) for the hardest. Saves tokens on easy work, lifts quality on hard. Cache-safe (routes at task entry). | `janus consensus route "…"` · `janus setup` |
...
| **Measurable self-improvement** | A self-growing eval **learning curve** (pass-rate over time) plus continual-learning metrics — forward/backward transfer, forgetting, and a skill-diversity **collapse early-warning**. Failed sessions auto-draft quarantined regression-pin evals. Proves the loop is actually getting smarter, not just bigger. | `janus evals trend` · `janus learning stats` |
| **ACE playbook** (recursive) | The loop improves its **own prompts**: distills guidance from its recent activity and prepends it to the memory-mining / lesson steps. Admitted only through the red-team gate (fails closed) and capped, so it can't bloat or collapse. The one place Janus tunes its own learning machinery — safely. | `janus learning playbook` · `learning.playbook.enabled: true` |

The slash variants (`/memory`, `/aspire`, `/interest`) work from the messaging gateway (Telegram, Discord, …) and the TUI too.

---

## Skip the API-key collection — Cloud Industry Portal
...
If you have an older install with data in `~/.hermes`, Janus imports it for you (non-destructively — your `~/.hermes` is left untouched until you choose to delete it).
...
janus migrate hermes --source PATH  # Point at a previous install's home elsewhere
```
...
Built by [Cloud Industry](https://cloud-industry.com).
