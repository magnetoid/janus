...
# 5. Symlinks the 'janus' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Janus Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/janus-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink janus into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up janus command..."

JANUS_BIN="$SCRIPT_DIR/venv/bin/janus"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$JANUS_BIN" "$COMMAND_LINK_DIR/janus"
echo -e "${GREEN}✓${NC} Symlinked janus → $COMMAND_LINK_DISPLAY_DIR/janus"
...
# ============================================================================
# Seed bundled skills into ~/.janus/skills/
# ============================================================================

JANUS_SKILLS_DIR="${JANUS_HOME:-$HOME/.janus}/skills"
mkdir -p "$JANUS_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.janus/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$JANUS_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  janus status        # Check configuration"
if is_termux; then
    echo "  janus gateway       # Run gateway in foreground"
else
    echo "  janus gateway install # Install gateway service (messaging + cron)"
fi
echo "  janus cron list     # View scheduled jobs"
echo "  janus doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m janus_cli.main setup
fi
