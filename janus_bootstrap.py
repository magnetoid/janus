"""Windows UTF-8 bootstrap for Janus entry points.
...
should be imported at the very top of every Janus entry point
(``janus``, ``janus-agent``, ``janus-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import janus_bootstrap``
# (or ``from janus_bootstrap import apply_windows_utf8_bootstrap``) at
