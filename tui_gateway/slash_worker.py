"""Persistent slash-command worker — one JanusCLI per TUI session.
...
def _run(cli: JanusCLI, command: str) -> str:
...
def main():
    ...
    os.environ["JANUS_SESSION_KEY"] = args.session_key
    os.environ["JANUS_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = JanusCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
