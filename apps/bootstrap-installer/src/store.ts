...
export const $janusHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  try {
    const [logPath, janusHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_janus_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $janusHome.set(janusHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Janus" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  // Update is driven by the desktop handing off (Janus-Setup.exe --update);
...
export async function launchJanusDesktop(): Promise<void> {
  const installRoot = $bootstrap.get().installRoot
  if (!installRoot) throw new Error('no install root')
  await invoke('launch_janus_desktop', { installRoot })
}
