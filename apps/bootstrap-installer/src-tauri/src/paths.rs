...
//!
//! Mirrors `janus_constants.get_janus_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\janus
//!   macOS:   ~/.janus
//!   Linux:   ~/.janus  (override via $JANUS_HOME)
//!
//! NOTE (macOS): Python's get_janus_home(), scripts/install.sh, and the
//! Electron desktop's resolveJanusHome() ALL use ~/.janus on macOS — there
...
/// Returns the canonical Janus home directory, respecting $JANUS_HOME if set.
pub fn janus_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("JANUS_HOME") {
    ...
    {
        // %LOCALAPPDATA%\janus — matches scripts/install.ps1's $JanusHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("janus");
        }
    }

    // macOS + Linux + fallback: ~/.janus (matches Python get_janus_home(),
    // install.sh, and the Electron desktop's resolveJanusHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".janus");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    janus_home().join("bootstrap-cache")
}
...
/// JANUS_HOME so it survives repo checkout deletion (unlike anything under
/// janus-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\janus\janus-setup.exe`; on other
...
pub fn copy_self_to_janus_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to JANUS_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_JANUS_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".janus-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under JANUS_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[janus-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("JANUS_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_janus_home() -> String {
    janus_home().to_string_lossy().into_owned()
}
