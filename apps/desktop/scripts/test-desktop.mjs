...
// no longer bundles the Janus Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Janus.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Janus'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Janus.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'janus'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default JANUS_HOME for non-sandboxed runs -- matches main.cjs's
// resolveJanusHome(). On Windows it's %LOCALAPPDATA%\janus; elsewhere
// it's ~/.janus. The fresh-install sandbox launchFresh() sets its own
// JANUS_HOME and never touches this.
const DEFAULT_JANUS_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'janus')
  }
  return path.join(os.homedir(), '.janus')
})()
const VENV_ROOT = path.join(DEFAULT_JANUS_HOME, 'janus-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'janus-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.JANUS_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Janus-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Janus-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Janus-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Janus-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.JANUS_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.JANUS_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const janusHome = path.join(sandbox, 'janus-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(janusHome, { recursive: true })
  ...
  env.JANUS_DESKTOP_CWD = cwd
  env.JANUS_DESKTOP_IGNORE_EXISTING = '1'
  env.JANUS_DESKTOP_TEST_MODE = 'fresh-install'
  env.JANUS_DESKTOP_USER_DATA_DIR = userDataDir
  env.JANUS_HOME = janusHome
  delete env.JANUS_DESKTOP_JANUS
  delete env.JANUS_DESKTOP_JANUS_ROOT
  ...
  console.log(`  JANUS_HOME: ${janusHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(janusHome, 'janus-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Janus Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of janus_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'janus-agent', 'janus_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Janus
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + JANUS_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  JANUS_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
