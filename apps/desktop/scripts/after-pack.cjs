 ...
 *
 * Stamps the Janus icon + identity onto the packed Windows Janus.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `janus desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Janus')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Janus'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Janus.exe keeps the stock Electron icon`)
  }
}
