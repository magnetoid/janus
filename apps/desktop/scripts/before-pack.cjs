 ...
 * product name (`Janus`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Janus'
 *
 * This is a hard failure with no obvious cause for the user — `janus desktop`
 ...
 * (the mac-unpacked Janus.app bundle) and Windows (win-unpacked), so we
