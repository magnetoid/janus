...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'janus_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'janus_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'janus_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Janus.app/Contents/MacOS/Janus', 'darwin'),
    '/Applications/Janus.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Janus.app/Contents/MacOS/Janus', 'darwin'),
    '/Users/x/Applications/Janus.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Janus\\Janus.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Janus'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\janus-desktop\\Janus.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\janus-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Janus.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_JanusXXXX/janus', 'linux', { APPIMAGE: '/home/x/Apps/Janus.AppImage' }),
    '/home/x/Apps/Janus.AppImage'
  )
})

test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(
    resolveRemovableAppPath('/opt/janus/linux-unpacked/janus', 'linux', {}),
    '/opt/janus/linux-unpacked'
  )
  // A system-package install (/usr/bin) → null, left to apt/dnf.
  assert.equal(resolveRemovableAppPath('/usr/bin/janus', 'linux', {}), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Janus.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Janus.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.janus/janus-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.janus/janus-agent',
    uninstallArgs: ['-m', 'janus_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/janus/linux-unpacked',
    janusHome: '/home/x/.janus'
  })
  ...
  assert.match(script, /'-m' 'janus_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/janus\/linux-unpacked'/)
  assert.match(script, /export JANUS_HOME='\/home\/x\/\.janus'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.janus/janus-agent',
    agentRoot: '/home/x/.janus/janus-agent',
    uninstallArgs: ['-m', 'janus_cli.uninstall', '--mode', 'full'],
    appPath: null,
    janusHome: '/home/x/.janus'
  })
  // System python + source on PYTHONPATH so import janus_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.janus\/janus-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'janus_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'janus_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    janusHome: '/h'
  })
  assert.doesNotMatch(script, /rm -rf '\//)
  // Still runs the uninstall.
  assert.match(script, /'-m' 'janus_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\janus',
    uninstallArgs: ['-m', 'janus_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Janus',
    janusHome: 'C:\\Users\\x\\AppData\\Local\\janus'
  })
  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import janus_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\janus;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "janus_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Janus" >nul 2>&1/)
