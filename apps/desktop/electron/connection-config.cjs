 ...
 *     `X-Janus-Session-Token` header; WS uses `?token=`.
...
// janus_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - janus_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - janus_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / janus #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-janus_session_at', '__Secure-janus_session_at', 'janus_session_at']
const RT_COOKIE_VARIANTS = ['__Host-janus_session_rt', '__Secure-janus_session_rt', 'janus_session_rt']
...
/**
 * True if any cookie in `cookies` is a janus session ACCESS-token cookie
