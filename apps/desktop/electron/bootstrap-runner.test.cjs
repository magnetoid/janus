...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'janus-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/janus-runner-test',
    sourceRepoRoot: null,
    janusHome: '/tmp/janus-runner-test',
    logRoot: '/tmp/janus-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'janus-agent', 'scripts')
