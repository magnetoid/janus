...
import { getJanusConfigRecord, type JanusConfigRecord, saveJanusConfig } from '@/janus'
...
export interface I18nConfigClient {
  getConfig: () => Promise<JanusConfigRecord>
  saveConfig: (config: JanusConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.janusDesktop?.api) {
      return Promise.resolve({})
    }

    return getJanusConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.janusDesktop?.api) {
...
export function getConfigDisplayLanguage(config: JanusConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: JanusConfigRecord, locale: Locale): JanusConfigRecord {
