...
import { getProfiles, setApiRequestProfile } from '@/janus'
...
import type { ProfileInfo } from '@/types/janus'
...
// /api/profiles/active `current`). "default" is the root ~/.janus. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'janus.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'janus.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.janusDesktop.api<ActiveProfileResponse>({ path: '/api/profiles/active' })
...
// Persist the choice and relaunch the backend under the new JANUS_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.janusDesktop.profile.set(name)
}
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'janus.desktop.showAllProfiles'
...
// Switch to the default (root ~/.janus) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.janusDesktop?.touchBackend?.(target).catch(() => undefined)
}
