...
const UPDATE_TOAST_SNOOZE_KEY = 'janus:update-toast-snooze-until'
...
export function reportBackendContract(contract: number | undefined): void {
  ...
  notify({
    action: { label: translateNow('notifications.updateJanus'), onClick: () => void applyUpdates() },
    durationMs: 0,
    id: SKEW_TOAST_ID,
    kind: 'warning',
...
export async function refreshDesktopVersion(): Promise<DesktopVersionInfo | null> {
  ...
  try {
    const next = await window.janusDesktop?.getVersion?.()
...
export async function checkUpdates(): Promise<DesktopUpdateStatus | null> {
  const bridge = window.janusDesktop?.updates
  ...
  try {
    ...
    // The update check pulls the latest janus_cli + bundled package metadata
...
export async function applyUpdates(opts: DesktopUpdateApplyOptions = {}): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    // `janus update` themselves. Land on a dedicated manual state so the
    ...
    if (result?.manual) {
      $updateApply.set({
        ...
        applying: false,
        stage: 'manual',
        message: result.command ?? 'janus update',
        command: result.command ?? 'janus update'
      })
    }
...
function onFocus() {
  ...
  // updated Janus from another window/CLI between focuses, and About should
