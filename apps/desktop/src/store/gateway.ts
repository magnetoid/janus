import type { ConnectionState, GatewayEvent } from '@janus/shared'
...
const isOpen = (gateway: JanusGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<JanusGateway | null>(null)
...
let primaryGateway: JanusGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: JanusGateway | null, profile = 'default'): void {
...
export function activeGateway(): JanusGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<JanusGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
