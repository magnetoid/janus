...
async function mediaSrc(path: string): Promise<string> {
  ...
  if (window.janusDesktop && ['audio', 'video'].includes(mediaKind(path))) {
  ...
  if (window.janusDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.janusDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.janusDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
function OpenMediaButton({ kind, path }: { kind: 'audio' | 'video'; path: string }) {
  return (
    <button
      ...
      onClick={() => void window.janusDesktop?.openExternal(mediaExternalUrl(path))}
      type="button"
    >
      Open {kind} file
    </button>
