...
import { formatRefValue, janusDirectiveFormatter } from './directive-text'
...
describe('janusDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = janusDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = janusDirectiveFormatter.parse('@file:src/main.tsx the entry point')
