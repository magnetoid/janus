...
const JANUS_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type JanusRefType = (typeof JANUS_REF_TYPES)[number]
...
const ICON_PATHS: Record<JanusRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as JanusRefType] ?? ICON_FALLBACK
...
const JANUS_DIRECTIVE_RE = new RegExp(
...
export const janusDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(JANUS_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as JanusRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: JanusRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing Janus directives (`@file:...`, `@image:...`) as
...
export function DirectiveContent({ text }: { text: string }) {
  ...
  const segments = useMemo(() => janusDirectiveFormatter.parse(cleanedText), [cleanedText])
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.janusDesktop && isRemoteGateway()
        ? gatewayMediaDataUrl(id)
        : window.janusDesktop?.readFileDataUrl(id)
