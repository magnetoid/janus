...
import { getJanusConfig, getJanusConfigDefaults } from '@/janus'
...
interface JanusConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useJanusConfig({ activeSessionIdRef, refreshProjectBranch }: JanusConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshJanusConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getJanusConfig(), getJanusConfigDefaults().catch(() => ({}))])
  ...
  return { refreshJanusConfig, sttEnabled, voiceMaxRecordingSeconds }
}
