...
import type { RpcEvent } from '@/types/janus'

interface GatewayBootOptions {
  handleGatewayEvent: (event: RpcEvent) => void
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.janusDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: JanusGateway | null) => void
  refreshJanusConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: JanusConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Janus...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // as an opaque "Could not connect to Janus gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshJanusConfig().catch(() => undefined)
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshJanusConfig()
