...
// bug that drives the REAL useGatewayBoot hook + REAL JanusGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshJanusConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { janusDesktop?: unknown }).janusDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { janusDesktop?: unknown }).janusDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForJanus) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startJanus()'s remote branch awaits waitForJanus() for 45s before it
    ...
    ;(window as { janusDesktop?: unknown }).janusDesktop = desktop
    ...
    // After ~45s waitForJanus gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Janus backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
