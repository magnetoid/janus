...
const DESKTOP_COMMAND_META = [
  ...
  ['/profile', 'Switch the active Janus profile'],
  ...
  ['/version', 'Show Janus Agent version'],
...
export function isDesktopSlashCommand(command: string): boolean {
  ...
  return DESKTOP_COMMANDS.has(canonical) || !isKnownJanusSlashCommand(normalized)
}

/**
 * An "extension" command is anything the backend surfaces that is NOT one of
 * Janus' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownJanusSlashCommand(normalized)
}
...
function isKnownJanusSlashCommand(command: string): boolean {
