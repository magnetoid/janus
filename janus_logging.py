"""Centralized logging setup for Janus Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.janus/logs/`` (profile-aware via ``get_janus_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``janus_cli.web_server``,
    ``janus_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
from janus_constants import get_config_path, get_janus_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_janus_session_injector", False):
...
# Used by _ComponentFilter and exposed for ``janus logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "janus_plugins"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("janus_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "janus_cli.web_server",
        "janus_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Janus logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    janus_home
        Override for the Janus home directory.  Falls back to
        ``get_janus_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_janus_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._janus_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the janus group. However, both ``_open()`` (initial creation)
