...
_JANUS_CORE_TOOLS = [
    ...
    "pin_agreement", "recall_agreements", "propose_plan",
    ...
    # spawned as a kanban worker (JANUS_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `janus tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "moa": {
        ...
        "tools": ["mixture_of_agents", "deep_reason", "deliberate"],
        "includes": []
    },
    ...
    "browser": {
        ...
        "tools": [
            ...
            "browser_dialog", "web_search", "pagemem_remember"
        ],
        "includes": []
    },
    ...
    "memory": {
        ...
        "tools": ["memory", "recall_memory", "recall_lessons", "pin_agreement", "recall_agreements", "propose_plan"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (JANUS_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full Janus toolsets (CLI + messaging platforms)
    #
    ...
    "janus-cli": {
        ...
        "tools": _JANUS_CORE_TOOLS,
        "includes": []
    },

    "janus-cron": {
        # Mirrors janus-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `janus tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as janus-cli; gated by `janus tools`",
        "tools": _JANUS_CORE_TOOLS,
        "includes": []
    },
    ...
    "janus-email": {
        "description": "Email bot toolset - interact with Janus via email (IMAP/SMTP)",
        "tools": _JANUS_CORE_TOOLS,
        "includes": []
    },
    ...
    "janus-sms": {
        "description": "SMS bot toolset - interact with Janus via SMS (Twilio)",
        "tools": _JANUS_CORE_TOOLS,
        "includes": []
    },
    ...
    "janus-webhook": {
        ...
        "tools": _JANUS_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "janus-gateway": {
        ...
        "tools": [],
        "includes": ["janus-telegram", "janus-discord", "janus-whatsapp", "janus-slack", "janus-signal", "janus-bluebubbles", "janus-homeassistant", "janus-email", "janus-sms", "janus-mattermost", "janus-matrix", "janus-dingtalk", "janus-feishu", "janus-wecom", "janus-wecom-callback", "janus-weixin", "janus-qqbot", "janus-webhook", "janus-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (janus-<name>).
        # Gives them _JANUS_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("janus-"):
            platform_name = name[len("janus-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_JANUS_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
